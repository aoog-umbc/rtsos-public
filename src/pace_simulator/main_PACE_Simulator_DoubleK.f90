...
REAL*8 :: LBDO_LD,FLAM,CHLa,pBRDFa_input,pBRDFk_input,pBRDFb_input,pBRDFe_input,&
          fiso_input,fvol_input,fgeo_input
...
DO IWV_BAND=NWV_BAND_START,NWV_BAND_END
  ...
  IF(IWV_BAND >= NWV_BAND_SEG1_HYPO .or. IWV_BAND == NWV_BAND_START)THEN
     RTMP=1.0D0
  ELSE
     RTMP=0.5D0
  ENDIF
...
! testing absorption block
  ...
  DO IWV_CAL=1,NWV_SUB_CAL
        ...
        CALL RTSOSINIT(Indx_Inelastic_Scattering,INDX_WV_CAL(IWV_CAL),&
           WAVELENGTH_MICRON,NREC,NTLYERA,NTLYERO,PNDLY,ARSL_TYPE,TAUR,DEPOL_A,TAU_TG,       &
           RECDATASTREAM,NTLYER,NUMMIERT,MAXLORDINPUT,NUMMIEANGINPUT, &
           PHMXDATASTREAM,LBDO_LD,FLAM,CHLa,TEMPERTURE,SALINITY,pBRDFa_input,pBRDFk_input,pBRDFb_input,pBRDFe_input,&
           fiso_input,fvol_input,fgeo_input)
...
IF(ILS_FLAG)THEN
  ...
  DO IWV_AVG=NWV_START,NWV_END
     ...
     DO ILAYER=1,NTLYERA
        ...
        IF((WV_SEG_FLAG==4 .AND. MONOCHROMATIC_FLAG) .or. WV_BAND(IWV_AVG)>890D0)THEN
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'OCEAN_CASE_SELECT', H5T_NATIVE_INTEGER, space, dset, hdferr)
...
IF((WV_SEG_FLAG==4) .or. (WV_SEG_FLAG<4 .AND. WAVELENGTH_NANOMETER>890D0))THEN
...
SUBROUTINE RTSOSINIT(Indx_Inelastic_Scattering,IWVSAVE,WAVELENGTH_MICRON,&
       NREC,NTLYERA,NTLYERO,PNDLY,ARSL_TYPE,TAUR,DEPOL_A,TAU_TG,RECDATASTREAM,NTLYER,NUMMIERT,  &
       MAXLORDINPUT,NUMMIEANGINPUT,PHMXDATASTREAM,LBDO_LD,FLAM,CHLa,TEMPERTURE,SALINITY,&
       pBRDFa_input,pBRDFk_input,pBRDFb_input,pBRDFe_input,fiso_input,fvol_input,fgeo_input)
...
REAL*8,INTENT(IN)::WAVELENGTH_MICRON,CHLa,pBRDFa_input,pBRDFk_input,&
                    pBRDFb_input,pBRDFe_input,fiso_input,fvol_input,fgeo_input
...
! mRPV LAND BOTTOM
    ...
    RECDATASTREAM(NTLYERA+IREC_SHFT,4)=pBRDFe_input
	RECDATASTREAM(NTLYERA+IREC_SHFT,5)=NMBREINPUT
	RECDATASTREAM(NTLYERA+IREC_SHFT,6)=NMBIMINPUT
	RETURN
ENDIF
...
! Snow LAND BOTTOM
	...
	RECDATASTREAM(NTLYERA+IREC_SHFT,2)=fiso_input
	RECDATASTREAM(NTLYERA+IREC_SHFT,3)=fvol_input
	RECDATASTREAM(NTLYERA+IREC_SHFT,4)=fgeo_input
	RECDATASTREAM(NTLYERA+IREC_SHFT,5)=0.0d0
	RECDATASTREAM(NTLYERA+IREC_SHFT,6)=0.0d0
	RETURN
ENDIF
