...
INTEGER :: WV_SEG_FLAG !wv_seg_flag, 0: all; 1: seg1only; 2: seg2only; 3 seg3only; 4: seg1+2
...
INTEGER :: NREC,NCOLINPUT,NQUADAINPUT,NQUADOINPUT,NUMMIEUSE,NUMMIERT, &
           MAXLORDINPUT,MAXMORDINPUT,NWV_BAND_START,NWV_BAND_END
...
IF(WV_SEG_FLAG==0)THEN
    NWV_BAND_START=1
    NWV_BAND_END=NWV_BAND-NWV_BAND_SEG3
...
ELSEIF(WV_SEG_FLAG==2)THEN
	...
	NWV_BAND_END=NWV_BAND-NWV_BAND_SEG3
ELSEIF(WV_SEG_FLAG==3)THEN
   !THIS OPTION IS BEST USED WITH MONOCHROMATIC_FLAG==1
	NWV_BAND_START=NWV_BAND-NWV_BAND_SEG3+1
...
IF(WV_SEG_FLAG==0 .OR. WV_SEG_FLAG==4)THEN
	NWV_START=1
	NWV_END=NWV
ELSEIF(WV_SEG_FLAG==2)THEN
...
IF(ILS_FLAG)THEN
  ...
  ELSEIF(WV_SEG_FLAG==1)THEN
		NWV_START=1
		NWV_END=NWV_BAND_SEG1
  ELSEIF(WV_SEG_FLAG==2)THEN
		NWV_START=NWV_BAND_SEG1+1
		NWV_END=NWV_BAND-NWV_BAND_SEG3
  ELSEIF(WV_SEG_FLAG==3)THEN
	   !THIS OPTION IS BEST USED WITH MONOCHROMATIC_FLAG==1
		NWV_START=NWV_BAND-NWV_BAND_SEG3+1
		NWV_END=NWV_BAND
  ELSEIF(WV_SEG_FLAG==4)THEN
		NWV_START=1
		NWV_END=NWV_BAND-NWV_BAND_SEG3
...
! OCEAN INTERFACE 
	...
	IF(SUNGLINT_INPUT)THEN
		RECDATASTREAM(NTLYERA+3,5)=0
    ELSE
        RECDATASTREAM(NTLYERA+3,5)=1
    ENDIF
