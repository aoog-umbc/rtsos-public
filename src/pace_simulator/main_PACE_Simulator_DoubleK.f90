...
INTEGER :: NREC,NCOLINPUT,NQUADAINPUT,NQUADOINPUT, MAXMORDINPUT_SAVE,&
           NUMMIEUSE,NUMMIERT,MAXLORDINPUT,MAXMORDINPUT,NWV_BAND_START,NWV_BAND_END,NWV_BAND_CAL
...
REAL*8 :: RH_simu,AeroFMF,Height_Particle,RTMP,RTMP1
!REAL,DIMENSION(5) :: RNDNMBR
INTEGER:: INTTMP,ICOM,IWV,IWV_CAL,IWV_SUB,IWVEX,IWVEXSTART,ILAYER,IPT, IAEROSOL, &
...
IF(ATMOS_ZERO)THEN
    ...
    GAS_ABS_FLAG=.false.
	WRITE(*,*)'ATMOS_ZERO IS TRUE,TAU_REF=',TAU_REF
...
CALL Particle_PHMX_Assign(NUMMIEUSE,NWV_BAND,IAEROSOL,AeroFMF,RH_simu,REFF1_Save,    &
...
CALL ARSL_VERP_INIT(IAEROSOL,NTLYERA,PNDLY,Height_Particle)
...
ENDIF
DEALLOCATE(DIRAD,DSTOKES,PNDLY,ARSLND1_Save,ARSLND2_Save,REFF1_Save,REFF2_Save,&
...
DO ITLYERA=1,NTLYERA
  TAULYR(ITLYERA)=PNDLY(ITLYERA)*TAU_REF*CSCAT_ARSL_BAND(IWV_BAND)/CEXT_REF + &
                    TAUR(ITLYERA)
ENDDO
TAU_SCAT_TOTAL=SUM(TAULYR)
!WRITE(*,*)'IWV_BAND,TAU_REF,CSCAT_ARSL_BAND(IWV_BAND),CEXT_REF=',&
!           IWV_BAND,TAU_REF,CSCAT_ARSL_BAND(IWV_BAND),CEXT_REF
...
ELSE
    ...
    IWV_BAND=LOCINDXTMP(1)
	ExtX_AEROSOL=POLINT_SIMPLE(NWV_BAND_SEG1,WV_BAND,CEXT_ARSL_BAND,WAVELENGTH_NANOMETER,3)
	ScatX_AEROSOL=POLINT_SIMPLE(NWV_BAND_SEG1,WV_BAND,CSCAT_ARSL_BAND,WAVELENGTH_NANOMETER,3)
ENDIF
...
IMIE=1
DO ITLYERA=1,NTLYERA
   TAULYR(ITLYERA)=PNDLY(ITLYERA)*TAU_REF*ExtX_AEROSOL/CEXT_REF
...
SUBROUTINE ARSL_VERP_INIT(IAEROSOL,NTLYERA,PNDLY,Height_Particle)
USE GLOBAL_DATA, ONLY : WV_PACE_REF,CEXT_REF,ALT_LYRA,WV_SEG_FLAG,MONOCHROMATIC_FLAG
...
REAL*8, INTENT(IN) :: Height_Particle
REAL*8  :: Height_Variance_Particle
...
REAL*8 :: ALTMID,TAU_TEMP
INTEGER :: ILYERA,IHEIGHT,IWV_BAND
...
DO ILYERA=1,NTLYERA
 ...
 IF(IAEROSOL==-98)THEN
    IF(ALTMID<Height_Particle .AND. ALTMID>0.5D0*Height_Particle)THEN
       PNDLY(ILYERA)=1.0D0
    ELSE
       PNDLY(ILYERA)=0.0D0
    ENDIF
...
CEXT_REF=CEXT_ARSL_BAND(IWV_BAND)
TAU_TEMP=SUM(PNDLY*CEXT_REF)
PNDLY=PNDLY*CEXT_REF/TAU_TEMP

write(*,*)'IWV_BAND,WV_BAND,CEXT_REF is:',IWV_BAND,WV_BAND(IWV_BAND),CEXT_REF
...
SUBROUTINE Particle_PHMX_Assign(NUMMIEUSE,NWV_PHMX,IAEROSOL,AeroFMF,RH_simu,REFF1_Save,    &
                 REFF2_Save,VEFF1_Save,VEFF2_Save,MRR1_Save,MRI1_Save,MRR2_Save,&
                 MRI2_Save,ARSLND1_Save,ARSLND2_Save,ARSLND_NonSpherical,       &
                 REFF_NonSpherical,VEFF_NonSpherical,MRR_NonSpherical,          &
				 MRI_NonSpherical,CFILE_AEROSOLS)
...
USE MIE_PHMX_PACE
USE GLOBAL_DATA,ONLY : WV_PACE_REF,OCEAN_CASE_SELECT,                                   &
                PHYTOPLANKTON_INDEX_REFRACTION,PHYTOPLANKTON_SPECTRAL_SLOPE,&
                SEDIMENT_INDEX_REFRACTION,SEDIMENT_SPECTRAL_SLOPE
IMPLICIT NONE
INTEGER, INTENT(IN)::NUMMIEUSE,NWV_PHMX,IAEROSOL
...
IF(IAEROSOL>=-1)THEN
	CALL PHMXMIE_PACE_INIT(FREEFORMFLAG,IAEROSOL,AeroFMF,RH_Simu,NWV_PHMX,REFF1_LOCAL,VEFF1_LOCAL,      &
