...
REAL*8,DIMENSION(:,:,:),ALLOCATABLE :: DIRADFULL_SAVE
REAL*8,DIMENSION(:,:,:,:,:),ALLOCATABLE :: DSTOKESFULL_SAVE
!REAL*8,DIMENSION(NDET,NWV_CAL,2) :: DIRADFULL_SAVE
!REAL*8,DIMENSION(NDET,NWV_CAL,NTHETAOUT,NPHIOUT,4) :: DSTOKESFULL_SAVE
...
REAL*8,DIMENSION(NUMMIEUSE) :: Height_Variance_Particle,Height_Particle
REAL*8,DIMENSION(0:NUMMIEUSE) :: CEXT_REF,TAU_REF
...
REAL*8,DIMENSION(NWV_AW_IOCCG_LUT) :: WV_AW_IOCCG_LUT,AW_IOCCG_LUT
...
REAL*8 :: NO2_COLUMN ! NO2 IN THE WHOLE COLUMN IN DOBSON UNIT
...
REAL*8,ALLOCATABLE,DIMENSION(:) :: wavelength_land_ref,fiso_input,fvol_input,fgeo_input,Bpol_input
...
REAL*8 :: LBDO_LD,FLAM,CHLa,pBRDFa_input,pBRDFk_input,pBRDFb_input,pBRDFe_input
...
REAL*8,DIMENSION(:),ALLOCATABLE:: TAU_ARSL_Ext_TMP,TAU_ARSL_Scat_TMP
...
REAL*8 :: RH_simu,AeroFMF,RTMP,RTMP1
...
!  WRITE(*,*)'MINVAL(TAU_TG_DKP),INDEX_TAUPRIME,NTLYERA=',MINVAL(TAU_TG_DKP),INDEX_TAUPRIME,NTLYERA
  ...
  DO ILS_AVG=1,N_AVG_SIZE
      YU(ILS_AVG)=POLINT_SIMPLE(NILS_AVG,XK,YK,XU(ILS_AVG),2)
...
REAL*8,DIMENSION(NWV_BAND_CAL) :: WAVELENGTH_Table_Save
REAL*8,DIMENSION(NWV_BAND_CAL,NTLYERA) ::TAU_ARSL_Ext_Save,TAU_ARSL_Scat_Save, &
...
REAL*8,INTENT(IN) :: TEMPERTURE,SALINITY,AeroFMF,RH_simu,CHLa

REAL*8,DIMENSION(NWV_BAND) :: TAU_TG_AVG,TAU_TG_H2O_AVG,TAU_TG_CO2_AVG, &
...
REAL*8,DIMENSION(NWV_BAND) :: fiso_avg,fgeo_avg,fvol_avg,Bpol_avg

REAL*8,DIMENSION(:,:,:),ALLOCATABLE :: DIRADFULL_AVG
! DIRADFULL(:,1) DOWNWELLING IRADIANCE
! DIRAD(:,2) UPWELLING IRADIANCE
REAL*8,DIMENSION(:,:,:,:,:),ALLOCATABLE :: DSTOKESFULL_AVG

!REAL*8,DIMENSION(NDET,NWV_BAND,2) :: DIRADFULL_AVG
...
INTEGER :: N_AVG_SIZE,MPL,NILS_AVG
...
IF(ILS_FLAG)THEN
  ...
  DO IWV_AVG=NWV_START,NWV_END
     IF(OCEAN_CASE_SELECT<0) THEN
      fiso_avg(IWV_AVG)=POLINT_SIMPLE(nwv_land_ref,wavelength_land_ref,fiso_input,WV_BAND(IWV_AVG),2)
      fgeo_avg(IWV_AVG)=POLINT_SIMPLE(nwv_land_ref,wavelength_land_ref,fgeo_input,WV_BAND(IWV_AVG),2)
      fvol_avg(IWV_AVG)=POLINT_SIMPLE(nwv_land_ref,wavelength_land_ref,fvol_input,WV_BAND(IWV_AVG),2)
	  Bpol_avg(IWV_AVG)=POLINT_SIMPLE(nwv_land_ref,wavelength_land_ref,Bpol_input,WV_BAND(IWV_AVG),2)
     ENDIF
     ...
     DO ILS_AVG=1,N_AVG_SIZE
        YU(ILS_AVG)=POLINT_SIMPLE(NILS_AVG,XK,YK,XU(ILS_AVG),MPL)
        IF(YU(ILS_AVG)<0.0D0)YU(ILS_AVG)=0.0D0
     ENDDO
     ILS_INT_CONST=TRAPZOID(N_AVG_SIZE,XU,YU)
     ...
     DO ILAYER=1,NTLYERA
        ...
        ELSE
			...
			TAU_ARSL_Ext_AVG(IWV_AVG,ILAYER)=POLINT_SIMPLE(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,WV_BAND(IWV_AVG),3)
			TempU=TAU_ARSL_Scat_Save(:,ILAYER)
			TAU_ARSL_Scat_AVG(IWV_AVG,ILAYER)=POLINT_SIMPLE(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,WV_BAND(IWV_AVG),3)
			TempU=TAU_RAY_Save(:,ILAYER)
			TAU_RAY_AVG(IWV_AVG,ILAYER)=POLINT_SIMPLE(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,WV_BAND(IWV_AVG),4)
			TempU=DEPOL_A_Save(:,ILAYER)
			DEPOL_A_AVG(IWV_AVG,ILAYER)=POLINT_SIMPLE(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,WV_BAND(IWV_AVG),3)
		ENDIF
...
!INTERPOLATE DSTOKESFULL_SAVE TO GET DSTOKESFULL_AVG
        ...
        ELSE
			...
			DO ICOM=1,2
				...
				DIRADFULL_AVG(IDET,IWV_AVG,ICOM)= POLINT_SIMPLE(NWV_BAND_CAL,&
			...
			DO IPHI=1,NPHIOUT
				...
				DSTOKESFULL_AVG(IDET,IWV_AVG,ITHETA,IPHI,ICOM)= &
					POLINT_SIMPLE(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,&
								  WV_BAND(IWV_AVG),2)
			ENDDO
			ENDDO
			ENDDO
...
REAL*8,INTENT(IN)::WAVELENGTH_MICRON,CHLa,pBRDFa_input,pBRDFk_input,&
                    pBRDFb_input,pBRDFe_input

REAL*8,DIMENSION(NTLYERA),INTENT(IN) :: TAUR,DEPOL_A,TAU_TG,PNDLY
REAL*8,DIMENSION(NTLYERA),INTENT(OUT) :: TAU_ARSL_Ext_TMP,TAU_ARSL_Scat_TMP
INTEGER,DIMENSION(NTLYERA),INTENT(IN) :: ARSL_TYPE
...
real*8 ::fiso_local,fvol_local,fgeo_local,Bpol_local

REAL*8,DIMENSION(:),ALLOCATABLE :: TAURfrac,TAULYR,LBDOLYR,RARRAYTMP
...
REAL*8 :: RTMP,RTMP1,RTMP2,OCEAN_ALBEDO, &
...
REAL*8,DIMENSION(0:NUMMIEUSE) ::ExtX_AEROSOL,ScatX_AEROSOL,AEROSOL_ALBEDO,AEROSOL_ABSORB_FAC
...
ELSE
    ...
    DO IMIE=1,NUMMIEUSE
		...
		ExtX_AEROSOL(IMIE)=POLINT_SIMPLE(NWV_BAND_SEG1,WV_BAND,RARRAYTMP,WAVELENGTH_NANOMETER,3)
        RARRAYTMP=CSCAT_ARSL_BAND(IMIE,:)
        ScatX_AEROSOL(IMIE)=POLINT_SIMPLE(NWV_BAND_SEG1,WV_BAND,RARRAYTMP,WAVELENGTH_NANOMETER,3)
    ENDDO
ENDIF
...
! Ross_Li + Polarized reflectance LAND BOTTOM
	fiso_local=POLINT_SIMPLE(nwv_land_ref,wavelength_land_ref,fiso_input,WAVELENGTH_NANOMETER,2)
	fvol_local=POLINT_SIMPLE(nwv_land_ref,wavelength_land_ref,fvol_input,WAVELENGTH_NANOMETER,2)
	fgeo_local=POLINT_SIMPLE(nwv_land_ref,wavelength_land_ref,fgeo_input,WAVELENGTH_NANOMETER,2)
	Bpol_local=POLINT_SIMPLE(nwv_land_ref,wavelength_land_ref,Bpol_input,WAVELENGTH_NANOMETER,2)
	NMBREINPUT=1.5d0
	NMBIMINPUT=0.0d0
...
REAL*8 :: ALTMID,TAU_TEMP,HEIGHT_TEMP,HEIGHT_TEMP1,HEIGHT_TEMP2
...
! TESTING RESCALE ALGORITHM AND SEE IF TAU_REF IS CORRECTLY CALCULATED
    ...
    write(*,*)'IMIE,WV_BAND,TAU_REF is:',IMIE,WV_BAND(IWV_BAND),TAU_TEMP
ENDDO
...
REAL*8 :: MU_LOCAL,RTMP

! DELTA FIT DIFINITION
real*8,  DIMENSION(:),ALLOCATABLE :: ANG,P11,P12,P22,P33,P34,P44
real*8 :: FTRUNC_LOCAL
...
real*8,DIMENSION(NUMANG),intent(inout) :: ANG,P11,P22,P33,P44,P12,P34
real*8,intent(out) :: FTRUNC_LOCAL
real*8,DIMENSION(0:MAXLORDINPUT),intent(out) :: BETAL_LOCAL,ALPHAL_LOCAL,ZETAL_LOCAL,DELTAL_LOCAL,&
								   GAMMAL_LOCAL,EPSILONL_LOCAL

INTEGER :: IWV_BAND,IMIEUSE,IANG
INTEGER :: LORD
REAL*8 :: MU_LOCAL,RTMP,ANGCUT

real*8,DIMENSION(:),ALLOCATABLE :: DL00,DL20,DL2P2,DL2N2
! DELTA FIT DIFINITION

real*8,DIMENSION(:),ALLOCATABLE :: TMPL,P11_TRUNC
...
REAL*8,intent(in) :: TAURfracLOCAL
REAL*8 :: RTMP
...
REAL*8,DIMENSION(NUMMIERT,NUMMIEANGMAX,0:6),intent(in) ::PHMXDATASTREAM

! DELTA FIT DIFINITION
integer :: NUMANG,numordfull_local,IANG,LORD
real*8,  DIMENSION(:),ALLOCATABLE :: muang,ANG,P11,P12,P22,P33,P34,P44
real*8 :: FTRUNC_LOCAL
real*8,DIMENSION(:),ALLOCATABLE :: BETAL_LOCAL,ALPHAL_LOCAL,ZETAL_LOCAL,DELTAL_LOCAL,&
								   GAMMAL_LOCAL,EPSILONL_LOCAL
...
REAL*8 :: BPTC_exp_coeff,BPTC660,BPTC550,Ap_Bricaud,Ep_Bricaud
...
REAL*8 :: ACDM400,ACDM370,SF_COEFF,AP_PICO_SPECIFIC676NM=0.023D0,AP_MICRO_SPECIFIC676NM=0.0086D0
REAL*8 :: POLINT_SIMPLE
  ...
  ! I_AW_UV==1 USE ZP LEE 2015 doi.org/10.1364/AO.54.000546
	...
	IF(I_AW_UV ==1) THEN
	  IF(WAVELENGTH_NANOMETER<350.0D0)THEN
		 AWLOCAL=  POLINT_SIMPLE(NWV_AW_IOCCG_LUT,WV_AW_IOCCG_LUT,AW_IOCCG_LUT,WAVELENGTH_NANOMETER,2)
	  ELSE
...
REAL*8::AeroFMF,RH_Simu,REFF1_LOCAL,VEFF1_LOCAL,REFF2_LOCAL,VEFF2_LOCAL,ARSLND1_LOCAL,ARSLND2_LOCAL
