...
!testing
!open(unit=1,file='stokesdata',Status='old',Access = 'append')
!write(1,'("lambda,DSTOKES,Irad,reflectance",4(2x,E12.6))')WAVELENGTH_MICRON,&
!       DSTOKES(NDET,1,1,1),DIRAD(NDET,1),DSTOKES(NDET,1,1,1)/DIRAD(NDET,1)
!close(1)
...
IF(OCEAN_CASE_SELECT .NE. -1)THEN
	dimscl=(/ NWV_OUTPUT /)
	ALLOCATE(HDF5RARR(NWV_OUTPUT))
	IF(ILS_FLAG .OR. MONOCHROMATIC_FLAG)THEN
...
IF((WV_SEG_FLAG==4 .AND. MONOCHROMATIC_FLAG) .or. WAVELENGTH_NANOMETER>890D0)THEN
...
REAL*8,INTENT(IN)::WAVELENGTH_MICRON,CHLa
...
LOOP_LAYERS: DO ITLYERW=1,NTLYERO+1
  ...
  IF(OCEAN_CASE_SELECT .NE. -1) LBDO_LD=0.33d0*RTMP2/(RTMP2+RTMP1)
...
ENDDO
IF((WV_SEG_FLAG==4 .AND. MONOCHROMATIC_FLAG))THEN
