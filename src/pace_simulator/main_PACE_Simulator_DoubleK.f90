...
REAL*8,DIMENSION(0:NUMMIEUSE) :: CEXT_REF,TAU_REF,Height_Particle
  ...
  IF(OCEAN_FCDOM_FLAG .OR. OCEAN_FCHLA_FLAG .OR. OCEAN_RAMAN_FLAG)THEN
    ...
    DO IWVEX=1,NFLRSCGRID
       ...
       CALL RTSOSINIT(Indx_Inelastic_Scattering,IWVEX,WAVELENGTH_MICRON,&
          NREC,NTLYERA,NTLYERO,PNDLY,ARSL_TYPE,TAUR,DEPOL_A,TAU_TG,RECDATASTREAM,NTLYER, &
...
! testing absorption block
  ...
  DO IWV_CAL=1,NWV_SUB_CAL
        ...
        CALL RTSOSINIT(Indx_Inelastic_Scattering,INDX_WV_CAL(IWV_CAL),WAVELENGTH_MICRON,&
           NREC,NTLYERA,NTLYERO,PNDLY,ARSL_TYPE,TAUR,DEPOL_A,TAU_TG,RECDATASTREAM,      &
...
!            DIRADTMP(1:NDET,IWV_CAL,1:2)=DIRADTMP(1:NDET,IWV_CAL,1:2)-DIRAD(1:NDET,1:2)*RTMP1
  ...
  IWV_CAL=1
  IF(NWV_SUB_CAL>1 .AND. IWV_BAND>1 )THEN
      IF(ABS(WAVELENGTH_Table_Save(IWV_BAND-NWV_BAND_START)-WV(INDX_WV_CAL(IWV_CAL)))<1.0E-4)THEN
		  DO IWV_CAL=1,NWV_SUB_CAL
			 IF(ABS(WAVELENGTH_Table_Save(IWV_BAND-NWV_BAND_START)-WV(INDX_WV_CAL(IWV_CAL)))>1.0E-4)EXIT
		  ENDDO
      ENDIF
      IF(IWV_CAL>NWV_SUB_CAL)IWV_CAL=NWV_SUB_CAL
  ENDIF
  WAVELENGTH_Table_Save(IWV_BAND-NWV_BAND_START+1)=WV(INDX_WV_CAL(IWV_CAL))
  TAU_ARSL_Ext_Save(IWV_BAND-NWV_BAND_START+1,:)=TAU_ARSL_Ext_TMP(IWV_CAL,:)
  TAU_ARSL_Scat_Save(IWV_BAND-NWV_BAND_START+1,:)=TAU_ARSL_Scat_TMP(IWV_CAL,:)
  TAU_RAY_Save(IWV_BAND-NWV_BAND_START+1,:)=TAU_RAY_TMP(IWV_CAL,:)
  DEPOL_A_Save(IWV_BAND-NWV_BAND_START+1,:)=DEPOL_A_TMP(IWV_CAL,:)
  IF(MONOCHROMATIC_FLAG)THEN
...
SUBROUTINE RTSOSINIT(Indx_Inelastic_Scattering,IWVSAVE,WAVELENGTH_MICRON,NREC,  &
       NTLYERA,NTLYERO,PNDLY,ARSL_TYPE,TAUR,DEPOL_A,TAU_TG,RECDATASTREAM,NTLYER,&
...
IMIE=1
DO ITLYERA=1,NTLYERA
   TAULYR(ITLYERA)=PNDLY(ITLYERA)*ExtX_AEROSOL(ARSL_TYPE(ITLYERA))
...
! total optical depth=aerosol+rayleigh+gas absorption
   ...
   SCATTERING_TAU=TAUR(ITLYERA)+TAULYR(ITLYERA)*AEROSOL_ALBEDO(ARSL_TYPE(ITLYERA))
...
USE GLOBAL_DATA, ONLY : NUMMIEUSE,WV_PACE_REF,TAU_REF,CEXT_REF,Height_Particle, &
                        ALT_LYRA,WV_SEG_FLAG,MONOCHROMATIC_FLAG
...
REAL*8,DIMENSION(NUMMIEUSE) :: Height_Variance_Particle
...
Height_Variance_Particle=2.0d0
PNDLY=0.0D0
