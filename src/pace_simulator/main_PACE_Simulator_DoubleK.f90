...
DOUBLE PRECISION,DIMENSION(:,:,:,:,:),ALLOCATABLE :: DSTOKESFULL
! DSTOKES(NWV,       NTHETAOUT,       NPHIOUT,         4)
! ITH WAVELENGTH , theta out index, phi out index, stokes parameters I Q U V

DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE :: DIRADFULL_SAVE
DOUBLE PRECISION,DIMENSION(:,:,:,:,:),ALLOCATABLE :: DSTOKESFULL_SAVE
!DOUBLE PRECISION,DIMENSION(NDET,NWV_CAL,2) :: DIRADFULL_SAVE
!DOUBLE PRECISION,DIMENSION(NDET,NWV_CAL,NTHETAOUT,NPHIOUT,4) :: DSTOKESFULL_SAVE
...
INTEGER :: NDET
DOUBLE PRECISION :: WV_PACE_REF
INTEGER,PARAMETER :: NUMMIEUSE=2
DOUBLE PRECISION,DIMENSION(NUMMIEUSE) :: Height_Variance_Particle,Height_Particle
DOUBLE PRECISION,DIMENSION(0:NUMMIEUSE) :: CEXT_REF,TAU_REF

INTEGER,PARAMETER :: NFINEMODE=10
DOUBLE PRECISION,DIMENSION(NWV):: AW,BBW,BWRAMAN,BW
DOUBLE PRECISION,DIMENSION(NWV) ::BPTC,BBPTC,BBPTCfrac,APTC,ACDM
DOUBLE PRECISION,DIMENSION(NWV) ::BSTC,BBSTC,BBSTCfrac,ASTC

INTEGER,PARAMETER ::NWV_PF_LUT=2250
DOUBLE PRECISION,DIMENSION(NWV_PF_LUT) :: WV_PF_LUT,AW_PF_LUT,BW_PF_LUT

INTEGER,PARAMETER ::NWV_AW_LEE_LUT=144
DOUBLE PRECISION,DIMENSION(NWV_AW_LEE_LUT) :: WV_AW_LEE_LUT,AW_LEE_LUT

INTEGER,PARAMETER ::NWV_AW_MCF_LUT=131
DOUBLE PRECISION,DIMENSION(NWV_AW_MCF_LUT) :: WV_AW_MCF_LUT,AW_MCF_LUT

INTEGER,PARAMETER ::NWV_AW_IOCCG_LUT=211
DOUBLE PRECISION,DIMENSION(NWV_AW_IOCCG_LUT) :: WV_AW_IOCCG_LUT,AW_IOCCG_LUT

INTEGER, PARAMETER :: N_ALT_INIT=104
DOUBLE PRECISION,DIMENSION(N_ALT_INIT) :: ALT_LYRA_INIT= &
	(/ 100.0D0, 95.00D0, 90.0D0,85.00D0,  80.0D0,75.0D0, &
        70.00D0, 65.0D0, 60.00D0,55.0D0,  50.0D0, 47.5D0,  45.0D0,42.5D0,&
        40.0D0, 37.50D0, 35.0D0, 32.50D0, 30.0D0, 27.50D0, 25.0D0,22.50D0,     &
		20.0D0, 19.75D0, 19.5D0, 19.25D0, 19.0D0, 18.75d0, 18.5d0,18.25D0,     &
		18.0D0, 17.75D0, 17.5D0, 17.25D0, 17.0D0, 16.75d0, 16.5d0,16.25D0,     &
		16.0D0, 15.75D0, 15.5D0, 15.25D0, 15.0D0, 14.75d0, 14.5d0,14.25D0,     &
		14.0D0, 13.75D0, 13.5D0, 13.25D0, 13.0D0, 12.75d0, 12.5d0,12.25D0,     &
		12.0D0, 11.75D0, 11.5D0, 11.25D0, 11.0D0, 10.75d0, 10.5d0,10.25D0,     &
		10.0D0,  9.75D0,  9.5D0,  9.25D0,  9.0D0,  8.75d0,  8.5d0, 8.25D0,     &
		 8.0D0,  7.75D0,  7.5D0,  7.25D0,  7.0D0,  6.75d0,  6.5d0, 6.25D0,     &
		 6.0D0,  5.75D0,  5.5D0,  5.25D0,  5.0D0,  4.75d0,  4.5d0, 4.25D0,     &
		 4.0D0,  3.75D0,  3.5D0,  3.25D0,  3.0D0,  2.75d0,  2.5d0, 2.25D0,     &
		 2.0D0,  1.75D0,  1.5D0,  1.25D0,  1.0D0,  0.75D0,  0.5D0, 0.25D0, 0.0D0, 0.0D0/)
DOUBLE PRECISION, ALLOCATABLE,DIMENSION(:) :: ALT_LYRA

DOUBLE PRECISION :: OZONE_COLUMN ! OZONE IN THE WHOLE COLUMN IN DOBSON UNIT
DOUBLE PRECISION :: H2O_COLUMN   ! WATER VAPOR IN THE WHOLE COLUMN IN CENTIMETERS.
DOUBLE PRECISION :: NO2_COLUMN ! NO2 IN THE WHOLE COLUMN IN DOBSON UNIT
DOUBLE PRECISION :: PRESSURE_SURFACE ! SURFACE PRESSURE IN mb.

DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: GAS_ABS_COEFF,GAS_ABS_COEFF_H2O,GAS_ABS_COEFF_CO2, &
		   GAS_ABS_COEFF_CH4,GAS_ABS_COEFF_O2, GAS_ABS_COEFF_O3,GAS_ABS_COEFF_NO2

LOGICAL :: CHLA_HOMOGENEITY
DOUBLE PRECISION :: CHLA_C0,CHLA_Slope,CHLA_Cmax, CHLA_Etamax,CHLA_DeltaEta
DOUBLE PRECISION :: PHYTOPLANKTON_INDEX_REFRACTION,PHYTOPLANKTON_SPECTRAL_SLOPE, &
          SEDIMENT_INDEX_REFRACTION,SEDIMENT_SPECTRAL_SLOPE,        &
          SEDIMENT_CONCENTRATION

DOUBLE PRECISION :: adg440_Case3,bbp660_Case3,Bp660_backscatteringfraction_Case3, &
          Sdg_Case3,Sbp_Case3,S_Bp_Case3

DOUBLE PRECISION,DIMENSION(NWV) :: RINDX_WATER
DOUBLE PRECISION ::WNDSPD,THETA0
!DOUBLE PRECISION,PARAMETER :: DEPOL_A=0.0284D0
DOUBLE PRECISION,PARAMETER :: DEPOL_O=0.087D0
DOUBLE PRECISION,PARAMETER :: Xi_kok=25.6d0,Alpha_kok=4.0d0, T0_kok=0.25d0,Dpol90_Kok=0.67d0
...
DOUBLE PRECISION,DIMENSION(:), ALLOCATABLE ::TAU_REDUCE_ATMOS_PRESERVE,TAU_REDUCE_OCEAN_PRESERVE
DOUBLE PRECISION :: ABSORPTION_TAU_REPLACE

!LOGICAL :: WATER_DET_FREE_LOCATION
!INTEGER,PARAMETER :: N_DET_INWATER_REPORT=21
!DOUBLE PRECISION :: DET_INWATER_REPORT_DEPTH_INCREMENT
!DOUBLE PRECISION,DIMENSION(N_DET_INWATER_REPORT):: DET_INWATER_REPORT_DEPTH ! LARGEST WATER DETECT DEPTH REPORTED

INTEGER :: OCEAN_CASE_SELECT
                              !==-203 Ross Li Surface Reflectance
...
DOUBLE PRECISION :: AirSensor_Height
...
DOUBLE PRECISION,DIMENSION(NDK_CAL) :: IMS,DK,GK,EXPOCOEFF
DOUBLE PRECISION :: EXPOFAC,ALPHAFAC,BETAFAC,GAMMAFAC

CONTAINS

SUBROUTINE DOUBLEKPARA_EVALUATION(NWV_SUB_CAL,TAU_TG_MAX,ErrFLAG)
INTEGER, INTENT(IN) :: NWV_SUB_CAL
DOUBLE PRECISION, INTENT(IN) :: TAU_TG_MAX
LOGICAL,INTENT(OUT) :: ErrFLAG

DOUBLE PRECISION :: ALPHALW,ALPHAHI,XACC
DOUBLE PRECISION, DIMENSION(NDK_CAL) :: IMSABS
INTEGER :: IDK_CAL

DOUBLE PRECISION,EXTERNAL :: FUNCALPHA

DOUBLE PRECISION :: RTMP, RTMP1,DRTMP
ErrFLAG=.FALSE.
IMSABS=ABS(IMS)
...
CALL SUB_ALPHA(ALPHALW,RTMP,DRTMP)
CALL SUB_ALPHA(ALPHAHI,RTMP1,DRTMP)

IF(RTMP*RTMP1<0.0D0)THEN
  DOUBLEK_FLAG=.TRUE.
  CALL bisection(FUNCALPHA,ALPHALW, ALPHAHI,ALPHAFAC,XACC)
...
DOUBLE PRECISION FUNCTION DOUBLEKFUNC_EVALUATION(NWV_SUB_CAL,DK_VAL,DKP_VAL)
IMPLICIT NONE
INTEGER :: NWV_SUB_CAL
DOUBLE PRECISION,INTENT(IN) :: DK_VAL,DKP_VAL
...
DOUBLE PRECISION FUNCTION FUNCALPHA(ALPHA_VAL)
USE DOUBLEKPARA
IMPLICIT NONE
DOUBLE PRECISION, INTENT(IN) :: ALPHA_VAL
!DOUBLE PRECISION, INTENT(OUT) :: FUNCALPHA_VAL,DFUNCALPHA_VAL

!write(*,*)'Test FUNCALPHA Line 01,', DK(1),DK(2),DK(3),ALPHA_VAL,EXPOFAC

FUNCALPHA=(DK(2)+ALPHA_VAL)**(EXPOFAC+1)-(DK(1)+ALPHA_VAL)*(DK(3)+ALPHA_VAL)**EXPOFAC
...
END FUNCTION FUNCALPHA


SUBROUTINE SUB_ALPHA(ALPHA_VAL,FUNCALPHA_VAL,DFUNCALPHA_VAL)
USE DOUBLEKPARA
IMPLICIT NONE
DOUBLE PRECISION, INTENT(IN) :: ALPHA_VAL
DOUBLE PRECISION, INTENT(OUT) :: FUNCALPHA_VAL,DFUNCALPHA_VAL
...
USE RTUTILITY, ONLY : PI,FACTOR,NUMMIEANGMAX,PSEUDO_SPHERICAL_SHELL,&
                      SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION
...
DOUBLE PRECISION, ALLOCATABLE,DIMENSION(:,:) :: TAU_ARSL_Ext_Save,TAU_ARSL_Scat_Save,TAU_RAY_Save,DEPOL_A_Save,&
						TAU_TG_Save, TAU_TG_H2O_Save,TAU_TG_CO2_Save,TAU_TG_CH4_Save,TAU_TG_O2_Save,&
						TAU_TG_O3_Save,TAU_TG_NO2_Save

DOUBLE PRECISION, ALLOCATABLE,DIMENSION(:) :: WAVELENGTH_Table_Save
DOUBLE PRECISION, ALLOCATABLE,DIMENSION(:) :: TAUR,DEPOL_A,TAU_TG,TAU_TG_H2O,TAU_TG_CO2,TAU_TG_CH4,TAU_TG_O2,TAU_TG_O3,&
									TAU_TG_NO2, PNDLY,ARSLND1_Save,ARSLND2_Save,&
                                    REFF1_Save,REFF2_Save,VEFF1_Save,VEFF2_Save
INTEGER, ALLOCATABLE,DIMENSION(:) :: ARSL_TYPE
DOUBLE PRECISION, ALLOCATABLE,DIMENSION(:,:) ::MRR1_Save,MRI1_Save,MRR2_Save,MRI2_Save

DOUBLE PRECISION :: ARSLND_NonSpherical,REFF_NonSpherical,VEFF_NonSpherical
DOUBLE PRECISION, ALLOCATABLE,DIMENSION(:) :: MRR_NonSpherical,MRI_NonSpherical
...
INTEGER :: NREC,NCOLINPUT,NQUADAINPUT,NQUADOINPUT, MAXMORDINPUT_SAVE,&
           NUMMIERT,MAXLORDINPUT,MAXMORDINPUT,NWV_BAND_START,NWV_BAND_END,NWV_BAND_CAL
DOUBLE PRECISION :: DELTATAUAINPUT,DELTATAUOINPUT
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: RECDATASTREAM
DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE :: PHMXDATASTREAM !,COEFFDATASTREAM
INTEGER,DIMENSION(:),ALLOCATABLE :: NUMMIEANGINPUT

DOUBLE PRECISION :: LBDO_LD,FLAM,CHLa,pBRDFa_input,pBRDFk_input,pBRDFb_input,pBRDFe_input

INTEGER :: NTHETAOUT,NPHIOUT,ITHETA,IPHI,IMIE,IDET,IDK_CAL,ILS_AVG,NILS_AVG
DOUBLE PRECISION :: MU_IN,WAVELENGTH_MICRON,WAVELENGTH_NANOMETER,TEMPERTURE,SALINITY,&
          AWLOCAL,BWLOCAL,BBWLOCAL,BWRAMANLOCAL,BPTCLOCAL,BBPTCLOCAL,       &
          BBPTCfracLOCAL,APTCLOCAL,BSTCLOCAL,ASTCLOCAL,ACDMLOCAL,&
          RINDX_WATERLOCAL

DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: PHIOUT,MUOUT

LOGICAL :: WLR_FLAG_INPUT,WCFLG_INPUT,GAS_ABS_FLAG,CLOUDFLAG
INTEGER :: DELTAM_INPUT
DOUBLE PRECISION,DIMENSION(:,:) ,ALLOCATABLE:: DIRAD  ! DIRAD(:,1) DOWNWELLING IRADIANCE
                                            ! DIRAD(:,2) UPWELLING IRADIANCE
                                            ! First dimension is detector label
DOUBLE PRECISION,DIMENSION(:,:,:,:),ALLOCATABLE :: DSTOKES
! DSTOKES(NWV,       NTHETAOUT,       NPHIOUT,         4)
! ITH WAVELENGTH , theta out index, phi out index, stokes parameters I Q U V

INTEGER :: INDEX_PRIME_SCATTERING
DOUBLE PRECISION :: FSUN_BAND_AVG
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: TAU_TG_DK,TAU_TG_DKP
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE:: TAU_ARSL_Ext_TMP,TAU_ARSL_Scat_TMP
DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE::DIRADTMP
DOUBLE PRECISION,DIMENSION(:,:,:,:,:),ALLOCATABLE::DSTOKESTMP

LOGICAL :: WVNUMRESET,ErrFLAG
DOUBLE PRECISION,DIMENSION(NDK_CAL) :: TAU_TG_DK_CAL,TAU_TG_INCR
DOUBLE PRECISION,DIMENSION(NDK_CAL) :: TAU_TG_DKP_STAT

DOUBLE PRECISION :: TAU_TG_MAX,TAU_TG_MIN,DK_VAL,DKP_VAL,TAU_ABS_AVG
INTEGER :: NWV_SUB_CAL,NTAU_TG_DK,IWV_BAND,IWV_BAND_TEMP,IWV_SUB_START,IWV_SUB_END
INTEGER,DIMENSION(:),ALLOCATABLE :: INDX_WV_CAL

INTEGER :: N_AVG_SIZE
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XK,YK
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XU,YU,ZU,FSUN_BAND

DOUBLE PRECISION :: Func_UVIP3P,ILS_INT_CONST,TRAPZOID

INTEGER,DIMENSION(1) ::LOCINDXTMP,LOCINDXTMP1
LOGICAL,DIMENSION(:),ALLOCATABLE :: MASKMINLOC
DOUBLE PRECISION :: RH_simu,AeroFMF,RTMP,RTMP1
!REAL,DIMENSION(5) :: RNDNMBR
INTEGER:: INTTMP,ICOM,IWV,IWV_CAL,IWV_SUB,IWVEX,IWVEXSTART,ILAYER,IPT, &
          IULO,MPL,KLO,INDEX_TAUPRIME,IREC_SHFT

INTEGER:: IAEROSOL
!IAEROSOL=-99,-98,-97, -1,1,20.
!-99 read aerosol pmhx in from file;
!-98 read water cloud phmx from file;
!-97 two layer aerosols, with aerosol phase matrix read from two input files
...
READ(1,*)WV_PACE_REF
READ(1,*)TAU_REF(1)
READ(1,*)Height_Particle(1)
READ(1,*)Height_Variance_Particle(1)
READ(1,*)TAU_REF(2)
READ(1,*)Height_Particle(2)
READ(1,*)Height_Variance_Particle(2)
READ(1,*)IAEROSOL
...
READ(1,*)NO2_COLUMN   ! NO2 IN THE WHOLE COLUMN IN DOBSON UNIT
...
DO INTTMP=1,NUMMIEUSE
   IF(index(CFILE_AEROSOLS(INTTMP),'#')>1) &
        CFILE_AEROSOLS(INTTMP)=trim(CFILE_AEROSOLS(INTTMP)(1:index(CFILE_AEROSOLS(INTTMP),'#')-1))
ENDDO
IF(index(OUTFILE,'#')>1) &
...
IF(Height_Particle(1)<Height_Particle(2)) &
   stop 'check, Height_Particle_toplayer<Height_Particle_lowlayer'
...
IF(OCEAN_CASE_SELECT>=0)THEN
	...
	WRITE(*,'(1x,"CHLA_HOMOGENEITY=",L2)')CHLA_HOMOGENEITY
	WRITE(*,'(1x,"OCEAN_RAMAN_FLAG,OCEAN_FCHLA_FLAG,OCEAN_FCDOM_FLAG=",3(L2,2x))') &
			 OCEAN_RAMAN_FLAG,OCEAN_FCHLA_FLAG,OCEAN_FCDOM_FLAG
	WRITE(*,'(1x,"HYSPECTRAL_FLAG,NPQ_FLAG,OCEAN_PHMX_ONE,GAS_ABS_FLAG=",4(L2,2x))')&
			   HYSPECTRAL_FLAG,NPQ_FLAG,OCEAN_PHMX_ONE,GAS_ABS_FLAG

	WRITE(*,'(1x,"adg440,bbp660,Bp660=",3(E12.4,2x))') &
			   adg440_Case3,bbp660_Case3,Bp660_backscatteringfraction_Case3
    WRITE(*,'(1x,"Sdg,Sbp,S_Bp=",3(E12.4,2x))') &
               Sdg_Case3,Sbp_Case3,S_Bp_Case3
ENDIF

WRITE(*,'(1x,"SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION IS:",L2)') &
		   SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION
...
IF(.NOT. GAS_ABS_FLAG) MONOCHROMATIC_FLAG=.TRUE.
...
CALL qsort2a(NTLYERA,ALT_LYRA_INIT,ALT_LYRA)
...
write(*,*)'ABS_COEFF_INIT elapsed time =', finish_time-start_time
...
IF(OCEAN_CASE_SELECT<0)THEN
  ...
  OCEAN_FCDOM_FLAG=.FALSE.
  HYSPECTRAL_FLAG=.false.
ELSEIF(OCEAN_CASE_SELECT==0)THEN
  NTLYERO=1
  OCEAN_PHMX_ONE=.TRUE.
  OCEAN_RAMAN_FLAG=.false.
  OCEAN_FCHLA_FLAG=.false.
...
ALLOCATE(TAU_ARSL_Ext_TMP(NTLYERA),TAU_ARSL_Scat_TMP(NTLYERA))
...
ALLOCATE(RECDATASTREAM(NREC,7),PNDLY(NTLYERA),ARSL_TYPE(NTLYERA),               &
...
CALL WATER_ABSORPTION_IOCCG_READ

call date_and_time(values=time_array_0)
start_time = time_array_0 (5) * 3600 + time_array_0 (6) * 60 &
				 + time_array_0 (7) + 0.001 * time_array_0 (8)

CALL Particle_PHMX_Assign(NWV_BAND,IAEROSOL,AeroFMF,RH_simu,REFF1_Save,    &
...
CALL ARSL_COEFF_PACE_TRUNC(NUMMIEUSE,MAXLORDINPUT,DELTAM_INPUT)

call date_and_time(values=time_array_1)
finish_time = time_array_1 (5) * 3600 + time_array_1 (6) * 60 &
			   + time_array_1 (7) + 0.001 * time_array_1 (8)

write(*,*)'Particle_PHMX_Assign elapsed time =', finish_time-start_time


CALL ARSL_VERP_INIT(IAEROSOL,NTLYERA,PNDLY,ARSL_TYPE)
  ...
  IF(OCEAN_FCDOM_FLAG .OR. OCEAN_FCHLA_FLAG .OR. OCEAN_RAMAN_FLAG)THEN
    ...
    DO IWVEX=1,NFLRSCGRID
       ...
       CALL RTSOSINIT(Indx_Inelastic_Scattering,IWVEX,WAVELENGTH_MICRON,&
          NREC,NTLYERA,NTLYERO,PNDLY,ARSL_TYPE,TAUR,DEPOL_A,TAU_TG,TAU_ARSL_Ext_TMP,&
          TAU_ARSL_Scat_TMP,RECDATASTREAM,NTLYER, &
          NUMMIERT,MAXLORD_Inelas,NUMMIEANGINPUT,PHMXDATASTREAM,DELTAM_INPUT,LBDO_LD,FLAM,   &
          CHLa,TEMPERTURE,SALINITY,pBRDFa_input,pBRDFk_input,pBRDFb_input,pBRDFe_input)
...
!       write(*,*)'rtsosinit elapsed time =', finish_time-start_time
       ...
       call date_and_time(values=time_array_0)
             start_time = time_array_0 (5) * 3600 + time_array_0 (6) * 60 &
                        + time_array_0 (7) + 0.001 * time_array_0 (8)
       write(*,*)'rtsos elapsed time =', start_time-finish_time,NFLRSCGRID,IWVEX
...
ENDIF
NWV_BAND_CAL=NWV_BAND_END-NWV_BAND_START+1

IF(MONOCHROMATIC_FLAG) THEN
	ALLOCATE(DIRADFULL_SAVE(NDET,NWV_BAND_CAL,2))
	ALLOCATE(DSTOKESFULL_SAVE(NDET,NWV_BAND_CAL,NTHETAOUT,NPHIOUT,4))
	DIRADFULL_SAVE=0.0D0
	DSTOKESFULL_SAVE=0.0D0
ELSE
...
ALLOCATE(WAVELENGTH_Table_Save(NWV_BAND_CAL), &
         TAU_ARSL_Ext_Save(NWV_BAND_CAL,NTLYERA), &
...
ALLOCATE(XK(MAXVAL(NILS_PACE)),YK(MAXVAL(NILS_PACE)))
...
DO IWV_BAND=NWV_BAND_START,NWV_BAND_END
  ...
  IF(WV_SEG_FLAG==4)THEN
    LOCINDXTMP= MINLOC(ABS(WV_BAND(NWV_BAND_SEG1P2+NWV_BAND_SEG3+1:NWV_BAND)-WAVELENGTH_NANOMETER))
    LOCINDXTMP= LOCINDXTMP+NWV_BAND_SEG1P2+NWV_BAND_SEG3
  ELSE
    LOCINDXTMP= MINLOC(ABS(WV_BAND(1:NWV_BAND_SEG1P2)-WAVELENGTH_NANOMETER))
  ENDIF

  XK=ILS_DeltaWaveLength(LOCINDXTMP(1),:)
  YK=ILS_PACE(LOCINDXTMP(1),:)

  IF(LOCINDXTMP(1)<=NWV_BAND_SEG1P2)THEN
		NILS_AVG=NILS_PACE(LOCINDXTMP(1))
  ELSE IF(LOCINDXTMP(1)<=NWV_BAND_SEG1P2+NWV_BAND_SEG3)THEN
		NILS_AVG=NILS_SPEXone
  ELSE
		NILS_AVG=NILS_HARP
  ENDIF
...
!		   YK(1),YK(NILS_AVG/2),YK(NILS_AVG)


  FLAG1=WAVELENGTH_NANOMETER>=WV_BAND_HYPO(NWV_BAND_SEG1_HYPO) .OR. &
        WAVELENGTH_NANOMETER<=WV_BAND_HYPO(1) .OR. WV_SEG_FLAG==4
  IF(FLAG1) THEN
     RTMP=1.0D0
  ELSE
     RTMP=0.5D0
  ...
  ELSE
       IWV_SUB_END  =LOCINDXTMP1(1)-1
  ENDIF
  WRITE(*,'("WV_BAND_HYPO,WV_SUB_START,WV_SUB_END=",3(f9.2,1x))') &
             WV_BAND_HYPO(IWV_BAND),WV(IWV_SUB_START),WV(IWV_SUB_END)

  ALLOCATE(TAU_TG_DK(IWV_SUB_END-IWV_SUB_START+1),TAU_TG_DKP(IWV_SUB_END-IWV_SUB_START+1))

  CALL TAURCAL(WAVELENGTH_MICRON,NTLYERA,TAUR,DEPOL_A)
  INDEX_TAUPRIME=INDEX_PRIME_SCATTERING(NTLYERA,TAUR,PNDLY,ARSL_TYPE,WAVELENGTH_NANOMETER)
...
!  WRITE(*,*)'MINVAL(TAU_TG_DKP),INDEX_TAUPRIME,NTLYERA=',MINVAL(TAU_TG_DKP),INDEX_TAUPRIME,NTLYERA
  ...
  DO ILS_AVG=1,N_AVG_SIZE
      IF(XU(ILS_AVG)<XK(1) .OR. XU(ILS_AVG)>XK(NILS_AVG))CYCLE
	  YU(ILS_AVG)=Func_UVIP3P(NILS_AVG,XK,YK,XU(ILS_AVG),2)
	  IF(YU(ILS_AVG)<0.0D0)YU(ILS_AVG)=0.0D0
	  FSUN_BAND(ILS_AVG)=Func_UVIP3P(NSUNL,WVSUN,FSUN,WV(ILS_AVG+IWV_SUB_START-1)/1000.0d0,1)
  ENDDO
...
!write(*,*)'WAVELENGTH_NANOMETER,ILS_INT_CONST=',WAVELENGTH_NANOMETER,ILS_INT_CONST
  ...
  IF(NWV_SUB_CAL==1)THEN
     LOCINDXTMP=MINLOC(ABS(WV_BAND_HYPO(IWV_BAND)-WV))
     INDX_WV_CAL(1)=LOCINDXTMP(1)
  ELSE
...
! testing absorption block use this block to by pass scattering simulation
!WAVELENGTH_Table_Save(IWV_BAND-NWV_BAND_START+1)=WV(INDX_WV_CAL(1))
!DEALLOCATE(TAU_TG_DK,TAU_TG_DKP,INDX_WV_CAL,XU,YU,ZU,FSUN_BAND)
!CYCLE !testing absorption optical depth calculation
! testing absorption block
  ...
  DO IWV_CAL=1,NWV_SUB_CAL
        ...
        write(*,'("SUB WAVELENGTH INFO",I4,2x,f9.2,2x,E12.6)')IWV_CAL,WV(INDX_WV_CAL(IWV_CAL)),&
        ...
        CALL RTSOSINIT(Indx_Inelastic_Scattering,INDX_WV_CAL(IWV_CAL),WAVELENGTH_MICRON,&
           NREC,NTLYERA,NTLYERO,PNDLY,ARSL_TYPE,TAUR,DEPOL_A,TAU_TG,TAU_ARSL_Ext_TMP,   &
           TAU_ARSL_Scat_TMP,RECDATASTREAM,      &
           NTLYER,NUMMIERT,MAXLORDINPUT,NUMMIEANGINPUT,PHMXDATASTREAM,DELTAM_INPUT,LBDO_LD,FLAM,CHLa,&
...
!testing
!IF(IWV_BAND==1)THEN
!write(*,*)'WAVELENGTH_MICRON,WV,FSUN_BAND,DIRAD,DSTOKES(1)',WAVELENGTH_MICRON,&
!	WV(INDX_WV_CAL(IWV_CAL)-IWV_SUB_START+1), &
!    FSUN_BAND(INDX_WV_CAL(IWV_CAL)-IWV_SUB_START+1),DIRAD(1,1),DSTOKES(1,1,1,1)
!stop
!ENDIF
...
!            DIRADTMP(1:NDET,IWV_CAL,1:2)=DIRADTMP(1:NDET,IWV_CAL,1:2)-DIRAD(1:NDET,1:2)*RTMP1
  ...
  ENDDO ! NWV_SUB_CAL Loop

  IF(MONOCHROMATIC_FLAG)THEN
		DIRADFULL_SAVE(1:NDET,IWV_BAND-NWV_BAND_START+1,1:2)=DIRADTMP(1:NDET,1,1:2)
		DSTOKESFULL_SAVE(1:NDET,IWV_BAND-NWV_BAND_START+1,1:NTHETAOUT,1:NPHIOUT,1:4)= &
	  ...
	  ELSE
		 ...
		 DO IWV_SUB=IWV_SUB_START,IWV_SUB_END
			...
			IF(SNG_SCAT_EXACT)THEN
			  ...
			  CALL RTSOSINIT(Indx_Inelastic_Scattering,IWV_SUB,WAVELENGTH_MICRON,&
				NREC,NTLYERA,NTLYERO,PNDLY,ARSL_TYPE,TAUR,DEPOL_A,TAU_TG,        &
				TAU_ARSL_Ext_TMP,TAU_ARSL_Scat_TMP,                              &
				RECDATASTREAM,NTLYER,NUMMIERT,MAXLORDINPUT,NUMMIEANGINPUT,       &
				PHMXDATASTREAM,DELTAM_INPUT,LBDO_LD,FLAM,CHLa,TEMPERTURE,SALINITY,            &
                pBRDFa_input,pBRDFk_input,pBRDFb_input,pBRDFe_input)
...
!	   WRITE(*,*)'DOUBLE K DOUBLEKFUNC_EVALUATION:',WV(IWV_SUB),IWV_SUB,RTMP
    ...
    DEALLOCATE(TAU_TG_DK,TAU_TG_DKP, &
               DIRADTMP,DSTOKESTMP,INDX_WV_CAL)
    DEALLOCATE(XU,YU,ZU,FSUN_BAND)
ENDDO ! WAVELENG LOOP


CALL STOKESOUT(OUTFILE,IAEROSOL,MU_IN,NTHETAOUT,NPHIOUT,PHIOUT,MUOUT,NWV_BAND_CAL,     &
		...
		TAU_TG_NO2_Save, NTLYERA,ARSLND1_Save,ARSLND2_Save,ARSL_TYPE,&
        REFF1_Save,REFF2_Save,VEFF1_Save,VEFF2_Save,                          &
        ...
        TEMPERTURE,SALINITY,AeroFMF,RH_simu,CHLa)
CALL PACE_FWHM_DEALLO
CALL PHMXMIE_PACE_DEALLO
...
CLOSE(55)
IF(MONOCHROMATIC_FLAG) THEN
   DEALLOCATE(DIRADFULL_SAVE,DSTOKESFULL_SAVE)
ELSE
   DEALLOCATE(DIRADFULL,DSTOKESFULL)
ENDIF
DEALLOCATE(DIRAD,DSTOKES,PNDLY,ARSL_TYPE,ARSLND1_Save,ARSLND2_Save,REFF1_Save,REFF2_Save,&
...
DEALLOCATE(TAUR,DEPOL_A,ALT_LYRA,TAU_TG,TAU_ARSL_Ext_TMP,TAU_ARSL_Scat_TMP)
...
SUBROUTINE  STOKESOUT(CFILE1,IAEROSOL,MU_IN,NTHETAOUT,NPHIOUT,PHIOUT,MUOUT,NWV_BAND_CAL,&
        WAVELENGTH_Table_Save,TAU_ARSL_Ext_Save,TAU_ARSL_Scat_Save,            &
        TAU_RAY_Save,DEPOL_A_Save,TAU_TG_Save,TAU_TG_H2O_Save,TAU_TG_CO2_Save, &
		TAU_TG_CH4_Save,TAU_TG_O2_Save,TAU_TG_O3_Save, TAU_TG_NO2_Save,        &
		NTLYERA,ARSLND1_Save,ARSLND2_Save,ARSL_TYPE,                 &
        REFF1_Save,REFF2_Save,VEFF1_Save,VEFF2_Save,                           &
        MRR1_Save,MRR2_Save,MRI1_Save,MRI2_Save,ARSLND_NonSpherical,           &
        REFF_NonSpherical,VEFF_NonSpherical,MRR_NonSpherical,MRI_NonSpherical, &
        TEMPERTURE,SALINITY,AeroFMF,RH_simu,CHLa)
...
INTEGER,INTENT(IN) :: NWV_BAND_CAL,IAEROSOL
INTEGER :: NTHETAOUT,NPHIOUT,NTLYERA,ICOM

DOUBLE PRECISION,DIMENSION(NWV_BAND_CAL) :: WAVELENGTH_Table_Save
DOUBLE PRECISION,DIMENSION(NWV_BAND_CAL,NTLYERA) ::TAU_ARSL_Ext_Save,TAU_ARSL_Scat_Save, &
                   TAU_RAY_Save,DEPOL_A_Save
DOUBLE PRECISION,DIMENSION(NWV,NTLYERA) ::TAU_TG_Save,TAU_TG_H2O_Save,TAU_TG_CO2_Save,      &
				   TAU_TG_CH4_Save,TAU_TG_O2_Save,TAU_TG_O3_Save,TAU_TG_NO2_Save

DOUBLE PRECISION,DIMENSION(NTHETAOUT) :: MUOUT
DOUBLE PRECISION,DIMENSION(NPHIOUT) :: PHIOUT

!DOUBLE PRECISION,DIMENSION(1:NTLYERA+1) ::ALT_LYRA
DOUBLE PRECISION,DIMENSION(NUMMIEUSE) ::REFF1_Save,REFF2_Save,VEFF1_Save,VEFF2_Save,ARSLND1_Save,ARSLND2_Save
DOUBLE PRECISION,DIMENSION(NUMMIEUSE,NWV_BAND) ::MRR1_Save,MRR2_Save,MRI1_Save,MRI2_Save
INTEGER,DIMENSION(NTLYERA) :: ARSL_TYPE
DOUBLE PRECISION,INTENT(IN) :: ARSLND_NonSpherical,REFF_NonSpherical,VEFF_NonSpherical
DOUBLE PRECISION,DIMENSION(NWV_BAND),INTENT(IN) :: MRR_NonSpherical,MRI_NonSpherical

DOUBLE PRECISION,INTENT(IN) :: TEMPERTURE,SALINITY,AeroFMF,RH_simu,CHLa

DOUBLE PRECISION,DIMENSION(NWV_BAND) :: TAU_TG_AVG,TAU_TG_H2O_AVG,TAU_TG_CO2_AVG, &
                        TAU_TG_CH4_AVG,TAU_TG_O2_AVG,TAU_TG_O3_AVG,TAU_TG_NO2_AVG
DOUBLE PRECISION,DIMENSION(NWV_BAND,NTLYERA) ::TAU_ARSL_Ext_AVG,TAU_ARSL_Scat_AVG, &
...
! DIRAD(:,2) UPWELLING IRADIANCE
DOUBLE PRECISION,DIMENSION(:,:,:,:,:),ALLOCATABLE :: DSTOKESFULL_AVG

!DOUBLE PRECISION,DIMENSION(NDET,NWV_BAND,2) :: DIRADFULL_AVG
!DOUBLE PRECISION,DIMENSION(NDET,NWV_BAND,NTHETAOUT,NPHIOUT,4) :: DSTOKESFULL_AVG

DOUBLE PRECISION :: MU_IN,RTMP,RTMP1
LOGICAL :: MUFLG
DOUBLE PRECISION,PARAMETER :: PI=3.141592653589793d0
DOUBLE PRECISION,PARAMETER :: FACTOR=PI/180.0d0
INTEGER:: MPLIN,NDETOUT,ITHETA,IPHI,IWV,ILAYER,IDET

! HDF 5 DEFINITION
! This should map to DOUBLE PRECISION on most modern processors
...
TYPE(C_PTR) :: f_ptr
REAL,DIMENSION(1),TARGET :: HDF5RTMP
INTEGER,DIMENSION(1),TARGET :: HDF5ITMP
REAL,DIMENSION(:),TARGET,ALLOCATABLE :: HDF5RARR
REAL,DIMENSION(:,:),TARGET,ALLOCATABLE :: HDF5RARR2DIM
INTEGER,DIMENSION(:),TARGET,ALLOCATABLE :: HDF5IARR

INTEGER(hsize_t), DIMENSION(1:4) :: dims4
INTEGER(HSIZE_T), DIMENSION(1:4) :: maxdims4
REAL,DIMENSION(:,:,:,:),TARGET,ALLOCATABLE :: HDF5RARR4DIM

INTEGER :: NWV_START,NWV_END,NWV_OUTPUT,IWV_AVG,ILS_AVG

DOUBLE PRECISION :: WAVELENGTH_BAND_LO,WAVELENGTH_BAND_HI,WAVELENGTH_MICRON
DOUBLE PRECISION :: AWLOCAL,BWLOCAL,BBWLOCAL,BWRAMANLOCAL,BPTCLOCAL,BBPTCLOCAL,   &
...
INTEGER :: N_AVG_SIZE,MPL,NILS_AVG,nwv_intp_lo
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XK,YK
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XU,YU,ZU,TempU
DOUBLE PRECISION :: Func_UVIP3P,ILS_INT_CONST,TRAPZOID
logical:: exist
ALLOCATE(DIRADFULL_AVG(NDET,NWV_BAND,2),&
		 DSTOKESFULL_AVG(NDET,NWV_BAND,NTHETAOUT,NPHIOUT,4))
ALLOCATE(XK(MAXVAL(NILS_PACE)),YK(MAXVAL(NILS_PACE)))
DIRADFULL_AVG=0.0D0
...
IF(ILS_FLAG)THEN
  ...
  IF(WV_SEG_FLAG==0)THEN
		NWV_START=1
		NWV_END=NWV_BAND
  ELSEIF(WV_SEG_FLAG==1 .or. WV_SEG_FLAG==3)THEN
		NWV_START=1
		NWV_END=NWV_BAND_SEG1P2+NWV_BAND_SEG3
  ELSEIF(WV_SEG_FLAG==2)THEN
		NWV_START=NWV_BAND_SEG1+1
		NWV_END=NWV_BAND_SEG1P2
  ELSEIF(WV_SEG_FLAG==4)THEN
		NWV_START=NWV_BAND_SEG1P2+NWV_BAND_SEG3+1
		NWV_END=NWV_BAND
  ENDIF
  ...
  ALLOCATE(TempU(NWV_BAND_CAL))
  DO IWV_AVG=NWV_START,NWV_END
	 IF(WV_SEG_FLAG==1 .AND.WV_BAND(IWV_AVG)>WV_BAND(NWV_BAND_SEG1))CYCLE
     IF(OCEAN_CASE_SELECT<0) THEN
      fiso_avg(IWV_AVG)=Func_UVIP3P(nwv_land_ref,wavelength_land_ref,fiso_input,WV_BAND(IWV_AVG),1)
	 ...
	 IF(IWV_AVG<=NWV_BAND_SEG1P2)THEN
			NILS_AVG=NILS_PACE(IWV_AVG)
	 ELSE IF(IWV_AVG<=NWV_BAND_SEG1P2+NWV_BAND_SEG3)THEN
			NILS_AVG=NILS_SPEXone
	 ELSE
			NILS_AVG=NILS_HARP
	 ENDIF
     ...
     DO ILS_AVG=1,N_AVG_SIZE
		IF(XU(ILS_AVG)<XK(1) .OR. XU(ILS_AVG)>XK(NILS_AVG))THEN
			 YU(ILS_AVG)=0.0D0
		ELSE
             YU(ILS_AVG)=Func_UVIP3P(NILS_AVG,XK,YK,XU(ILS_AVG),2)
             IF(YU(ILS_AVG)<0.0D0)YU(ILS_AVG)=0.0D0
     ...
     ZU(1:N_AVG_SIZE)=SUM(TAU_TG_Save(LOCINDXTMP(1):LOCINDXTMP1(1),:),2)
	 ZU=EXP(-ZU)
	 ZU=ZU*YU
	 TAU_TG_AVG(IWV_AVG)=TRAPZOID(N_AVG_SIZE,XU,ZU)
	 TAU_TG_AVG(IWV_AVG)=TAU_TG_AVG(IWV_AVG)/ILS_INT_CONST
	 TAU_TG_AVG(IWV_AVG)=-LOG(TAU_TG_AVG(IWV_AVG))

	 ZU(1:N_AVG_SIZE)=SUM(TAU_TG_H2O_Save(LOCINDXTMP(1):LOCINDXTMP1(1),:),2)
     ZU=EXP(-ZU)
	 ZU=ZU*YU
	 TAU_TG_H2O_AVG(IWV_AVG)=TRAPZOID(N_AVG_SIZE,XU,ZU)
     TAU_TG_H2O_AVG(IWV_AVG)=TAU_TG_H2O_AVG(IWV_AVG)/ILS_INT_CONST
     TAU_TG_H2O_AVG(IWV_AVG)=-LOG(TAU_TG_H2O_AVG(IWV_AVG))

     ZU(1:N_AVG_SIZE)=SUM(TAU_TG_CO2_Save(LOCINDXTMP(1):LOCINDXTMP1(1),:),2)
     ZU=EXP(-ZU)
	 ZU=ZU*YU
	 TAU_TG_CO2_AVG(IWV_AVG)=TRAPZOID(N_AVG_SIZE,XU,ZU)
     TAU_TG_CO2_AVG(IWV_AVG)=TAU_TG_CO2_AVG(IWV_AVG)/ILS_INT_CONST
     TAU_TG_CO2_AVG(IWV_AVG)=-LOG(TAU_TG_CO2_AVG(IWV_AVG))

	 ZU(1:N_AVG_SIZE)=SUM(TAU_TG_CH4_Save(LOCINDXTMP(1):LOCINDXTMP1(1),:),2)
     ZU=EXP(-ZU)
	 ZU=ZU*YU
	 TAU_TG_CH4_AVG(IWV_AVG)=TRAPZOID(N_AVG_SIZE,XU,ZU)
     TAU_TG_CH4_AVG(IWV_AVG)=TAU_TG_CH4_AVG(IWV_AVG)/ILS_INT_CONST
     TAU_TG_CH4_AVG(IWV_AVG)=-LOG(TAU_TG_CH4_AVG(IWV_AVG))

	 ZU(1:N_AVG_SIZE)=SUM(TAU_TG_O2_Save(LOCINDXTMP(1):LOCINDXTMP1(1),:),2)
	 ZU=EXP(-ZU)
	 ZU=ZU*YU
	 TAU_TG_O2_AVG(IWV_AVG)=TRAPZOID(N_AVG_SIZE,XU,ZU)
     TAU_TG_O2_AVG(IWV_AVG)=TAU_TG_O2_AVG(IWV_AVG)/ILS_INT_CONST
     TAU_TG_O2_AVG(IWV_AVG)=-LOG(TAU_TG_O2_AVG(IWV_AVG))

	 ZU(1:N_AVG_SIZE)=SUM(TAU_TG_O3_Save(LOCINDXTMP(1):LOCINDXTMP1(1),:),2)
     ZU=EXP(-ZU)
	 ZU=ZU*YU
	 TAU_TG_O3_AVG(IWV_AVG)=TRAPZOID(N_AVG_SIZE,XU,ZU)
     TAU_TG_O3_AVG(IWV_AVG)=TAU_TG_O3_AVG(IWV_AVG)/ILS_INT_CONST
     TAU_TG_O3_AVG(IWV_AVG)=-LOG(TAU_TG_O3_AVG(IWV_AVG))

	 ZU(1:N_AVG_SIZE)=SUM(TAU_TG_NO2_Save(LOCINDXTMP(1):LOCINDXTMP1(1),:),2)
     ZU=EXP(-ZU)
	 ZU=ZU*YU
	 TAU_TG_NO2_AVG(IWV_AVG)=TRAPZOID(N_AVG_SIZE,XU,ZU)
	 ...
	 LOCINDXTMP2=MINLOC(ABS(WV_BAND(IWV_AVG)-WAVELENGTH_Table_Save))
		IF(LOCINDXTMP2(1)==NWV_BAND_CAL)THEN
			nwv_intp_lo=LOCINDXTMP2(1)-1
		ELSE
			nwv_intp_lo=LOCINDXTMP2(1)
		ENDIF

     DO ILAYER=1,NTLYERA

        IF((WV_SEG_FLAG==4 .AND. MONOCHROMATIC_FLAG) .or. WV_BAND(IWV_AVG)>900D0)THEN
        ...
        ELSE
			...
			IF(TAU_ARSL_Ext_Save(nwv_intp_lo,ILAYER)<1.0D-6)THEN
				TAU_ARSL_Ext_AVG(IWV_AVG,ILAYER)=TAU_ARSL_Ext_Save(nwv_intp_lo,ILAYER)
			ELSE
				RTMP=LOG(TAU_ARSL_Ext_Save(nwv_intp_lo+1,ILAYER)/TAU_ARSL_Ext_Save(nwv_intp_lo,ILAYER))
				RTMP1=LOG(WAVELENGTH_Table_Save(nwv_intp_lo+1)/WAVELENGTH_Table_Save(nwv_intp_lo))
				RTMP=RTMP/RTMP1
				TAU_ARSL_Ext_AVG(IWV_AVG,ILAYER)=TAU_ARSL_Ext_Save(nwv_intp_lo,ILAYER) * &
								  (WV_BAND(IWV_AVG)/WAVELENGTH_Table_Save(nwv_intp_lo))**RTMP
			ENDIF

			IF(TAU_ARSL_Scat_Save(nwv_intp_lo,ILAYER)<1.0D-6)THEN
				TAU_ARSL_Scat_AVG(IWV_AVG,ILAYER)=TAU_ARSL_Scat_Save(nwv_intp_lo,ILAYER)
			ELSE
				RTMP=LOG(TAU_ARSL_Scat_Save(nwv_intp_lo+1,ILAYER)/TAU_ARSL_Scat_Save(nwv_intp_lo,ILAYER))
				RTMP1=LOG(WAVELENGTH_Table_Save(nwv_intp_lo+1)/WAVELENGTH_Table_Save(nwv_intp_lo))
				RTMP=RTMP/RTMP1
				TAU_ARSL_Scat_AVG(IWV_AVG,ILAYER)=TAU_ARSL_Scat_Save(nwv_intp_lo,ILAYER) * &
								  (WV_BAND(IWV_AVG)/WAVELENGTH_Table_Save(nwv_intp_lo))**RTMP
			...
			IF(TAU_RAY_Save(nwv_intp_lo,ILAYER)<1.0D-6)THEN
				TAU_RAY_AVG(IWV_AVG,ILAYER)=TAU_RAY_Save(nwv_intp_lo,ILAYER)
			ELSE
				RTMP=LOG(TAU_RAY_Save(nwv_intp_lo+1,ILAYER)/TAU_RAY_Save(nwv_intp_lo,ILAYER))
				RTMP1=LOG(WAVELENGTH_Table_Save(nwv_intp_lo+1)/WAVELENGTH_Table_Save(nwv_intp_lo))
				RTMP=RTMP/RTMP1
				TAU_RAY_AVG(IWV_AVG,ILAYER)=TAU_RAY_Save(nwv_intp_lo,ILAYER) * &
								  (WV_BAND(IWV_AVG)/WAVELENGTH_Table_Save(nwv_intp_lo))**RTMP
...
!			TAU_ARSL_Ext_AVG(IWV_AVG,ILAYER)=Func_UVIP3P(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,WV_BAND(IWV_AVG),2)
!			TempU=TAU_ARSL_Scat_Save(:,ILAYER)
!			TAU_ARSL_Scat_AVG(IWV_AVG,ILAYER)=Func_UVIP3P(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,WV_BAND(IWV_AVG),2)
!			TempU=TAU_RAY_Save(:,ILAYER)
!			TAU_RAY_AVG(IWV_AVG,ILAYER)=Func_UVIP3P(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,WV_BAND(IWV_AVG),3)
!			TempU=DEPOL_A_Save(:,ILAYER)
!			DEPOL_A_AVG(IWV_AVG,ILAYER)=Func_UVIP3P(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,WV_BAND(IWV_AVG),2)
		ENDIF

     ENDDO

	 IF(MONOCHROMATIC_FLAG)THEN
!INTERPOLATE DIRADFULL_SAVE TO GET DIRADFULL_AVG
!INTERPOLATE DSTOKESFULL_SAVE TO GET DSTOKESFULL_AVG
        IF(WV_SEG_FLAG==4 .OR. WV_BAND(IWV_AVG)>900.0d0)THEN
			LOCINDXTMP2=MINLOC(ABS(WV_BAND(IWV_AVG)-WAVELENGTH_Table_Save))
			DO IDET=1,NDET
			DO ICOM=1,2
				DIRADFULL_AVG(IDET,IWV_AVG,ICOM)= DIRADFULL_SAVE(IDET,LOCINDXTMP2(1),ICOM)
			ENDDO
			ENDDO
			...
			DO ITHETA=1,NTHETAOUT
			DO IPHI=1,NPHIOUT
				DSTOKESFULL_AVG(IDET,IWV_AVG,ITHETA,IPHI,ICOM)= &
					 DSTOKESFULL_SAVE(IDET,LOCINDXTMP2(1),ITHETA,IPHI,ICOM)
			ENDDO
			ENDDO
			ENDDO
			ENDDO
        ...
        ELSE
			DO IDET=1,NDET
			DO ICOM=1,2
				TempU(:)=DIRADFULL_SAVE(IDET,:,ICOM)
				DIRADFULL_AVG(IDET,IWV_AVG,ICOM)= Func_UVIP3P(NWV_BAND_CAL,&
								   WAVELENGTH_Table_Save,TempU,WV_BAND(IWV_AVG),2)
			ENDDO
			ENDDO
			...
			DO ITHETA=1,NTHETAOUT
			DO IPHI=1,NPHIOUT
				TempU(1:NWV_BAND_CAL)=DSTOKESFULL_SAVE(IDET,1:NWV_BAND_CAL,ITHETA,IPHI,ICOM)
				DSTOKESFULL_AVG(IDET,IWV_AVG,ITHETA,IPHI,ICOM)= &
					Func_UVIP3P(NWV_BAND_CAL,WAVELENGTH_Table_Save,TempU,&
								  WV_BAND(IWV_AVG),2)
			ENDDO
			ENDDO
			ENDDO
...
IF(ILS_FLAG)THEN
	CALL h5screate_simple_f(1, dimscl, space, hdferr)
	CALL h5dcreate_f(file, 'fiso', H5T_IEEE_F32LE, space, c_dim_nwv_output, hdferr)
	ALLOCATE(HDF5RARR(NWV_OUTPUT))
	HDF5RARR(1:NWV_OUTPUT)=fiso_avg(NWV_START:NWV_END)
	CALL h5dwrite_f(c_dim_nwv_output, H5T_NATIVE_REAL, C_LOC(HDF5RARR(1)), hdferr)
	DEALLOCATE(HDF5RARR)
	CALL h5dsset_scale_f(c_dim_nwv_output, hdferr)
	CALL h5sclose_f(space, hdferr)
	...
	CALL h5screate_simple_f(1, dimscl, space, hdferr)
	CALL h5dcreate_f(file, 'fgeo', H5T_IEEE_F32LE, space, c_dim_nwv_output, hdferr)
	ALLOCATE(HDF5RARR(NWV_OUTPUT))
	HDF5RARR(1:NWV_OUTPUT)=fgeo_avg(NWV_START:NWV_END)
	CALL h5dwrite_f(c_dim_nwv_output, H5T_NATIVE_REAL, C_LOC(HDF5RARR(1)), hdferr)
	DEALLOCATE(HDF5RARR)
	CALL h5dsset_scale_f(c_dim_nwv_output, hdferr)
	CALL h5sclose_f(space, hdferr)
	...
	CALL h5screate_simple_f(1, dimscl, space, hdferr)
	CALL h5dcreate_f(file, 'fvol', H5T_IEEE_F32LE, space, c_dim_nwv_output, hdferr)
	ALLOCATE(HDF5RARR(NWV_OUTPUT))
	HDF5RARR(1:NWV_OUTPUT)=fvol_avg(NWV_START:NWV_END)
	CALL h5dwrite_f(c_dim_nwv_output, H5T_NATIVE_REAL, C_LOC(HDF5RARR(1)), hdferr)
	DEALLOCATE(HDF5RARR)
	CALL h5dsset_scale_f(c_dim_nwv_output, hdferr)
	CALL h5sclose_f(space, hdferr)
	...
	CALL h5screate_simple_f(1, dimscl, space, hdferr)
	CALL h5dcreate_f(file, 'Bpol', H5T_IEEE_F32LE, space, c_dim_nwv_output, hdferr)
	ALLOCATE(HDF5RARR(NWV_OUTPUT))
	HDF5RARR(1:NWV_OUTPUT)=Bpol_avg(NWV_START:NWV_END)
	CALL h5dwrite_f(c_dim_nwv_output, H5T_NATIVE_REAL, C_LOC(HDF5RARR(1)), hdferr)
	DEALLOCATE(HDF5RARR)
	CALL h5dsset_scale_f(c_dim_nwv_output, hdferr)
	CALL h5sclose_f(space, hdferr)
...
IF(ILS_FLAG)THEN
	...
	HDF5RARR2DIM(1:NUMMIEUSE,1:NWV_OUTPUT)=MRR1_Save(1:NUMMIEUSE,NWV_START:NWV_END)
	...
	CALL h5dsattach_scale_f(dset, c_dim_nummieuse, 2, hdferr)
	CALL h5dclose_f(dset, hdferr)
	CALL h5sclose_f(space, hdferr)
	...
	dims= (/NUMMIEUSE,NWV_OUTPUT /)
	ALLOCATE(HDF5RARR2DIM(NUMMIEUSE,NWV_OUTPUT))
	HDF5RARR2DIM(1:NUMMIEUSE,1:NWV_OUTPUT)=MRI1_Save(1:NUMMIEUSE,NWV_START:NWV_END)
	CALL h5screate_simple_f(2, dims, space, hdferr)
	...
	dims= (/NUMMIEUSE,NWV_OUTPUT /)
	ALLOCATE(HDF5RARR2DIM(NUMMIEUSE,NWV_OUTPUT))
	HDF5RARR2DIM(1:NUMMIEUSE,1:NWV_OUTPUT)=MRR2_Save(1:NUMMIEUSE,NWV_START:NWV_END)
	CALL h5screate_simple_f(2, dims, space, hdferr)
	...
	dims= (/NUMMIEUSE,NWV_OUTPUT /)
	ALLOCATE(HDF5RARR2DIM(NUMMIEUSE,NWV_OUTPUT))
	HDF5RARR2DIM(1:NUMMIEUSE,1:NWV_OUTPUT)=MRI2_Save(1:NUMMIEUSE,NWV_START:NWV_END)
	CALL h5screate_simple_f(2, dims, space, hdferr)
...
IF(ILS_FLAG)THEN
	...
	HDF5RARR2DIM(1:NWV_OUTPUT,1:NTLYERA)=TAU_ARSL_Ext_AVG(NWV_START:NWV_END,1:NTLYERA)
	CALL h5screate_simple_f(2, dims, space, hdferr)
	...
	dims= (/NWV_OUTPUT,NTLYERA /)
	ALLOCATE(HDF5RARR2DIM(NWV_OUTPUT,NTLYERA ))
	HDF5RARR2DIM(1:NWV_OUTPUT,1:NTLYERA)=TAU_ARSL_Scat_AVG(NWV_START:NWV_END,1:NTLYERA)
	CALL h5screate_simple_f(2, dims, space, hdferr)
	...
	dims= (/NWV_OUTPUT,NTLYERA /)
	ALLOCATE(HDF5RARR2DIM(NWV_OUTPUT,NTLYERA ))
	HDF5RARR2DIM(1:NWV_OUTPUT,1:NTLYERA)=TAU_RAY_AVG(NWV_START:NWV_END,1:NTLYERA)
	CALL h5screate_simple_f(2, dims, space, hdferr)
	...
	dims= (/NWV_OUTPUT,NTLYERA /)
	ALLOCATE(HDF5RARR2DIM(NWV_OUTPUT,NTLYERA ))
	HDF5RARR2DIM(1:NWV_OUTPUT,1:NTLYERA)=DEPOL_A_AVG(NWV_START:NWV_END,1:NTLYERA)
	CALL h5screate_simple_f(2, dims, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=TAU_TG_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=SUM(TAU_TG_Save(NWV_START:NWV_END,:),2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT ))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=TAU_TG_H2O_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=SUM(TAU_TG_H2O_Save(NWV_START:NWV_END,:),2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT ))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=TAU_TG_CO2_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=SUM(TAU_TG_CO2_Save(NWV_START:NWV_END,:),2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT ))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=TAU_TG_CH4_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=SUM(TAU_TG_CH4_Save(NWV_START:NWV_END,:),2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT ))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=TAU_TG_O2_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=SUM(TAU_TG_O2_Save(NWV_START:NWV_END,:),2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT ))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=TAU_TG_O3_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=SUM(TAU_TG_O3_Save(NWV_START:NWV_END,:),2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT ))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=TAU_TG_NO2_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=SUM(TAU_TG_NO2_Save(NWV_START:NWV_END,:),2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
dimscl=(/ NTLYERA /)
ALLOCATE(HDF5IARR(NTLYERA))
HDF5IARR=ARSL_TYPE(1:NTLYERA)
CALL h5screate_simple_f(1, dimscl, space, hdferr)
CALL h5dcreate_f(file, 'Aerosol_Types', H5T_NATIVE_INTEGER, space, c_dim_ntlyera, hdferr)
CALL h5dwrite_f(c_dim_ntlyera, H5T_NATIVE_INTEGER, C_LOC(HDF5IARR(1)), hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=AW_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=AW(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=BW_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=BW(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=RINDX_WATER_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=RINDX_WATER(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=APTC_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=APTC(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=BPTC_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=BPTC(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=BBPTC_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=BBPTC(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=BBPTCfrac_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=BBPTCfrac(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=BSTC_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=BSTC(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=BBSTC_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=BBSTC(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=BBSTCfrac_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=BBSTCfrac(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=ASTC_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=ASTC(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=ACDM_AVG(NWV_START:NWV_END)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=ACDM(NWV_START:NWV_END)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'ABSORPTION_OPTICAL_DEPTH_REDUCTION_FLAG', H5T_NATIVE_INTEGER, space, dset, hdferr)
IF(ABSORPTION_TAU_REDUCE_FLAG)THEN
  HDF5ITMP=1
ELSE
  HDF5ITMP=0
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'Bottom_Case', H5T_NATIVE_INTEGER, space, dset, hdferr)
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'WAVEBAND_SEG_FLAG', H5T_NATIVE_INTEGER, space, dset, hdferr)
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'ATMOS_ZERO',H5T_NATIVE_INTEGER, space, dset, hdferr)
IF(ATMOS_ZERO)THEN
  HDF5ITMP=1
ELSE
  HDF5ITMP=0
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'SUN_GLINT_FLAG',H5T_NATIVE_INTEGER, space, dset, hdferr)
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'NonPhotochemicalQuenching_FLAG',H5T_NATIVE_INTEGER, space, dset, hdferr)
IF(NPQ_FLAG)THEN
  HDF5ITMP=1
ELSE
  HDF5ITMP=0
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'OCEAN_FCHLA_FLAG',H5T_NATIVE_INTEGER, space, dset, hdferr)
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'OCEAN_FCDOM_FLAG', H5T_NATIVE_INTEGER, space, dset, hdferr)
IF(OCEAN_FCDOM_FLAG)THEN
  HDF5ITMP=1
ELSE
  HDF5ITMP=0
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'OCEAN_RAMAN_FLAG',H5T_NATIVE_INTEGER, space, dset, hdferr)
IF(OCEAN_RAMAN_FLAG)THEN
  HDF5ITMP=1
ELSE
  HDF5ITMP=0
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'MONOCHROMATIC_FLAG',H5T_NATIVE_INTEGER, space, dset, hdferr)
IF(MONOCHROMATIC_FLAG)THEN
  HDF5ITMP=1
ELSE
  HDF5ITMP=0
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'OCEAN_PHMX_ONE',H5T_NATIVE_INTEGER, space, dset, hdferr)
IF(OCEAN_PHMX_ONE)THEN
  HDF5ITMP=1
ELSE
  HDF5ITMP=0
...
CALL h5screate_f(H5S_SCALAR_F, space, hdferr)
CALL h5dcreate_f(file, 'CHLA_HOMOGENEITY',H5T_NATIVE_INTEGER, space, dset, hdferr)
...
dimscl=(/ NWV_OUTPUT /)
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG .OR. MONOCHROMATIC_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL_AVG(1,NWV_START:NWV_END,1)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL(1,NWV_START:NWV_END,1)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG .OR. MONOCHROMATIC_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL_AVG(1,NWV_START:NWV_END,2)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL(1,NWV_START:NWV_END,2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG .OR. MONOCHROMATIC_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL_AVG(2,NWV_START:NWV_END,1)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL(2,NWV_START:NWV_END,1)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG .OR. MONOCHROMATIC_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL_AVG(2,NWV_START:NWV_END,2)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL(2,NWV_START:NWV_END,2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG .OR. MONOCHROMATIC_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL_AVG(3,NWV_START:NWV_END,1)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL(3,NWV_START:NWV_END,1)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG .OR. MONOCHROMATIC_FLAG)THEN
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL_AVG(3,NWV_START:NWV_END,2)
ELSE
  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL(3,NWV_START:NWV_END,2)
ENDIF
CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
IF(OCEAN_CASE_SELECT >= 0)THEN
	...
	ALLOCATE(HDF5RARR(NWV_OUTPUT))
	IF(ILS_FLAG .OR. MONOCHROMATIC_FLAG)THEN
	  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL_AVG(4,NWV_START:NWV_END,1)
	ELSE
	  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL(4,NWV_START:NWV_END,1)
	ENDIF
	CALL h5screate_simple_f(1, dimscl, space, hdferr)
	...
	ALLOCATE(HDF5RARR(NWV_OUTPUT))
	IF(ILS_FLAG .OR. MONOCHROMATIC_FLAG)THEN
	  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL_AVG(4,NWV_START:NWV_END,2)
	ELSE
	  HDF5RARR(1:NWV_OUTPUT)=DIRADFULL(4,NWV_START:NWV_END,2)
	ENDIF
	CALL h5screate_simple_f(1, dimscl, space, hdferr)
...
DEALLOCATE(DIRADFULL_AVG,DSTOKESFULL_AVG)
DEALLOCATE(XK,YK)
RETURN
...
DOUBLE PRECISION,DIMENSION(NTHETAOUT) :: MUOUT
DOUBLE PRECISION,DIMENSION(NPHIOUT) :: PHIOUT
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE ::THETAOUT_FREE
INTEGER :: IANG
...
INTEGER FUNCTION INDEX_PRIME_SCATTERING(NTLYERA,TAUR,PNDLY,ARSL_TYPE,WAVELENGTH_NANOMETER)
USE MIE_PHMX_PACE, ONLY : CSCAT_ARSL_BAND
USE GLOBAL_DATA, ONLY : CEXT_REF,TAU_REF,WV_SEG_FLAG,MONOCHROMATIC_FLAG
USE PACE_INSTRUMENT_DESIGN, ONLY : WV_BAND,NWV_BAND_SEG1,NWV_BAND_SEG1P2,&
                                   NWV_BAND_SEG3,NWV_BAND
IMPLICIT NONE

INTEGER,INTENT(IN) :: NTLYERA
DOUBLE PRECISION,DIMENSION(NTLYERA),INTENT(IN) :: TAUR,PNDLY
INTEGER,DIMENSION(NTLYERA),INTENT(IN) :: ARSL_TYPE
DOUBLE PRECISION,INTENT(IN) :: WAVELENGTH_NANOMETER

INTEGER :: IWV_BAND,ITLYERA
DOUBLE PRECISION:: TAU_SCAT_TOTAL
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: TAULYR
DOUBLE PRECISION :: TAUTMP,TAU_UPPER_LIM
INTEGER,DIMENSION(1) ::LOCINDXTMP

IF((WV_SEG_FLAG==4) .or. (WV_SEG_FLAG<4 .AND. WAVELENGTH_NANOMETER>900D0))THEN

    IF(WV_SEG_FLAG==4) THEN
		LOCINDXTMP =MINLOC(ABS(WAVELENGTH_NANOMETER-WV_BAND(NWV_BAND_SEG1P2+NWV_BAND_SEG3+1:NWV_BAND)))
		IWV_BAND=LOCINDXTMP(1)+NWV_BAND_SEG1P2+NWV_BAND_SEG3
    ELSE
		LOCINDXTMP =MINLOC(ABS(WAVELENGTH_NANOMETER-WV_BAND(NWV_BAND_SEG1+1:NWV_BAND_SEG1P2)))
		IWV_BAND=LOCINDXTMP(1)+NWV_BAND_SEG1
	ENDIF

ELSE
	LOCINDXTMP =MINLOC(ABS(WAVELENGTH_NANOMETER-WV_BAND(1:NWV_BAND_SEG1P2)))
...
DO ITLYERA=1,NTLYERA
  TAULYR(ITLYERA)=PNDLY(ITLYERA)*CSCAT_ARSL_BAND(ARSL_TYPE(ITLYERA),IWV_BAND) + &
                    TAUR(ITLYERA)
ENDDO
...
SUBROUTINE RTSOSINIT(Indx_Inelastic_Scattering,IWVSAVE,WAVELENGTH_MICRON,NREC,  &
       NTLYERA,NTLYERO,PNDLY,ARSL_TYPE,TAUR,DEPOL_A,TAU_TG,TAU_ARSL_Ext_TMP,    &
       TAU_ARSL_Scat_TMP,RECDATASTREAM,NTLYER,&
       NUMMIERT,MAXLORDINPUT,NUMMIEANGINPUT,PHMXDATASTREAM,DELTAM_INPUT,LBDO_LD,FLAM,CHLa,   &
       TEMPERTURE,SALINITY,pBRDFa_input,pBRDFk_input,pBRDFb_input,pBRDFe_input)

USE GLOBAL_DATA
USE SAVE_OCEAN_MOD
USE RTUTILITY, ONLY : NUMMIEANGMAX,PI,PHMX_COEFF_EXT_UNASSIGN_FLAG
USE PHASE_MATRIX_TRUNCATION
USE PACE_INSTRUMENT_DESIGN,ONLY : NWV_BAND,WV_BAND,NWV_BAND_SEG1,&
                              NWV_BAND_SEG1P2,NWV_BAND_SEG3,NWV_BAND_SEG4
USE MIE_PHMX_PACE, ONLY : NUM_SCAT_ANG,CEXT_ARSL_BAND,CSCAT_ARSL_BAND,&
                          PHMXMIE_ARSL,REFF_MINERAL,AREA_MINERAL,&
...
DOUBLE PRECISION,DIMENSION(NUMMIERT,NUMMIEANGMAX,0:6),INTENT(OUT) ::PHMXDATASTREAM

DOUBLE PRECISION,DIMENSION(NREC,7),INTENT(OUT) :: RECDATASTREAM

DOUBLE PRECISION,INTENT(INOUT):: LBDO_LD,FLAM,TEMPERTURE,SALINITY

DOUBLE PRECISION :: WAVELENGTH_NANOMETER,CHLaLOCAL,AWLOCAL,BWLOCAL,BBWLOCAL,BWRAMANLOCAL,BPTCLOCAL,&
...
DOUBLE PRECISION ::fiso_local,fvol_local,fgeo_local,Bpol_local

DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: TAURfrac,TAULYR,LBDOLYR,RARRAYTMP
...
INTEGER ::INDXDETECTOR,INDXLAMB,INDXOCEAN,INDXOCEAN1,INDXpBRDF,INDXSNOW,INDXROSSLI


INTEGER :: IMIE,IPT, NANGMIE_LOCAL,IANG
INTEGER,DIMENSION(1) ::LOCINDXTMP

!integer time_array_0(8), time_array_1(8)
!real start_time, finish_time

INTEGER :: ITLYERA,ITLYER,IREC_SHFT,ITLYERW,IWV_BAND,nwv_intp_lo,MPL
DOUBLE PRECISION :: RTMP,RTMP1,RTMP2,OCEAN_ALBEDO, &
...
DOUBLE PRECISION,DIMENSION(0:NUMMIEUSE) ::ExtX_AEROSOL,ScatX_AEROSOL,AEROSOL_ALBEDO,AEROSOL_ABSORB_FAC
...
ELSE
	LOCINDXTMP =MINLOC(ABS(WAVELENGTH_NANOMETER-WV_BAND(1:NWV_BAND_SEG1)))
    IWV_BAND=LOCINDXTMP(1)
	IF(IWV_BAND==119 .or. IWV_BAND==291 .or. IWV_BAND==341 .or. IWV_BAND==NWV_BAND)THEN
		nwv_intp_lo=IWV_BAND-1
	ELSE
		nwv_intp_lo=IWV_BAND
	ENDIF
	RTMP1=LOG(WV_BAND(nwv_intp_lo+1)/WV_BAND(nwv_intp_lo))

    DO IMIE=1,NUMMIEUSE
        RTMP=LOG(CEXT_ARSL_BAND(IMIE,nwv_intp_lo+1)/CEXT_ARSL_BAND(IMIE,nwv_intp_lo))
		RTMP=RTMP/RTMP1
		ExtX_AEROSOL(IMIE)=CEXT_ARSL_BAND(IMIE,nwv_intp_lo) *                &
                          (WAVELENGTH_NANOMETER/WV_BAND(nwv_intp_lo))**RTMP

        RTMP=LOG(CSCAT_ARSL_BAND(IMIE,nwv_intp_lo+1)/CSCAT_ARSL_BAND(IMIE,nwv_intp_lo))
		RTMP=RTMP/RTMP1
		ScatX_AEROSOL(IMIE)=CSCAT_ARSL_BAND(IMIE,nwv_intp_lo) * &
                          (WAVELENGTH_NANOMETER/WV_BAND(nwv_intp_lo))**RTMP

!        RARRAYTMP=CEXT_ARSL_BAND(IMIE,:)
!		ExtX_AEROSOL(IMIE)=Func_UVIP3P(MPL,WV_BAND(nwv_intp_lo),RARRAYTMP(nwv_intp_lo),WAVELENGTH_NANOMETER,2)
!        RARRAYTMP=CSCAT_ARSL_BAND(IMIE,:)
!        ScatX_AEROSOL(IMIE)=Func_UVIP3P(MPL,WV_BAND(nwv_intp_lo),RARRAYTMP(nwv_intp_lo),WAVELENGTH_NANOMETER,2)
    ENDDO
ENDIF
...
DO IMIE=1,NUMMIEUSE
	IF(AEROSOL_ALBEDO(IMIE)>1.0D0)THEN
		WRITE(*,*)'CHECK, AEROSOL_ALBEDO=',AEROSOL_ALBEDO(IMIE)
		AEROSOL_ALBEDO(IMIE)=1.0D0
	ENDIF

	IF(AEROSOL_ALBEDO(IMIE)<0.0D0)THEN
		WRITE(*,*)'CHECK, AEROSOL_ALBEDO=',AEROSOL_ALBEDO(IMIE)
		AEROSOL_ALBEDO(IMIE)=0.0D0
	ENDIF
ENDDO
...
!write(*,*)'iwv_band,CEXT_REF,ExtX_AEROSOL=',iwv_band,CEXT_REF,ExtX_AEROSOL

PHMX_COEFF_EXT_UNASSIGN_FLAG=.false.
IF(.NOT. PHMX_COEFF_EXT_UNASSIGN_FLAG) &
	CALL PHMX_COEFF_ALLO(MAXLORDINPUT,NUMMIERT,NUMMIEANGINPUT)

IREC_SHFT=1
IMIE=1
DO ITLYERA=1,NTLYERA
   TAU_ARSL_Ext_TMP(ITLYERA)=PNDLY(ITLYERA)*ExtX_AEROSOL(ARSL_TYPE(ITLYERA))
   TAU_ARSL_Scat_TMP(ITLYERA)=PNDLY(ITLYERA)*ScatX_AEROSOL(ARSL_TYPE(ITLYERA))

   TAULYR(ITLYERA)=TAU_ARSL_Ext_TMP(ITLYERA)
   IPT=ITLYERA
   IF(TAULYR(ITLYERA)<1.0D-6) IPT=1
   RECDATASTREAM(ITLYERA+IREC_SHFT,1)=IPT

! total optical depth=aerosol+rayleigh+gas absorption
   ABSORPTION_TAU=TAU_TG(ITLYERA)+TAULYR(ITLYERA)*AEROSOL_ABSORB_FAC(ARSL_TYPE(ITLYERA))
   SCATTERING_TAU=TAUR(ITLYERA)+TAU_ARSL_Scat_TMP(ITLYERA)
  ...
  PHMXDATASTREAM(ITLYERA,1:NUMMIEANGMAX,0)=PHMXMIE_ARSL(1,IWV_BAND,1:NUMMIEANGMAX,0)

  IF(IPT==1 .OR. abs(1.0d0-TAURfrac(ITLYERA))<1.0e-8)THEN
    CALL RAYPHMXASSIGN(NUMMIERT,NUMMIEANGMAX,NUMMIEANGINPUT,PHMXDATASTREAM,ITLYERA,DEPOL_A(ITLYERA))
	IF(.NOT. PHMX_COEFF_EXT_UNASSIGN_FLAG) &
		CALL RAYCOEFFASSIGN(IPT,MAXLORDINPUT)
  ELSE
    PHMXDATASTREAM(ITLYERA,1:NUMMIEANGMAX,1:6)=PHMXMIE_ARSL(ARSL_TYPE(ITLYERA),IWV_BAND,1:NUMMIEANGMAX,1:6)
    CALL PHMXMIXING(NUMMIERT,NUMMIEANGMAX,NUMMIEANGINPUT,PHMXDATASTREAM,&
                     ITLYERA,TAURfrac(ITLYERA),DEPOL_A(ITLYERA))
	IF(.NOT. PHMX_COEFF_EXT_UNASSIGN_FLAG) &
       CALL COEFFMIXING(NUMMIERT,IPT,MAXLORDINPUT,IWV_BAND,ARSL_TYPE(ITLYERA),TAURfrac(ITLYERA))
...
IF(ABSORPTION_TAU_REDUCE_FLAG)THEN
  ...
  DO ITLYERA =1,NTLYERA+1
	RTMP=RTMP+TAU_REDUCE_ATMOS_PRESERVE(ITLYERA)
	TAU_REDUCE_ATMOS_PRESERVE(ITLYERA)=RTMP
  ENDDO
ENDIF
...
! Snow LAND BOTTOM
	...
	RECDATASTREAM(NTLYERA+IREC_SHFT,6)=0.0d0
CASE (-203)
! Ross_Li + Polarized reflectance LAND BOTTOM
	fiso_local=Func_UVIP3P(nwv_land_ref,wavelength_land_ref,fiso_input,WAVELENGTH_NANOMETER,1)
...
CASE DEFAULT
	...
	LOOP_LAYERS: DO ITLYERW=1,NTLYERO+1
	  ...
	  IF(OCEAN_CASE_SELECT >0 ) LBDO_LD=0.33d0*RTMP2/(RTMP2+RTMP1)
	  ...
	  ELSE
			IPT=NTLYERA+ITLYERW
	  ENDIF
	  RECDATASTREAM(NTLYERA+NDET+1+ITLYERW,1)=IPT
	...
	!RECDATASTREAM(NTLYERA+4+ITLYERW,2)
	  ...
	  IF(OCEAN_CASE_SELECT==0)THEN
		 ...
		 PHMXDATASTREAM(NTLYERA+1,1:NUMMIEANGMAX,0)=PHMXMIE_ARSL(1,IWV_BAND,1:NUMMIEANGMAX,0)
	  ...
	  ELSEIF(OCEAN_CASE_SELECT==1 .or. OCEAN_CASE_SELECT==3)THEN
		 ...
		 PHMXDATASTREAM(NTLYERA+ITLYERW,1:NUMMIEANGMAX,0)=PHMXMIE_ARSL(1,IWV_BAND,1:NUMMIEANGMAX,0)
	  ...
	  ELSE
		 ...
		 NUMMIEANGINPUT(ITLYER)=NUM_SCAT_ANG(3)
		 IF(NUM_SCAT_ANG(3) .NE. NUM_SCAT_ANG(4))STOP 'CHECK NUM_SCAT_ANG'
		 CALL PHMXMIXING(NUMMIERT,NUMMIEANGMAX,NUMMIEANGINPUT,PHMXDATASTREAM,&
						 ITLYER,TAURfracLOCAL,DEPOL_O)
	  ENDIF

! INITIALIZE OCEAN PHASE MATRIX EXPANSION
	IF(.NOT. PHMX_COEFF_EXT_UNASSIGN_FLAG) &
	  CALL PHMX_EXPANSION_SINGLELAYER(MAXLORDINPUT,DELTAM_INPUT,NUMMIERT,&
							 NUMMIEANGINPUT,PHMXDATASTREAM,IPT)

	ENDDO LOOP_LAYERS

	IF(ABSORPTION_TAU_REDUCE_FLAG)THEN
	  ABSORPTION_TAU_CUMMU=TAU_REDUCE_ATMOS_PRESERVE(NTLYERA+1)
...
IF(.NOT. PHMX_COEFF_EXT_UNASSIGN_FLAG) &
	CALL PHMX_ASSIGN(NUMMIERT,NUMMIEANGINPUT,PHMXDATASTREAM)
...
IMPLICIT NONE
DOUBLE PRECISION,INTENT(IN) :: WAVELENGTH_MICRON
INTEGER,INTENT(IN) ::NTLYERA
!DOUBLE PRECISION,DIMENSION(1:NTLYERA+1),INTENT(IN) ::ALT_LYRA
DOUBLE PRECISION,DIMENSION(NTLYERA),INTENT(OUT) :: TAUR,DEPOL_A

DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: TAUR_GRID,TAUR_CUMSUM,DEPOL_GRID

DOUBLE PRECISION, PARAMETER :: C1=400.,C2=450.   !unit ppmv
DOUBLE PRECISION, PARAMETER :: nas_coeff1=5792105D0, nas_coeff2=238.0185D0, &
                     nas_coeff3=167917D0,nas_coeff4=57.362D0
DOUBLE PRECISION, PARAMETER :: nws_coeff0=1.022D0,nws_coeff1=295.235D0,     &
		nws_coeff2=2.6422D0,nws_coeff3=-0.032380D0,nws_coeff4=0.004028D0
DOUBLE PRECISION, PARAMETER :: Mw=0.018015         !unit kg/mol
DOUBLE PRECISION, PARAMETER :: R_Universal=8.31451 !unit: J/mol/K
DOUBLE PRECISION, PARAMETER :: AvogadroNumber=6.022E23
DOUBLE PRECISION :: n_asn1e8,n_asn1,nwsn1e8,nwsn1,rho_ws
DOUBLE PRECISION, DIMENSION(:),ALLOCATABLE :: naxsn1,Ma_profile,Z_func

DOUBLE PRECISION, PARAMETER :: Z_func_a0=1.58123e-6,Z_func_a1=-2.9331e-8,      &
...
DOUBLE PRECISION, PARAMETER :: pressure_standard=101325.,temperature_standard=288.15 ! Kelvin
DOUBLE PRECISION, DIMENSION(:),ALLOCATABLE :: water_mole_number,water_vapor_partial_pressure, &
                                    rho_axs,rho_a,rho_w,rho_w_ratio,rho_a_ratio,    &
                                    n_refractive_index,King_Factor
DOUBLE PRECISION :: F1_N2,F2_O2,F3_Ar,F4_CO2,F5_H2O

INTEGER:: ILYERA,IHEIGHT
DOUBLE PRECISION :: RAY_SCALE_HEIGHT_SUB,RTMP
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: MOLEXT
...
DO IHEIGHT=1,NTLYERA
   TAUR(IHEIGHT)=Func_UVIP3P(NPGRID,Z_FIELD,TAUR_CUMSUM,ALT_LYRA(IHEIGHT+1),2)
   RTMP=0.5d0*(ALT_LYRA(IHEIGHT)+ALT_LYRA(IHEIGHT+1))
   DEPOL_A(IHEIGHT)=Func_UVIP3P(NPGRID,Z_FIELD,DEPOL_GRID,RTMP,2)
...
SUBROUTINE ARSL_VERP_INIT(IAEROSOL,NTLYERA,PNDLY,ARSL_TYPE)
USE GLOBAL_DATA, ONLY : NUMMIEUSE,WV_PACE_REF,TAU_REF,CEXT_REF,Height_Particle, &
				Height_Variance_Particle,ALT_LYRA,WV_SEG_FLAG,MONOCHROMATIC_FLAG
USE PACE_INSTRUMENT_DESIGN,ONLY : NWV_BAND,WV_BAND,NWV_BAND_SEG1P2,NWV_BAND_SEG3
USE MIE_PHMX_PACE, ONLY : CEXT_ARSL_BAND
IMPLICIT NONE

INTEGER :: NTLYERA,IAEROSOL
!DOUBLE PRECISION,DIMENSION(1:NTLYERA+1),INTENT(IN)::ALT_LYRA
DOUBLE PRECISION,DIMENSION(NTLYERA),INTENT(OUT)::PNDLY
INTEGER,DIMENSION(NTLYERA),INTENT(OUT)::ARSL_TYPE
...
!HEIGHT=(/45.2205,25.1867,21.8404,20.3539,12.0843,8.0429,4.9380,0./)
!DOUBLE PRECISION,DIMENSION(7) :: AEXPFAC,BEXPFAC

DOUBLE PRECISION :: ALTMID,TAU_TEMP,HEIGHT_TEMP,HEIGHT_TEMP1,HEIGHT_TEMP2
INTEGER :: ILYERA,IHEIGHT,IWV_BAND,IMIE

INTEGER,DIMENSION(1) ::LOCINDXTMP

PNDLY=0.0D0
ARSL_TYPE=0  ! DEFAULT VALUE REPRESENTING MOLECULES
!DO IHEIGHT=1,7
...
! IF(IAEROSOL==-98)THEN
!    IF(ALTMID<Height_Particle(1) .AND. ALTMID>0.5D0*Height_Particle(1))THEN
!       PNDLY(ILYERA)=1.0D0
!       ARSL_TYPE(ILYERA)=1         ! FIRST TYPE SCATTER, HERE MEANS CLOUD PARTICLE
!    ENDIF
! ELSEIF(IAEROSOL==-97)THEN
!    HEIGHT_TEMP=(Height_Particle(1)+Height_Particle(2))/2
!    HEIGHT_TEMP1=MAX(HEIGHT_TEMP,Height_Particle(1)-2*Height_Variance_Particle(1))
!    HEIGHT_TEMP2=MIN(HEIGHT_TEMP,Height_Particle(2)+2*Height_Variance_Particle(2))
!	IF(ALTMID<Height_Particle(1)+2*Height_Variance_Particle(1) .AND. ALTMID>HEIGHT_TEMP1)  THEN
!!Equation 2 of Wu L et al., Geophysical Research Letters, 2016. https://doi.org/10.1002/2016GL069848
!       PNDLY(ILYERA)=exp(-4.0D0*LOG(2.0D0)*(ALTMID-Height_Particle(1))**2/Height_Variance_Particle(1)**2)
!	   ARSL_TYPE(ILYERA)=1         ! FIRST TYPE SCATTER, HERE MEANS TOP LAYER AEROSOLS OR CLOUDS
!	ELSEIF(ALTMID<HEIGHT_TEMP2 .AND. ALTMID>Height_Particle(2)-2*Height_Variance_Particle(2))  THEN
!	   PNDLY(ILYERA)=exp(-4.0D0*LOG(2.0D0)*(ALTMID-Height_Particle(2))**2/Height_Variance_Particle(2)**2)
!       ARSL_TYPE(ILYERA)=2         ! SECOND TYPE SCATTER, HERE MEANS LOWER LAYER AEROSOLS
!	ENDIF
! ELSE ! SINGLE LAYER OPTIONS, USE FIRST LAYER VALUES. SECOND LAYER IS IGNORED.
!!Equation 2 of Wu L et al., Geophysical Research Letters, 2016. https://doi.org/10.1002/2016GL069848
!    PNDLY(ILYERA)=exp(-4.0D0*LOG(2.0D0)*(ALTMID-Height_Particle(1))**2/Height_Variance_Particle(1)**2)
!    IF(PNDLY(ILYERA)>1.0e-6) ARSL_TYPE(ILYERA)=1
! ENDIF

 IF(TAU_REF(2)>0.0d-6)THEN
    HEIGHT_TEMP=(Height_Particle(1)+Height_Particle(2))/2
    HEIGHT_TEMP1=MAX(HEIGHT_TEMP,Height_Particle(1)-2*Height_Variance_Particle(1))
    HEIGHT_TEMP2=MIN(HEIGHT_TEMP,Height_Particle(2)+2*Height_Variance_Particle(2))
	IF(ALTMID<Height_Particle(1)+2*Height_Variance_Particle(1) .AND. ALTMID>HEIGHT_TEMP1)  THEN
...
ENDDO
IF((WV_SEG_FLAG==4))THEN
	LOCINDXTMP =MINLOC(ABS(WV_PACE_REF - &
                           WV_BAND(NWV_BAND_SEG1P2+NWV_BAND_SEG3+1:NWV_BAND)))
    IWV_BAND=LOCINDXTMP(1)+NWV_BAND_SEG1P2+NWV_BAND_SEG3
ELSE
	LOCINDXTMP =MINLOC(ABS(WV_PACE_REF-WV_BAND(1:NWV_BAND_SEG1P2)))
    IWV_BAND=LOCINDXTMP(1)
ENDIF

CEXT_REF(1:NUMMIEUSE)=CEXT_ARSL_BAND(1:NUMMIEUSE,IWV_BAND)

DO IMIE=1,NUMMIEUSE
	TAU_TEMP=0.0d0
	DO ILYERA=1,NTLYERA
	   IF(ARSL_TYPE(ILYERA)==IMIE) &
		   TAU_TEMP=TAU_TEMP+PNDLY(ILYERA)*CEXT_REF(ARSL_TYPE(ILYERA))
	ENDDO
	DO ILYERA=1,NTLYERA
		IF(ARSL_TYPE(ILYERA)==IMIE) &
		  PNDLY(ILYERA)=PNDLY(ILYERA)*(TAU_REF(IMIE))/TAU_TEMP
	ENDDO
ENDDO

DO IMIE=1,NUMMIEUSE
! TESTING RESCALE ALGORITHM AND SEE IF TAU_REF IS CORRECTLY CALCULATED
	TAU_TEMP=0.0d0
	DO ILYERA=1,NTLYERA
	   IF(ARSL_TYPE(ILYERA)==IMIE) &
		   TAU_TEMP=TAU_TEMP+PNDLY(ILYERA)*CEXT_REF(ARSL_TYPE(ILYERA))
    ...
    write(*,'("IMIE,WV_BAND,TAU_REF is:",I4,f9.2,2(E12.4,1x))') &
            IMIE,WV_BAND(IWV_BAND),TAU_REF(IMIE),TAU_TEMP
ENDDO
...
DOUBLE PRECISION,DIMENSION(NUMMIERT,NUMMIEANGMAX,0:6) ::PHMXDATASTREAM

DOUBLE PRECISION :: DEPOL_LOCAL,MUF,DELTA_DEPOL,DELTA_DEPOLP
...
SUBROUTINE ARSL_COEFF_PACE_TRUNC(NUMMIEUSE,MAXLORDINPUT,DELTAM_INPUT)
USE RTTYPE, ONLY : FACTOR
USE PHASE_MATRIX_TRUNCATION
USE MIE_PHMX_PACE
USE GLOBAL_DATA, ONLY : WV_SEG_FLAG
implicit none
INTEGER,intent(in) :: NUMMIEUSE,MAXLORDINPUT,DELTAM_INPUT
INTEGER :: IWV_BAND,IMIEUSE,IANG
INTEGER :: LORD,NUMANG,NWV_START,NWV_END
DOUBLE PRECISION :: MU_LOCAL,RTMP

! DELTA FIT DIFINITION
DOUBLE PRECISION,  DIMENSION(:),ALLOCATABLE :: ANG,P11,P12,P22,P33,P34,P44
DOUBLE PRECISION :: FTRUNC_LOCAL
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: BETAL_LOCAL,ALPHAL_LOCAL,ZETAL_LOCAL,DELTAL_LOCAL,&
								   GAMMAL_LOCAL,EPSILONL_LOCAL

IF(MAXLORDINPUT>MAXVAL(NUMBETAL)-1) STOP 'MAXLORDINPUT>MAXVAL(NUMBETAL)-1'

ALLOCATE(BETAL_LOCAL(0:MAXLORDINPUT),ALPHAL_LOCAL(0:MAXLORDINPUT),ZETAL_LOCAL(0:MAXLORDINPUT),    &
		 DELTAL_LOCAL(0:MAXLORDINPUT),GAMMAL_LOCAL(0:MAXLORDINPUT),EPSILONL_LOCAL(0:MAXLORDINPUT)  )
...
ELSEIF(WV_SEG_FLAG==1 .or. WV_SEG_FLAG==3)THEN
	  NWV_START=1
	  NWV_END=NWV_BAND-NWV_BAND_SEG4
ELSEIF(WV_SEG_FLAG==2)THEN
	  NWV_START=NWV_BAND_SEG1+1
	  NWV_END=NWV_BAND_SEG1P2
ELSEIF(WV_SEG_FLAG==4)THEN
...
DO IMIEUSE=1,NUMMIEUSE
	NUMANG=NUM_SCAT_ANG(IMIEUSE)
	ALLOCATE(ANG(NUMANG),P11(NUMANG),P12(NUMANG),P22(NUMANG),P33(NUMANG),P34(NUMANG),P44(NUMANG))

DO IWV_BAND=NWV_START,NWV_END

		DO IANG=1,NUMANG
		    ANG(IANG)=PHMXMIE_ARSL(IMIEUSE,IWV_BAND,IANG,0)
		    P11(IANG)=PHMXMIE_ARSL(IMIEUSE,IWV_BAND,IANG,1)
			P22(IANG)=PHMXMIE_ARSL(IMIEUSE,IWV_BAND,IANG,2)
			P33(IANG)=PHMXMIE_ARSL(IMIEUSE,IWV_BAND,IANG,3)
			P44(IANG)=PHMXMIE_ARSL(IMIEUSE,IWV_BAND,IANG,4)
			P12(IANG)=PHMXMIE_ARSL(IMIEUSE,IWV_BAND,IANG,5)
			P34(IANG)=PHMXMIE_ARSL(IMIEUSE,IWV_BAND,IANG,6)
		ENDDO

		CALL MIECOEFF_TRUNC_MONO(NUMANG,MAXLORDINPUT,DELTAM_INPUT,  &
			ANG,P11,P22,P33,P44,P12,P34,FTRUNC_LOCAL,&
			BETAL_LOCAL,ALPHAL_LOCAL,ZETAL_LOCAL,DELTAL_LOCAL,&
			GAMMAL_LOCAL,EPSILONL_LOCAL)

		FTRUNC_ARSL(IMIEUSE,IWV_BAND)=FTRUNC_LOCAL

		DO LORD=0,MAXLORDINPUT
          BETAL_ARSL(IMIEUSE,IWV_BAND,LORD) =BETAL_LOCAL(LORD)
		  ALPHAL_ARSL(IMIEUSE,IWV_BAND,LORD)=ALPHAL_LOCAL(LORD)       !AL2(IL)
		  ZETAL_ARSL(IMIEUSE,IWV_BAND,LORD)=ZETAL_LOCAL(LORD)        !AL3(IL)
		  DELTAL_ARSL(IMIEUSE,IWV_BAND,LORD)=DELTAL_LOCAL(LORD)       !AL4(IL)
		  GAMMAL_ARSL(IMIEUSE,IWV_BAND,LORD)=GAMMAL_LOCAL(LORD)       !BET1(IL)
		  EPSILONL_ARSL(IMIEUSE,IWV_BAND,LORD)=-EPSILONL_LOCAL(LORD)  !BET2(IL)! convention sign change
											  ! double check later
		ENDDO

	ENDDO
	DEALLOCATE(ANG,P11,P12,P22,P33,P34,P44)
...
END SUBROUTINE ARSL_COEFF_PACE_TRUNC

SUBROUTINE MIECOEFF_TRUNC_MONO(NUMANG,MAXLORDINPUT,DELTAM_INPUT,  &
				ANG,P11,P22,P33,P44,P12,P34,FTRUNC_LOCAL,&
                BETAL_LOCAL,ALPHAL_LOCAL,ZETAL_LOCAL,DELTAL_LOCAL,&
                GAMMAL_LOCAL,EPSILONL_LOCAL)
USE RTTYPE, ONLY : FACTOR,PI
USE PHASE_MATRIX_TRUNCATION, ONLY : ANGLE_TRUNCATION
implicit none
INTEGER,intent(in) :: NUMANG,MAXLORDINPUT,DELTAM_INPUT
DOUBLE PRECISION,DIMENSION(NUMANG),intent(inout) :: ANG,P11,P22,P33,P44,P12,P34
DOUBLE PRECISION,intent(out) :: FTRUNC_LOCAL
DOUBLE PRECISION,DIMENSION(0:MAXLORDINPUT),intent(out) :: BETAL_LOCAL,ALPHAL_LOCAL,ZETAL_LOCAL,DELTAL_LOCAL,&
								   GAMMAL_LOCAL,EPSILONL_LOCAL

INTEGER :: IWV_BAND,IMIEUSE,IANG
INTEGER :: LORD
DOUBLE PRECISION :: MU_LOCAL,RTMP,ANGCUT

DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: DL00,DL20,DL2P2,DL2N2
! DELTA FIT DIFINITION

DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: TMPL,P11_TRUNC

ALLOCATE(DL00(0:MAXLORDINPUT),DL20(0:MAXLORDINPUT),DL2P2(0:MAXLORDINPUT),  &
		   DL2N2(0:MAXLORDINPUT))
ALLOCATE(TMPL(0:MAXLORDINPUT),P11_TRUNC(NUMANG))
ANGCUT=COS(ANGLE_TRUNCATION/180.0D0*PI)

BETAL_LOCAL=0.0D0
call bfit(MAXLORDINPUT,NUMANG,ANG,P11,ANGCUT,BETAL_LOCAL,FTRUNC_LOCAL,DELTAM_INPUT)

P11_TRUNC=0.0D0
DO IANG=1,NUMANG
   MU_LOCAL=COS(ANG(IANG)*FACTOR)
   CALL GETDMLlocal(MU_LOCAL,MAXLORDINPUT,DL00,DL20,DL2P2,DL2N2)

	DO LORD=0,MAXLORDINPUT
		P11_TRUNC(IANG)=P11_TRUNC(IANG)+DL00(LORD)*BETAL_LOCAL(LORD)
	ENDDO

	P22(IANG)=P22(IANG)/P11(IANG)*P11_TRUNC(IANG)
	P33(IANG)=P33(IANG)/P11(IANG)*P11_TRUNC(IANG)
	P44(IANG)=P44(IANG)/P11(IANG)*P11_TRUNC(IANG)
	P12(IANG)=P12(IANG)/P11(IANG)*P11_TRUNC(IANG)
	P34(IANG)=P34(IANG)/P11(IANG)*P11_TRUNC(IANG)
...
call bfit_ger(1,MAXLORDINPUT,NUMANG,ANG,P12,P11_TRUNC,ANGCUT,GAMMAL_LOCAL,DELTAM_INPUT)
call bfit_ger(2,MAXLORDINPUT,NUMANG,ANG,P22,P11_TRUNC,ANGCUT,ALPHAL_LOCAL,DELTAM_INPUT)
call bfit_ger(3,MAXLORDINPUT,NUMANG,ANG,P33,P11_TRUNC,ANGCUT,ZETAL_LOCAL,DELTAM_INPUT)
call bfit_ger(4,MAXLORDINPUT,NUMANG,ANG,P44,P11_TRUNC,ANGCUT,DELTAL_LOCAL,DELTAM_INPUT)
call bfit_ger(5,MAXLORDINPUT,NUMANG,ANG,P34,P11_TRUNC,ANGCUT,EPSILONL_LOCAL,DELTAM_INPUT)
TMPL=ALPHAL_LOCAL
ALPHAL_LOCAL=(ALPHAL_LOCAL+ZETAL_LOCAL)/2.0D0
ZETAL_LOCAL=(TMPL-ZETAL_LOCAL)/2.0D0
! **************************

DEALLOCATE(P11_TRUNC,TMPL,DL00,DL20,DL2P2,DL2N2)

END SUBROUTINE MIECOEFF_TRUNC_MONO

SUBROUTINE RAYCOEFFASSIGN(IPT,MAXLORDINPUT)
USE PHASE_MATRIX_TRUNCATION
implicit none

INTEGER,INTENT(IN) :: IPT,MAXLORDINPUT
INTEGER :: MAXLORD_LOCAL

MAXLORD_LOCAL=MIN(2,MAXLORDINPUT)
FTRUNC(IPT)=0.0D0
BETAL(IPT,0:MAXLORD_LOCAL)=BETAL_Ray_Depol0p029(0:MAXLORD_LOCAL)
ALPHAL(IPT,0:MAXLORD_LOCAL)=ALPHAL_Ray_Depol0p029(0:MAXLORD_LOCAL)
ZETAL(IPT,0:MAXLORD_LOCAL)=ZETAL_Ray_Depol0p029(0:MAXLORD_LOCAL)
DELTAL(IPT,0:MAXLORD_LOCAL)=DELTAL_Ray_Depol0p029(0:MAXLORD_LOCAL)
GAMMAL(IPT,0:MAXLORD_LOCAL)=GAMMAL_Ray_Depol0p029(0:MAXLORD_LOCAL)
EPSILONL(IPT,0:MAXLORD_LOCAL)=EPSILONL_Ray_Depol0p029(0:MAXLORD_LOCAL)

NUMORDFULLBETAL(IPT)=2
MAXLORD_LOCAL=2
FULLBETAL(IPT,0:MAXLORD_LOCAL)=BETAL_Ray_Depol0p029(0:MAXLORD_LOCAL)

END SUBROUTINE RAYCOEFFASSIGN

SUBROUTINE COEFFMIXING(NUMMIERT,IPT,MAXLORDINPUT,IWV_BAND,IMIEUSE,TAURfracLOCAL)
USE RTUTILITY, ONLY : SEC_RAD_CORR
USE PHASE_MATRIX_TRUNCATION
USE MIE_PHMX_PACE
implicit none

INTEGER,intent(in) :: NUMMIERT,IPT,MAXLORDINPUT,IWV_BAND,IMIEUSE
DOUBLE PRECISION,intent(in) :: TAURfracLOCAL
DOUBLE PRECISION :: RTMP
INTEGER :: LORD,MAXLORD_LOCAL

RTMP=1.0d0-TAURfracLOCAL
FTRUNC(IPT)=RTMP*FTRUNC_ARSL(IMIEUSE,IWV_BAND)
MAXLORD_LOCAL=MIN(2,MAXLORDINPUT)

DO LORD=0,MAXLORD_LOCAL
	BETAL(IPT,LORD) = TAURfracLOCAL*BETAL_Ray_Depol0p029(LORD) + &
						RTMP*BETAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	ALPHAL(IPT,LORD) = TAURfracLOCAL*ALPHAL_Ray_Depol0p029(LORD) +&
						RTMP*ALPHAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	ZETAL(IPT,LORD) = TAURfracLOCAL*ZETAL_Ray_Depol0p029(LORD) +&
						RTMP*ZETAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	DELTAL(IPT,LORD) = TAURfracLOCAL*DELTAL_Ray_Depol0p029(LORD) +&
						RTMP*DELTAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	GAMMAL(IPT,LORD) = TAURfracLOCAL*GAMMAL_Ray_Depol0p029(LORD) +&
						RTMP*GAMMAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	EPSILONL(IPT,LORD) = TAURfracLOCAL*EPSILONL_Ray_Depol0p029(LORD) +&
						RTMP*EPSILONL_ARSL(IMIEUSE,IWV_BAND,LORD)
ENDDO

IF(MAXLORDINPUT>2)THEN
	DO LORD=3,MAXLORDINPUT
		BETAL(IPT,LORD) =RTMP*BETAL_ARSL(IMIEUSE,IWV_BAND,LORD)
		ALPHAL(IPT,LORD) =RTMP* ALPHAL_ARSL(IMIEUSE,IWV_BAND,LORD)
		ZETAL(IPT,LORD) = RTMP*ZETAL_ARSL(IMIEUSE,IWV_BAND,LORD)
		DELTAL(IPT,LORD) =RTMP* DELTAL_ARSL(IMIEUSE,IWV_BAND,LORD)
		GAMMAL(IPT,LORD) =RTMP* GAMMAL_ARSL(IMIEUSE,IWV_BAND,LORD)
		EPSILONL(IPT,LORD) =RTMP* EPSILONL_ARSL(IMIEUSE,IWV_BAND,LORD)
	ENDDO
ENDIF

IF(.NOT. SEC_RAD_CORR) RETURN

NUMORDFULLBETAL(IPT)=NUMBETAL(IMIEUSE)-1
MAXLORD_LOCAL=MIN(2,NUMORDFULLBETAL(IPT))
DO LORD=0,MAXLORD_LOCAL
	FULLBETAL(IPT,LORD)=TAURfracLOCAL*BETAL_Ray_Depol0p029(LORD)+ &
			            RTMP*COEFFMIE_ARSL(IMIEUSE,IWV_BAND,LORD)
ENDDO
IF(NUMORDFULLBETAL(IPT)>2)THEN
	DO LORD=3,NUMORDFULLBETAL(IPT)
		FULLBETAL(IPT,LORD)=RTMP*COEFFMIE_ARSL(IMIEUSE,IWV_BAND,LORD)
	ENDDO
ENDIF

END SUBROUTINE COEFFMIXING

SUBROUTINE PHMX_EXPANSION_SINGLELAYER(MAXLORDINPUT,DELTAM_INPUT,NUMMIERT,&
                             NUMMIEANGINPUT,PHMXDATASTREAM,ITLYER)
USE RTUTILITY,ONLY : FACTOR,MAXLFULL,SEC_RAD_CORR
USE PHASE_MATRIX_TRUNCATION
IMPLICIT NONE
INTEGER,intent(in) ::    MAXLORDINPUT,DELTAM_INPUT,NUMMIERT, ITLYER
INTEGER,DIMENSION(NUMMIERT),intent(in) :: NUMMIEANGINPUT
DOUBLE PRECISION,DIMENSION(NUMMIERT,NUMMIEANGMAX,0:6),intent(in) ::PHMXDATASTREAM

! DELTA FIT DIFINITION
integer :: NUMANG,numordfull_local,IANG,LORD
DOUBLE PRECISION,  DIMENSION(:),ALLOCATABLE :: muang,ANG,P11,P12,P22,P33,P34,P44
DOUBLE PRECISION :: FTRUNC_LOCAL
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: BETAL_LOCAL,ALPHAL_LOCAL,ZETAL_LOCAL,DELTAL_LOCAL,&
								   GAMMAL_LOCAL,EPSILONL_LOCAL
DOUBLE PRECISION,dimension(:),allocatable::fullbetal_local

ALLOCATE(BETAL_LOCAL(0:MAXLORDINPUT),ALPHAL_LOCAL(0:MAXLORDINPUT),ZETAL_LOCAL(0:MAXLORDINPUT),    &
		 DELTAL_LOCAL(0:MAXLORDINPUT),GAMMAL_LOCAL(0:MAXLORDINPUT),EPSILONL_LOCAL(0:MAXLORDINPUT)  )

NUMANG=NUMMIEANGINPUT(ITLYER)
ALLOCATE(muang(NUMANG),ANG(NUMANG),P11(NUMANG),P12(NUMANG),P22(NUMANG),P33(NUMANG),P34(NUMANG),P44(NUMANG))

DO IANG=1,NUMANG
	ANG(IANG)=PHMXDATASTREAM(ITLYER,IANG,0)
	muang(IANG)=cos(ANG(IANG)*FACTOR)
	P11(IANG)=PHMXDATASTREAM(ITLYER,IANG,1)
	P22(IANG)=PHMXDATASTREAM(ITLYER,IANG,2)
	P33(IANG)=PHMXDATASTREAM(ITLYER,IANG,3)
	P44(IANG)=PHMXDATASTREAM(ITLYER,IANG,4)
	P12(IANG)=PHMXDATASTREAM(ITLYER,IANG,5)
	P34(IANG)=PHMXDATASTREAM(ITLYER,IANG,6)
ENDDO
...
FTRUNC(ITLYER)=FTRUNC_LOCAL

DO LORD=0,MAXLORDINPUT
	BETAL(ITLYER,LORD) =BETAL_LOCAL(LORD)
	ALPHAL(ITLYER,LORD) =ALPHAL_LOCAL(LORD)
	ZETAL(ITLYER,LORD) =ZETAL_LOCAL(LORD)
	DELTAL(ITLYER,LORD) =DELTAL_LOCAL(LORD)
	GAMMAL(ITLYER,LORD) =GAMMAL_LOCAL(LORD)
	EPSILONL(ITLYER,LORD) =EPSILONL_LOCAL(LORD)
ENDDO

IF(SEC_RAD_CORR) THEN

	allocate(fullbetal_local(0:MAXLFULL))
	call FULLBETAL_MONO(NUMANG,muang,P11,numordfull_local,MAXLFULL,fullbetal_local)

	NUMORDFULLBETAL(ITLYER)=numordfull_local-1
	
	DO LORD=0,NUMORDFULLBETAL(ITLYER)
		FULLBETAL(ITLYER,LORD)=fullbetal_local(LORD)
	ENDDO
	deallocate(fullbetal_local)
   
ENDIF

DEALLOCATE(muang,ANG,P11,P12,P22,P33,P34,P44)

DEALLOCATE(BETAL_LOCAL,ALPHAL_LOCAL,ZETAL_LOCAL,DELTAL_LOCAL,&
		   GAMMAL_LOCAL,EPSILONL_LOCAL)

END SUBROUTINE PHMX_EXPANSION_SINGLELAYER
...
INTEGER,DIMENSION(NUMMIERT) :: NUMMIEANGINPUT
DOUBLE PRECISION,DIMENSION(NUMMIERT,NUMMIEANGMAX,0:6) ::PHMXDATASTREAM
DOUBLE PRECISION :: TAURfracLOCAL

DOUBLE PRECISION :: DEPOL_LOCAL,MUF,DELTA_DEPOL,DELTA_DEPOLP
DOUBLE PRECISION,DIMENSION(6):: RAYPHMX_LOCAL
...
DOUBLE PRECISION,INTENT(IN) :: WAVELENGTH_MICRON,SEDIMENT_CONCENTRATION,SEDIMENT_SPECTRAL_SLOPE
DOUBLE PRECISION,INTENT(OUT):: ASTCLOCAL,BSTCLOCAL,BBSTCfracLOCAL

DOUBLE PRECISION :: ASTC443LOCAL,SEDIMENT_DENSITY,CSCAT_MINERAL,BSTC665LOCAL,WAVELENGTH_NANOMETER
INTEGER,DIMENSION(1) ::LOCINDXTMP
...
DOUBLE PRECISION,INTENT(IN) :: WAVELENGTH_MICRON,CHLaVal,TMPTR,SALINITY
DOUBLE PRECISION,INTENT(OUT)::AWLOCAL,BWLOCAL,BBWLOCAL,BWRAMANLOCAL,BPTCLOCAL,BBPTCLOCAL,&
...
DOUBLE PRECISION :: BPTC_exp_coeff,BPTC660,BPTC550,Ap_Bricaud,Ep_Bricaud
DOUBLE PRECISION :: S_cdm,WAVELENGTH_NANOMETER,BWFUNC,APTC440,ACDM440, CPTC550, CPTCLOCAL

INTEGER :: IREAD
DOUBLE PRECISION :: RTMP,RTMP1,RTMP2
REAL,DIMENSION(5) :: RNDNMBR
...
INTEGER :: I_AW_UV ! =1 USE ZP LEE 2015 doi.org/10.1364/AO.54.000546
                   ...
                   ! =3 Use IOCCG, Proctol, 2018

INTEGER :: I_AY_UV !=1 STANDARD PARAMETERIZATION, Eq.1 in Morel and Gentili, RSE, 2009
                   !=2 PACIFIC OCEAN GYRE, Morel et al. Biogeosciences, 4, 913–925, 2007, FIG. 9

INTEGER :: BIOCASE1_SELECT !==1 SCATTERING COEFFICIENT MODELED AS A POWER LAW
                           !==2 EXTINCTION COEFFICIENT MODELED AS A POWER LAW

DOUBLE PRECISION,DIMENSION(NWV_Aph1_ZPLee),PARAMETER :: WV_Aph1_ZPLee=(/350.0d0,355.0d0,360.0d0,  &
...
DOUBLE PRECISION,DIMENSION(NWV_Aph1_ZPLee),PARAMETER :: Aph1_ZPLee=(/0.001555842d0,0.00181407d0,  &
...
DOUBLE PRECISION, DIMENSION(NWV_AP_BIOSOPE) :: WV_AP_BIOSOPE,AP_BIOSOPE_UV20P,AP_BIOSOPE_UV50P,&
                                     AP_BIOSOPE_UV80P,AP_BIOSOPE_USE
DOUBLE PRECISION, DIMENSION(NWV_AP_COMPONENTS) :: WV_AP_COMPONENTS, AP_PICO_BASE, AP_MICRO_BASE
DOUBLE PRECISION :: ACDM400,ACDM370,SF_COEFF,AP_PICO_SPECIFIC676NM=0.023D0,AP_MICRO_SPECIFIC676NM=0.0086D0
DOUBLE PRECISION :: Func_UVIP3P

WAVELENGTH_NANOMETER=WAVELENGTH_MICRON*1000.0d0
BIOCASE1_SELECT=2
I_AP_UV=3
I_AY_UV=1 !=1 STANDARD PARAMETERIZATION, Eq.1 in Morel and Gentili, RSE, 2009
		  !=2 PACIFIC OCEAN GYRE, Morel et al. Biogeosciences, 4, 913–925, 2007, FIG. 9

I_AW_UV=3          ! =1 USE ZP LEE 2015 doi.org/10.1364/AO.54.000546
				   ! =2 USE Mason, Cone, Fry 2016
                   ! =3 Use IOCCG, Proctol, 2018
...
!  call aux_dir_readin
  ...
  ENDDO
  CLOSE(1)
  RTMP1=Func_UVIP3P(NWV_AP_COMPONENTS,WV_AP_COMPONENTS, AP_PICO_BASE,676.0D0,2)
  RTMP2=Func_UVIP3P(NWV_AP_COMPONENTS,WV_AP_COMPONENTS, AP_MICRO_BASE,676.0D0,2)
  AP_PICO_BASE=AP_PICO_BASE/RTMP1*AP_PICO_SPECIFIC676NM
  AP_MICRO_BASE=AP_MICRO_BASE/RTMP2*AP_MICRO_SPECIFIC676NM

  RTMP1=Func_UVIP3P(NWV_AP_COMPONENTS,WV_AP_COMPONENTS, AP_PICO_BASE,440.0D0,2)
  RTMP2=Func_UVIP3P(NWV_AP_COMPONENTS,WV_AP_COMPONENTS, AP_MICRO_BASE,440.0D0,2)
...
IF(WAVELENGTH_NANOMETER<400.0D0)THEN
    IF(I_AP_UV==1)THEN
      APTCLOCAL=Func_UVIP3P(NWV_Aph1_ZPLee,WV_Aph1_ZPLee,Aph1_ZPLee,WAVELENGTH_NANOMETER,2)
      RTMP =    Func_UVIP3P(NWV_Aph1_ZPLee,WV_Aph1_ZPLee,Aph1_ZPLee,440.0d0,2)
    ...
    ELSE
      ...
      APTCLOCAL=Func_UVIP3P(NWV_AP_BIOSOPE,WV_AP_BIOSOPE,AP_BIOSOPE_USE,WAVELENGTH_NANOMETER,2)
      RTMP=Func_UVIP3P(NWV_AP_BIOSOPE,WV_AP_BIOSOPE,AP_BIOSOPE_USE,440.0D0,2)
      APTCLOCAL=APTCLOCAL/RTMP*APTC440
    ENDIF
...
ELSEIF(WAVELENGTH_NANOMETER<700.0D0)THEN
  ...
  ELSE IF(AP_SELECT==2)THEN! USE CASE 2 AP MODEL
    RTMP1=Func_UVIP3P(NWV_AP_COMPONENTS,WV_AP_COMPONENTS, AP_PICO_BASE,WAVELENGTH_NANOMETER,2)
    RTMP2=Func_UVIP3P(NWV_AP_COMPONENTS,WV_AP_COMPONENTS, AP_MICRO_BASE,WAVELENGTH_NANOMETER,2)
...
IF(OCEAN_CASE_SELECT==1 .or. OCEAN_CASE_SELECT==2)THEN
	IF(abs(CHLaVal)<0.001D0)THEN
	   BBPTCfracLOCAL=0.01d0; ! used in Westberry, Boss, and Lee 2013
	ELSE
	   ! backscattering fraction at 550 nm and assume it is spectrally flat
	   ! derived from Morel, JGR, 2001, Eq. 13.
...
! See Eq.12 in Morel and Maritorena, JGR, 2001
    ...
    BBPTCLOCAL=BPTCLOCAL*BBPTCfracLOCAL
ENDIF
...
! See Eq.1 in Morel and Gentili, RSE, 2009
  ...
  ELSEIF(I_AY_UV==2)THEN
    S_cdm=0.016
! See FIG.9 in Morel et al. Biogeosciences, 4, 913–925, 2007.
	ACDM370=0.1*CHLaVal**0.8
	ACDMLOCAL=ACDM370*exp(-S_cdm*(WAVELENGTH_MICRON-0.37d0)*1000.)
  ELSE
     STOP 'I_AY_UV NOT INITIALIZED FOR OCEAN_CASE_SELECT ==1 OR 2'
  ENDIF
...
IF(WAVELENGTH_NANOMETER> 550.0D0) THEN
  AWLOCAL=  Func_UVIP3P(NWV_PF_LUT,WV_PF_LUT,AW_PF_LUT,WAVELENGTH_NANOMETER,2)
ELSE
  ! I_AW_UV==1 USE ZP LEE 2015 doi.org/10.1364/AO.54.000546
		 ! ==2 USE Mason, Cone, Fry 2016
         ! ==3 USE IOCCG PROCTOCOL 2018
	IF(I_AW_UV ==1) THEN
	  IF(WAVELENGTH_NANOMETER<350.0D0)THEN
		 AWLOCAL=  Func_UVIP3P(NWV_AW_IOCCG_LUT,WV_AW_IOCCG_LUT,AW_IOCCG_LUT,WAVELENGTH_NANOMETER,2)
	  ELSE
		 AWLOCAL=  Func_UVIP3P(NWV_AW_LEE_LUT,WV_AW_LEE_LUT,AW_LEE_LUT,WAVELENGTH_NANOMETER,2)
	  ENDIF
	ELSEIF(I_AW_UV ==2) THEN
	   AWLOCAL=  Func_UVIP3P(NWV_AW_MCF_LUT,WV_AW_MCF_LUT,AW_MCF_LUT,WAVELENGTH_NANOMETER,2)
    ELSEIF(I_AW_UV ==3) THEN
       AWLOCAL=  Func_UVIP3P(NWV_AW_IOCCG_LUT,WV_AW_IOCCG_LUT,AW_IOCCG_LUT,WAVELENGTH_NANOMETER,2)
    ELSE
       STOP 'I_AW_UV NOT INITIALIZED'
    ENDIF
ENDIF
...
IMPLICIT NONE
DOUBLE PRECISION, INTENT(IN) :: WAVELENGTH_NM,CHLaVal
DOUBLE PRECISION, INTENT(OUT) :: AP
integer,external :: bisearch
INTEGER,PARAMETER :: NWV_Bricaud=151
DOUBLE PRECISION,DIMENSION(NWV_Bricaud),PARAMETER :: &
...
INTEGER :: MPL
DOUBLE PRECISION :: Ap_Bricaud,Ep_Bricaud,RTMP
...
XI(1)=WAVELENGTH_NM
MPL=3
CALL UVIP3P(MPL,NWV_Bricaud,WV_Bricaud,Ap_Bricaud_LUT,1,XI,YI)
Ap_Bricaud=YI(1)
CALL UVIP3P(MPL,NWV_Bricaud,WV_Bricaud,Ep_Bricaud_LUT,1,XI,YI)
Ep_Bricaud=YI(1)
...
DOUBLE PRECISION :: CHLaLOCAL,BBPTCfracLOCAL,BWLOCAL,BPTCLOCAL
INTEGER :: NTLYERA,NUMMIERT, NUMMIEANGMAX,ITLYERW
INTEGER,DIMENSION(NUMMIERT) :: NUMMIEANGINPUT
DOUBLE PRECISION,DIMENSION(NUMMIERT,NUMMIEANGMAX,0:6) ::PHMXDATASTREAM

INTEGER :: IANG
DOUBLE PRECISION :: RTMP,RTMP1,ANGLE,NU,DLT90,XW,MUF,NORM,FFPH
...
IMPLICIT NONE
DOUBLE PRECISION :: BACKSCATTERING_FRACTION
END MODULE

SUBROUTINE FFPFCOEFF(BB,NU,DLT90)
USE FOURNIER_FORAND, ONLY : BACKSCATTERING_FRACTION
IMPLICIT NONE
DOUBLE PRECISION BB,NU,DLT90
DOUBLE PRECISION NURTLW,NURTHI,XACC,RTMP
DOUBLE PRECISION,EXTERNAL :: FNU
BACKSCATTERING_FRACTION=BB
NURTLW=-1.0D0
NURTHI=0.0D0
XACC=1.0E-12
CALL bisection(FNU, NURTLW, NURTHI, NU, XACC)
RTMP=(0.01D0-0.3084D0*NU)*(0.01D0-0.3084D0*NU)
DLT90=2.0D0/3.0D0/RTMP
END SUBROUTINE

DOUBLE PRECISION FUNCTION FNU(NU)
USE FOURNIER_FORAND
IMPLICIT NONE
DOUBLE PRECISION NU,BB
DOUBLE PRECISION DLTTMP,RTMP,DLTPNU
...
DOUBLE PRECISION FUNCTION FFPH(NU,DLT90,ANG)
IMPLICIT NONE
DOUBLE PRECISION NU,DLT90,ANG
DOUBLE PRECISION ANGTMP,ANGD2,SINANGD2,COSANG,DLT,DLTPNU,OMDLT,OMDLTPNU
DOUBLE PRECISION FCT1,TRM1,TRM2,DLT180,DLT180PNU,OMDLT180,OMDLT180PNU
...
DOUBLE PRECISION FUNCTION BWFUNC(WAVELENGTH_NANOMETER,TMPTR,SALINITY)
USE GLOBAL_DATA, only : DEPOL_O
USE RTUTILITY, ONLY : PI
IMPLICIT NONE

DOUBLE PRECISION WAVELENGTH_NANOMETER,TMPTR,SALINITY
DOUBLE PRECISION WAVELENGTH_NANOMETER2,WAVELENGTH_NANOMETER4,WAVELENGTH_NANOMETERM,&
...
SUBROUTINE CLOUDMODELSELECTION(WAVELENGTH_MICRON,           &
...
DOUBLE PRECISION,INTENT(IN)::WAVELENGTH_MICRON
DOUBLE PRECISION,DIMENSION(NUMMIEUSE),INTENT(OUT)::REFF1,VEFF1,REFF2, &
                                    VEFF2,MRR1,MRI1,MRR2,MRI2
DOUBLE PRECISION :: RADIUS_CLOUDS=10.0D0,VARIANCE_CLOUDS=0.1D0,&
          TMPTR=-20.0D0,SALINITY=0.0D0,WAVELENGTH_NANOMETER
DOUBLE PRECISION :: RTMP,RTMP1,RINDX_WATERLOCAL
WAVELENGTH_NANOMETER=WAVELENGTH_MICRON*1000.0d0
CALL WATER_REFRACTIVE_INDEX(WAVELENGTH_NANOMETER,TMPTR,SALINITY,RINDX_WATERLOCAL)
...
SUBROUTINE WATER_ABSORPTION_IOCCG_READ
USE GLOBAL_DATA
IMPLICIT NONE
INTEGER :: IREAD
DOUBLE PRECISION :: RTMP
...
DOUBLE PRECISION, INTENT(IN) :: LAMBDAEM
DOUBLE PRECISION,PARAMETER :: IPAR_WV_BEG=400.0D0,IPAR_WV_END=700.0D0
DOUBLE PRECISION, PARAMETER :: PLANK_CONSTANT_TIMES_C=1.98644568D-16  ! UNIT (J nm)
DOUBLE PRECISION, PARAMETER :: Avogadro_Constant=6.02214085774D23
DOUBLE PRECISION :: NPQ_ADJUST,PSII_RC_FRAC,QUANTUM_YIELD_CHLA_LOCAL,IPAR_AVG

DOUBLE PRECISION :: INTWEIGHT
...
INTEGER,EXTERNAL :: bisearch
DOUBLE PRECISION :: RTMP1,DY
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: SOURCEOTMP
DOUBLE PRECISION,DIMENSION(1) :: XI,YI

INTEGER :: IKPP,IWVEX,IULO,KLO,MPL
DOUBLE PRECISION :: Func_UVIP3P
MPL=2
...
DO IKPP=1,NKGRID
  ...
  IF(LAMBDAEX(IKPP)>LAMBDAEXFLRSC(1) .AND. LAMBDAEX(IKPP)<LAMBDAEXFLRSC(NFLRSCGRID))THEN
	IULO=bisearch(NFLRSCGRID,LAMBDAEXFLRSC,LAMBDAEX(IKPP))
	KLO=min(max(IULO-(MPL-1)/2,1),NFLRSCGRID-MPL)
	CALL UVIP3P(2,2,LAMBDAEXFLRSC(KLO),SOURCEOTMP(KLO),1,XI,YI)
	RTMP1=YI(1)
  ELSE
     RTMP1=0.0D0
  ENDIF
...
DOUBLE PRECISION,INTENT(IN) :: WAVELENGTH_MICRON,MU_IN

DOUBLE PRECISION :: Func_UVIP3P
INTEGER :: MAXM_LOCAL,ITAU,IQUAD,MVAR,I,ICOM,ILAYERW
DOUBLE PRECISION :: ESUN_EX

INTEGER :: IULO,MPL,KLO
DOUBLE PRECISION :: RTMP,RTMP1,RTMP2
DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE :: SOURCEOTMP
DOUBLE PRECISION,DIMENSION(3)::QSRC,ELCOEFF
DOUBLE PRECISION :: SQRTSIGN,TAUDIFF,TAUDIFF1
integer,external :: bisearch
DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: RARRAYTMP
DOUBLE PRECISION,DIMENSION(1) :: XI,YI
...
MAXM_LOCAL=MIN(MAXMORDINPUT_SAVE,2)
XI=WAVELENGTH_MICRON
MPL=2
KLO=bisearch(NSUNL,WVSUN,WAVELENGTH_MICRON)
CALL UVIP3P(1,2,WVSUN(KLO),FSUN(KLO),1,XI,YI)
ESUN_EX=YI(1)
...
DO ITAU=1,NUMTOTALTAUEX
    ...
    RTMP=Func_UVIP3P(NXJO_Inelas,XJO_Inelas,RARRAYTMP,XJO_MAIN(IQUAD),MPL)
...
DO MVAR=0,MAXM_LOCAL
DO ILAYERW=1,NWATER_DEPTH
  IULO=bisearch(NUMTOTALTAUEX,TAU_INELASTIC_EX,TAU_INELASTIC_AGD(ILAYERW))
...
DOUBLE PRECISION,INTENT(IN) :: WATER_DEPTH_Val,CHLaVal
DOUBLE PRECISION,INTENT(OUT) :: CHLaLOCAL

DOUBLE PRECISION :: ChlaZeu,ChlaZeuAvg,Zeu,ChlaD0,chla_eta,eta
...
!LOCINDXTMP=MINLOC(ABS(1038.31494140625d0-WV))
!write(*,*)'WV=',WV(LOCINDXTMP(1))
!write(*,*)'GAS_ABS_COEFF,GAS_ABS_COEFF_H2O,GAS_ABS_COEFF_CO2,GAS_ABS_COEFF_CH4,', &
!		  'GAS_ABS_COEFF_O2,GAS_ABS_COEFF_O3,GAS_ABS_COEFF_NO2=',               &
!        GAS_ABS_COEFF(LOCINDXTMP(1),npgrid),GAS_ABS_COEFF_H2O(LOCINDXTMP(1),npgrid),&
!		GAS_ABS_COEFF_CO2(LOCINDXTMP(1),npgrid),GAS_ABS_COEFF_CH4(LOCINDXTMP(1),npgrid),&
!		GAS_ABS_COEFF_O2(LOCINDXTMP(1),npgrid),GAS_ABS_COEFF_O3(LOCINDXTMP(1),npgrid),&
!        GAS_ABS_COEFF_NO2(LOCINDXTMP(1),npgrid)
...
!DOUBLE PRECISION,DIMENSION(1:NTLYERA+1),INTENT(IN)::ALT_LYRA
DOUBLE PRECISION,INTENT(IN) :: WAVELENGTH_NANOMETER
DOUBLE PRECISION,DIMENSION(NTLYERA),INTENT(OUT)::TAU_TG

INTEGER :: N_INT_SIZE
DOUBLE PRECISION :: Func_UVIP3P,ILS_INT_CONST,TRAPZOID
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XU,YU,ZU

DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: TAU_ABS_GRID,TAU_ABS_CUMSUM,TAU_TG_TEMP

INTEGER :: INTTMP,ILAYER,IWV_AVG,IWV_START,IWV_END

INTEGER,DIMENSION(1) ::LOCINDXTMP,LOCINDXTMP1
DOUBLE PRECISION :: RTMP,WV_TMP,GAS_ABS_TMP
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: RARRAYTMP
...
DO INTTMP=1,NTLYERA
   TAU_TG_TEMP(IWV_AVG,INTTMP)=Func_UVIP3P(NPGRID,Z_FIELD,ZU,ALT_LYRA(INTTMP+1),2)
ENDDO
...
!DOUBLE PRECISION,DIMENSION(1:NTLYERA+1),INTENT(IN)::ALT_LYRA
DOUBLE PRECISION,INTENT(IN) :: WAVELENGTH_NANOMETER
DOUBLE PRECISION,DIMENSION(NTLYERA),INTENT(OUT)::TAU_TG, TAU_TG_H2O,TAU_TG_CO2,TAU_TG_CH4, &
                                       TAU_TG_O2,TAU_TG_O3,TAU_TG_NO2

DOUBLE PRECISION :: Func_UVIP3P

DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE ::TAU_ABS_GRID,TAU_ABS_CUMSUM, TAU_ABS_H2O_CUMSUM, &
...
DO INTTMP=1,NTLYERA
   TAU_TG(INTTMP)=Func_UVIP3P(NPGRID,Z_FIELD,TAU_ABS_CUMSUM,ALT_LYRA(INTTMP+1),2)

   TAU_TG_H2O(INTTMP)=Func_UVIP3P(NPGRID,Z_FIELD,TAU_ABS_H2O_CUMSUM,ALT_LYRA(INTTMP+1),2)
   TAU_TG_CO2(INTTMP)=Func_UVIP3P(NPGRID,Z_FIELD,TAU_ABS_CO2_CUMSUM,ALT_LYRA(INTTMP+1),2)
   TAU_TG_CH4(INTTMP)=Func_UVIP3P(NPGRID,Z_FIELD,TAU_ABS_CH4_CUMSUM,ALT_LYRA(INTTMP+1),2)
   TAU_TG_O2(INTTMP) =Func_UVIP3P(NPGRID,Z_FIELD,TAU_ABS_O2_CUMSUM, ALT_LYRA(INTTMP+1),2)
   TAU_TG_O3(INTTMP) =Func_UVIP3P(NPGRID,Z_FIELD,TAU_ABS_O3_CUMSUM, ALT_LYRA(INTTMP+1),2)
   TAU_TG_NO2(INTTMP)=Func_UVIP3P(NPGRID,Z_FIELD,TAU_ABS_NO2_CUMSUM,ALT_LYRA(INTTMP+1),2)
ENDDO
...
!if(abs(WAVELENGTH_NANOMETER-1038d0)<1.0d-6)then
!write(*,*)'IWV_INDX,WV=',WV(IWV_INDX)
!write(*,*)'TAU_TG,TAU_TG_H2O,TAU_TG_CO2,TAU_TG_CH4,TAU_TG_O2,TAU_TG_O3,TAU_TG_NO2=',&
!          sum(TAU_TG),sum(TAU_TG_H2O),sum(TAU_TG_CO2),sum(TAU_TG_CH4),sum(TAU_TG_O2),&
!          sum(TAU_TG_O3),sum(TAU_TG_NO2)
!endif
...
USE MIE_PHMX_PACE
USE GLOBAL_DATA,ONLY : NUMMIEUSE,WV_PACE_REF,OCEAN_CASE_SELECT,                                   &
                PHYTOPLANKTON_INDEX_REFRACTION,PHYTOPLANKTON_SPECTRAL_SLOPE,&
                SEDIMENT_INDEX_REFRACTION,SEDIMENT_SPECTRAL_SLOPE,WV_SEG_FLAG
IMPLICIT NONE
INTEGER, INTENT(IN)::NWV_PHMX,IAEROSOL
DOUBLE PRECISION,DIMENSION(NUMMIEUSE),INTENT(OUT) :: REFF1_Save,REFF2_Save,VEFF1_Save,VEFF2_Save,&
                                           ARSLND1_Save,ARSLND2_Save
DOUBLE PRECISION,DIMENSION(NUMMIEUSE,NWV_PHMX),INTENT(OUT) ::MRR1_Save,MRI1_Save,MRR2_Save,MRI2_Save
DOUBLE PRECISION, INTENT(OUT) :: ARSLND_NonSpherical,REFF_NonSpherical,VEFF_NonSpherical
DOUBLE PRECISION, INTENT(OUT),DIMENSION(NWV_PHMX) :: MRR_NonSpherical,MRI_NonSpherical

DOUBLE PRECISION::AeroFMF,RH_Simu,REFF1_LOCAL,VEFF1_LOCAL,REFF2_LOCAL,VEFF2_LOCAL,ARSLND1_LOCAL,ARSLND2_LOCAL
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::MRR1_LOCAL,MRI1_LOCAL,MRR2_LOCAL,MRI2_LOCAL
...
CALL PHMXMIE_PACE_ALLO(NUMMIEUSE)
...
IF(IAEROSOL>=-1)THEN
	CALL PHMXMIE_PACE_INIT(FREEFORMFLAG,IAEROSOL,NUMMIEUSE,AeroFMF,RH_Simu,NWV_PHMX,REFF1_LOCAL,VEFF1_LOCAL,      &
			  REFF2_LOCAL,VEFF2_LOCAL,WV_PACE_REF,MRR1_LOCAL,MRI1_LOCAL,MRR2_LOCAL,MRI2_LOCAL, &
