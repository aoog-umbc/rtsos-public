  ...
  IF(OCEAN_FCDOM_FLAG .OR. OCEAN_FCHLA_FLAG .OR. OCEAN_RAMAN_FLAG)THEN
    ...
    DO IWVEX=1,NFLRSCGRID
       ...
       CALL RTSOSINIT(Indx_Inelastic_Scattering,IWVEX,WAVELENGTH_MICRON,&
          ...
          NUMMIERT,MAXLORDINPUT,NUMMIEANGINPUT,PHMXDATASTREAM,DELTAM_INPUT,LBDO_LD,FLAM,   &
...
FTRUNC(IPT)=FTRUNC_ARSL(IMIEUSE,IWV_BAND)*(1.0D0-TAURfracLOCAL)
...
DO LORD=0,MAXLORDINPUT
	...
	ELSE
		RTMP=0.0D0
	ENDIF
	BETAL(IPT,LORD) = RTMP*BETAL_Ray_Depol0p029(LORD) + &
			  (1.0D0-RTMP)*BETAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	ALPHAL(IPT,LORD) = RTMP*ALPHAL_Ray_Depol0p029(LORD) +&
				(1.0D0-RTMP)*ALPHAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	ZETAL(IPT,LORD) = RTMP*ZETAL_Ray_Depol0p029(LORD) +&
				(1.0D0-RTMP)*ZETAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	DELTAL(IPT,LORD) = RTMP*DELTAL_Ray_Depol0p029(LORD) +&
				(1.0D0-RTMP)*DELTAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	GAMMAL(IPT,LORD) = RTMP*GAMMAL_Ray_Depol0p029(LORD) +&
				(1.0D0-RTMP)*GAMMAL_ARSL(IMIEUSE,IWV_BAND,LORD)
	EPSILONL(IPT,LORD) = RTMP*EPSILONL_Ray_Depol0p029(LORD) +&
				(1.0D0-RTMP)*EPSILONL_ARSL(IMIEUSE,IWV_BAND,LORD)
ENDDO

IF(.NOT. SEC_RAD_CORR) RETURN

NUMORDFULLBETAL(IPT)=NUMBETAL(IMIEUSE)-1
MAXLORD_LOCAL=NUMORDFULLBETAL(IPT)
DO LORD=0,MAXLORD_LOCAL
	...
	FULLBETAL(IPT,LORD)=RTMP*BETAL_Ray_Depol0p029(LORD)+ &
			 (1.0D0-RTMP)*COEFFMIE_ARSL(IMIEUSE,IWV_BAND,LORD)
ENDDO
...
! DELTA FIT DIFINITION
integer :: NUMANG,numordfull_local,IANG,IPT,LORD
