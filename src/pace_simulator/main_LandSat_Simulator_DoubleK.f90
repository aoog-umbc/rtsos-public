...
REAL*8,PARAMETER :: WV_LandSat_REF=550.0D0
...
INTEGER :: OCEAN_CASE_SELECT
		  ...
		  ! ==-101 OCEAN SURFACE ONLY, WITH REMOTE SENSING REFLECTANCE SPECIFIED BY USER.
...
INTEGER :: WV_SEG_FLAG !wv_seg_flag, 0: all 9 bands; 1: Visible Only; 2: Visible+NIR;

INTEGER :: NWV_RSR0P
REAL*8,DIMENSION(:),ALLOCATABLE :: WV_RSR0P, RSR0P_ARR
...
!write(*,*)'Test FUNCALPHA', DK(1),DK(2),Dk(3),ALPHA_VAL,EXPOFAC,FUNCALPHA_VAL,DFUNCALPHA_VAL

END SUBROUTINE FUNCALPHA

PROGRAM SOSINT
USE GLOBAL_DATA
USE MIE_PHMX_LandSat
USE RAMANDATA
...
REAL*8, ALLOCATABLE,DIMENSION(:,:) :: TAU_ARSL_Ext_Save,TAU_ARSL_Scat_Save,TAU_RAY_Save,TAU_TG_Save, &
									TAU_TG_H2O_Save,TAU_TG_CO2_Save,TAU_TG_CH4_Save,TAU_TG_O2_Save,  &
									TAU_TG_O3_Save,TAU_TG_NO2_Save

REAL*8, ALLOCATABLE,DIMENSION(:) :: TAUR,TAU_TG,TAU_TG_H2O,TAU_TG_CO2,TAU_TG_CH4,TAU_TG_O2,TAU_TG_O3,&
...
INTEGER :: N_AVG_SIZE
REAL*8,DIMENSION(NILS_LandSat_VIRT) :: XK,YK
...
IF(NARGS.LT.1) THEN
    ...
    WRITE(*,*) ''
    WRITE(*,*) 'USEAGE:'
    WRITE(*,*) 'rtsos_LandSat_Simulator_DoubleK.exe input.file'
    WRITE(*,*) ''
    STOP
ENDIF
...
READ(1,*)CFILETMP          ! FILE NAME FOR SPECTRAL RSR0P
READ(1,*)OUTFILE
CLOSE(1)

IF(OCEAN_CASE_SELECT+101==0)THEN
  OPEN(unit=1,file=CFILETMP,status='old')
  READ(1,*)NWV_RSR0P
  ALLOCATE(WV_RSR0P(NWV_RSR0P), RSR0P_ARR(NWV_RSR0P))
  DO IWV=1,NWV_RSR0P
    READ(1,*)WV_RSR0P(IWV), RSR0P_ARR(IWV)
  ENDDO
  CLOSE(1)
ENDIF


IF(WV_SEG_FLAG<0 .OR. WV_SEG_FLAG>2) STOP 'CHECK WV_SEG_FLAG INPUT'
...
!CALL FILENAMEOUTPUT(CFILE1,ILS_FLAG,DIFFUSE_TRANSMITTANCE,WLR_FLAG_INPUT,WNDSPD,&
!                    MU_IN,CHLa,OCEAN_CASE_SELECT,SEDIMENT_CONCENTRATION, &
!                    Dpol90_Kok,TAU550,OCEAN_FCHLA_FLAG,NPQ_FLAG,          &
...
IF(OCEAN_CASE_SELECT==0)THEN
  ...
  OCEAN_FCDOM_FLAG=.FALSE.
  HYSPECTRAL_FLAG=.false.
ELSEIF(OCEAN_CASE_SELECT+101==0)THEN
	NTLYERO=0
	OCEAN_PHMX_ONE=.TRUE.
	OCEAN_RAMAN_FLAG=.false.
	OCEAN_FCHLA_FLAG=.false.
...
IF(OCEAN_PHMX_ONE .AND. NTLYERO>0) NUMMIERT=NTLYERA+1

!write(*,*)'NREC,NUMMIERT=',NREC,NUMMIERT

ALLOCATE(TAUR(NTLYERA), TAU_TG(NTLYERA),TAU_TG_H2O(NTLYERA),TAU_TG_CO2(NTLYERA), &
...
ALLOCATE(TAU_ARSL_Ext_Save(NWV,NTLYERA),TAU_ARSL_Scat_Save(NWV,NTLYERA), &
         ...
         TAU_TG_H2O_Save(NWV,NTLYERA),TAU_TG_CO2_Save(NWV,NTLYERA),      &
...
MAXLORD_Inelas=NQUADA_Inelas
  ...
  IF(OCEAN_FCDOM_FLAG .OR. OCEAN_FCHLA_FLAG .OR. OCEAN_RAMAN_FLAG)THEN
    ...
    DO IWVEX=1,NFLRSCGRID
       ...
       CALL TAURCAL(WAVELENGTH_MICRON,NTLYERA,TAUR)
...
ELSEIF(WV_SEG_FLAG==1)THEN
	NWV_BAND_START=1
	NWV_BAND_END=NWV_BAND_SEG1
ELSEIF(WV_SEG_FLAG==2)THEN
	NWV_BAND_START=1
	NWV_BAND_END=NWV_BAND_SEG1+1
ENDIF

DO IWV_BAND=NWV_BAND_START,NWV_BAND_END

  XK=ILS_DeltaWaveLength(IWV_BAND,:)
  YK=ILS_LandSat(IWV_BAND,:)

  IF(IWV_BAND <= NWV_BAND_SEG1)THEN
     RTMP=0.5D0
     NILS_TEMP=NILS_LandSat_VIRT
  ELSE
     RTMP=1.0D0
     NILS_TEMP=NILS_LandSat(IWV_BAND-NWV_BAND_SEG1)
  ENDIF

  LOCINDXTMP= MINLOC(ABS(WV_BAND(IWV_BAND)-RTMP*LandSat_FWHM(IWV_BAND)-WV))
  LOCINDXTMP1=MINLOC(ABS(WV_BAND(IWV_BAND)+RTMP*LandSat_FWHM(IWV_BAND)-WV))

  IWV_SUB_START=LOCINDXTMP(1)
  IWV_SUB_END  =LOCINDXTMP1(1)

  WRITE(*,*)'WV_BAND(IWV_BAND),IWV_BAND,IWV_SUB_START,IWV_SUB_END=', &
             WV_BAND(IWV_BAND),IWV_BAND,IWV_SUB_START,IWV_SUB_END
...
! PAY ATTENTION TO IWV_SUB_START AND IWV_SUB_END LATER, MAKE SURE COVERS THE WHOLE SPECTRUM.
  ...
  CALL TAURCAL(WAVELENGTH_MICRON,NTLYERA,TAUR)
  INDEX_TAUPRIME=INDEX_PRIME_SCATTERING(NTLYERA,TAUR,PNDLY,IWV_BAND)
...
!  WRITE(*,*)'MINVAL(TAU_TG_DKP),INDEX_TAUPRIME,NTLYERA=',MINVAL(TAU_TG_DKP),INDEX_TAUPRIME,NTLYERA
  ...
  DO ILS_AVG=1,N_AVG_SIZE
      YU(ILS_AVG)=POLINT_SIMPLE(NILS_TEMP,XK,YK,XU(ILS_AVG),2)
  ...
  DO IWV_CAL=1,NWV_SUB_CAL
        ...
        CALL RTSOSINIT(Indx_Inelastic_Scattering,INDX_WV_CAL(IWV_CAL),&
           WAVELENGTH_MICRON,NREC,NTLYERA,NTLYERO,PNDLY,TAUR,TAU_TG,       &
...
!        ENDIF
     ...
     DO IPHI=1,NPHIOUT
	    ...
	    CALL DOUBLEKPARA_EVALUATION(NWV_SUB_CAL,TAU_TG_MAX,ErrFLAG)
!testing
!        IF(ErrFLAG .and. idet==1 .and. itheta==1 .and. iphi==1)THEN
!            WRITE(*,*)'dktest',icom,DK(1:NWV_SUB_CAL),IMS(1:NWV_SUB_CAL)
...
CALL STOKESOUT(OUTFILE,MU_IN,NTHETAOUT,NPHIOUT,PHIOUT,MUOUT,             &
        DIRADFULL,DSTOKESFULL,TAU_ARSL_Ext_Save,TAU_ARSL_Scat_Save,  &
        TAU_RAY_Save,TAU_TG_Save,TAU_TG_H2O_Save,TAU_TG_CO2_Save,         &
...
IF(OCEAN_CASE_SELECT+101==0) DEALLOCATE(WV_RSR0P, RSR0P_ARR)
CLOSE(55)
...
USE FLUORESCENCEDATA
USE MIE_PHMX_LandSat,ONLY : BBFRAC_Plankton_BAND,BBFRAC_Mineral_BAND
USE HDF5
USE ISO_C_BINDING
...
REAL*8,DIMENSION(NWV,NTLYERA) ::TAU_ARSL_Ext_Save,TAU_ARSL_Scat_Save,TAU_RAY_Save, &
								TAU_TG_Save,TAU_TG_H2O_Save,TAU_TG_CO2_Save,      &
								TAU_TG_CH4_Save,TAU_TG_O2_Save,TAU_TG_O3_Save,    &
								TAU_TG_NO2_Save
...
IF(ILS_FLAG)THEN
  ...
  DO IWV_AVG=NWV_START,NWV_END
     ...
     YK=ILS_LandSat(IWV_AVG,:)
     WAVELENGTH_BAND_LO=WV_BAND(IWV_AVG)-LandSat_FWHM(IWV_AVG)
     WAVELENGTH_BAND_HI=WV_BAND(IWV_AVG)+LandSat_FWHM(IWV_AVG)
     ...
     DO ILS_AVG=1,N_AVG_SIZE
        YU(ILS_AVG)=POLINT_SIMPLE(NILS_LandSat(IWV_AVG-NWV_BAND_SEG1),XK,YK,XU(ILS_AVG),MPL)
        IF(YU(ILS_AVG)<0.0D0)YU(ILS_AVG)=0.0D0
     ENDDO
     ILS_INT_CONST=TRAPZOID(N_AVG_SIZE,XU,YU)
     DO ILAYER=1,NTLYERA
...
HDF5RTMP=REFF1_Save(IMIE)!*EXP(-2.5*RTMP) !output effective radius
f_ptr=C_LOC(HDF5RTMP(1))
CALL h5dwrite_f(dset,H5T_NATIVE_REAL,f_ptr, hdferr)
CALL h5dclose_f(dset , hdferr)
CALL h5sclose_f(space, hdferr)
...
!RTMP=LOG(VEFF1_Save(IMIE)+1.0)
!HDF5RTMP=SQRT(RTMP)
HDF5RTMP=VEFF1_Save(IMIE) !output effective variance
f_ptr=C_LOC(HDF5RTMP(1))
CALL h5dwrite_f(dset, H5T_NATIVE_REAL,f_ptr, hdferr)
CALL h5dclose_f(dset , hdferr)
CALL h5sclose_f(space, hdferr)
...
HDF5RTMP=REFF2_Save(IMIE)!*EXP(-2.5*RTMP) !output effective radius
f_ptr=C_LOC(HDF5RTMP(1))
CALL h5dwrite_f(dset, H5T_NATIVE_REAL,f_ptr, hdferr)
CALL h5dclose_f(dset , hdferr)
CALL h5sclose_f(space, hdferr)
...
!RTMP=LOG(VEFF2_Save(IMIE)+1.0)
!HDF5RTMP=SQRT(RTMP)
HDF5RTMP=VEFF2_Save(IMIE) !output effective variance
f_ptr=C_LOC(HDF5RTMP(1))
CALL h5dwrite_f(dset, H5T_NATIVE_REAL,f_ptr, hdferr)
CALL h5dclose_f(dset , hdferr)
CALL h5sclose_f(space, hdferr)
...
IF(OCEAN_CASE_SELECT>=0)THEN ! OCEAN PARAMETERS OUTPUT
dimscl=(/ NWV_OUTPUT /)
ALLOCATE(HDF5RARR(NWV_OUTPUT))
IF(ILS_FLAG)THEN
...
THETAOUT_FREE=(/0.0d0,5.0d0,10.0d0/)
PHIOUT=(/0.0d0, 40.0d0, 80.0d0, 120.0d0, 160.0d0/)
...
INTEGER FUNCTION INDEX_PRIME_SCATTERING(NTLYERA,TAUR,PNDLY,IWV_BAND)
USE MIE_PHMX_LandSat, ONLY : CSCAT_ARSL_BAND
USE GLOBAL_DATA, ONLY : CEXT_REF,TAU550
IMPLICIT NONE

INTEGER,INTENT(IN) :: NTLYERA,IWV_BAND
...
USE MIE_PHMX_LandSat, ONLY : NUM_SCAT_ANG,CEXT_ARSL_BAND,CSCAT_ARSL_BAND,&
                          PHMXMIE_ARSL_LandSat,REFF_MINERAL,AREA_MINERAL,&
                          CSCAT_Mineral_BAND,PHMXMIE_Plankton_LandSat,PHMXMIE_Mineral_LandSat
IMPLICIT none
...
REAL*8 :: RTMP,RTMP1,AEROSOL_ABSORB_FAC,AEROSOL_ALBEDO,OCEAN_ALBEDO, &
        ...
        finemoderatio,LAYER_DEPTH_TMP,OCEAN_TAU_TMP,CSCAT_MINERAL,RSR0P_LOCAL
...
IMIE=1
DO ITLYERA=1,NTLYERA
   TAULYR(ITLYERA)=PNDLY(ITLYERA)*TAU550*CEXT_ARSL_BAND(IWV_BAND)/CEXT_REF
...
! total optical depth=aerosol+rayleigh+gas absorption
  ...
  PHMXDATASTREAM(ITLYERA,1:NUMMIEANGMAX,0)=PHMXMIE_ARSL_LandSat(IWV_BAND,1:NUMMIEANGMAX,0)
  ...
  ELSE
    PHMXDATASTREAM(ITLYERA,1:NUMMIEANGMAX,1:6)=PHMXMIE_ARSL_LandSat(IWV_BAND,1:NUMMIEANGMAX,1:6)
...
! OCEAN INTERFACE
    ...
    IF(OCEAN_CASE_SELECT+101==0)THEN
       RECDATASTREAM(NTLYERA+IREC_SHFT,1)=OCEAN_CASE_SELECT
       IF(WAVELENGTH_NANOMETER<minval(WV_RSR0P) .or. WAVELENGTH_NANOMETER>maxval(WV_RSR0P))THEN
		  RSR0P_LOCAL=0.0d0
       ELSE
          RSR0P_LOCAL=POLINT_SIMPLE(NWV_RSR0P,WV_RSR0P,RSR0P_ARR,WAVELENGTH_NANOMETER,2)
       ENDIF
       RECDATASTREAM(NTLYERA+IREC_SHFT,6)=RSR0P_LOCAL
    ELSE
...
!RECDATASTREAM(NTLYERA+4+ITLYERW,2)
  ...
  IF(ABSORPTION_TAU_REDUCE_FLAG)THEN
    ...
    IF(OCEAN_TAU_TMP<0.0D0)THEN
         ...
         STOP 'MAIN_LandSat_Simulator.F90 CHECK OCEAN_TAU_TMP'
    ENDIF
    IF(ITLYERW<=NTLYERO) &
  ...
  IF(OCEAN_CASE_SELECT==0)THEN
     ...
     PHMXDATASTREAM(NTLYERA+1,1:NUMMIEANGMAX,0)=PHMXMIE_ARSL_LandSat(IWV_BAND,1:NUMMIEANGMAX,0)
  ...
  ELSEIF(OCEAN_CASE_SELECT==1 .or. OCEAN_CASE_SELECT==3)THEN
     ...
     PHMXDATASTREAM(NTLYERA+ITLYERW,1:NUMMIEANGMAX,0)=PHMXMIE_ARSL_LandSat(IWV_BAND,1:NUMMIEANGMAX,0)
  ...
  ELSE
     PHMXDATASTREAM(NTLYERA+ITLYERW,1:NUMMIEANGMAX,0)=PHMXMIE_Plankton_LandSat(IWV_BAND,1:NUMMIEANGMAX,0)
     finemoderatio=BPTCLOCAL/(BPTCLOCAL+BSTCLOCAL)
     PHMXDATASTREAM(NTLYERA+ITLYERW,1:NUMMIEANGMAX,1:6)=&
                   (1.0d0-finemoderatio)*PHMXMIE_Mineral_LandSat(IWV_BAND,1:NUMMIEANGMAX,1:6)&
                         +finemoderatio *PHMXMIE_Plankton_LandSat(IWV_BAND,1:NUMMIEANGMAX,1:6)
...
IF(ABSORPTION_TAU_REDUCE_FLAG)THEN
  RTMP=TAU_REDUCE_ATMOS_PRESERVE(NTLYERA+1)
  DO ITLYERW=1,NTLYERO+1
    RTMP=RTMP+TAU_REDUCE_OCEAN_PRESERVE(ITLYERW)
    TAU_REDUCE_OCEAN_PRESERVE(ITLYERW)=RTMP
...
SUBROUTINE TAURCAL(WVLENGTH,NTLYERA,TAUR)
USE GLOBAL_DATA, ONLY : ATMOS_ZERO,ALT_LYRA
USE Atmosphere_Profile, ONLY : NPGRID,Z_FIELD,NUMDEN_TOTAL_FIELD

IMPLICIT NONE
REAL*8,INTENT(IN) :: WVLENGTH
INTEGER,INTENT(IN) ::NTLYERA
!REAL*8,DIMENSION(1:NTLYERA+1),INTENT(IN) ::ALT_LYRA
REAL*8,DIMENSION(NTLYERA),INTENT(OUT) :: TAUR

REAL*8,DIMENSION(:),ALLOCATABLE :: ZU
REAL*8,DIMENSION(:),ALLOCATABLE :: TAUR_GRID,TAUR_CUMSUM

INTEGER:: ILYERA,IHEIGHT
REAL*8 :: RAY_SCALE_HEIGHT_SUB,MOLEXT
...
ALLOCATE(ZU(NPGRID))
ALLOCATE(TAUR_GRID(NPGRID),TAUR_CUMSUM(NPGRID))

! INITIALIZE TAU_TG VALUES
TAUR=0.0D0
...
DO IHEIGHT=2,NPGRID
	...
	TAUR_GRID(IHEIGHT)=RAY_SCALE_HEIGHT_SUB*MOLEXT* &
...
DO IHEIGHT=1,NTLYERA
   TAUR(IHEIGHT)=POLINT_SIMPLE(NPGRID,Z_FIELD,ZU,ALT_LYRA(IHEIGHT+1),3)
ENDDO
...
USE GLOBAL_DATA, ONLY : WV_LandSat_REF,CEXT_REF,ALT_LYRA
USE LandSat_INSTRUMENT_DESIGN,ONLY : NWV_BAND,WV_BAND
USE MIE_PHMX_LandSat, ONLY : CEXT_ARSL_BAND
IMPLICIT NONE
...
CEXT_REF=CEXT_ARSL_BAND(NWV_BAND+1)
...
SUBROUTINE NAP_PARA_INIT(WAVELENGTH_MICRON,SEDIMENT_CONCENTRATION, &
                         SEDIMENT_SPECTRAL_SLOPE,ASTCLOCAL,BSTCLOCAL)
USE LandSat_INSTRUMENT_DESIGN,ONLY : NWV_BAND,WV_BAND
USE MIE_PHMX_LandSat, ONLY : AREA_MINERAL,REFF_MINERAL,CSCAT_Mineral_BAND
...
IF(WAVELENGTH_NANOMETER<350.0D0 .OR. WAVELENGTH_NANOMETER> 550.0D0)THEN
...
SUBROUTINE TAU_TG_INIT(GAS_ABS_FLAG,NTLYERA,WAVELENGTH_NANOMETER,TAU_TG,TAU_TG_H2O, &
						TAU_TG_CO2,TAU_TG_CH4,TAU_TG_O2,TAU_TG_O3,TAU_TG_NO2)
USE GLOBAL_DATA, ONLY :NWV,WV,GAS_ABS_COEFF,ALT_LYRA,          &
...
SUBROUTINE Particle_PHMX_Assign(NUMMIEUSE,NWV_PHMX,IAEROSOL,IRH,REFF1_Save,    &
                 ...
                 MRI2_Save,ARSLND1_Save,ARSLND2_Save)
USE MIE_PHMX_LandSat
USE GLOBAL_DATA,ONLY : WV_LandSat_REF,OCEAN_CASE_SELECT,                                   &
...
ALLOCATE(MRR1_LOCAL(NWV_PHMX+1),MRI1_LOCAL(NWV_PHMX+1),MRR2_LOCAL(NWV_PHMX+1),MRI2_LOCAL(NWV_PHMX+1))
...
CALL PHMXMIE_LandSat_INIT(FREEFORMFLAG,IAEROSOL,IRH,REFF1_LOCAL,VEFF1_LOCAL,      &
          REFF2_LOCAL,VEFF2_LOCAL,WV_LandSat_REF,MRR1_LOCAL,MRI1_LOCAL,MRR2_LOCAL,MRI2_LOCAL, &
...
MRR1_Save(1,1:NWV_PHMX)=MRR1_LOCAL(1:NWV_PHMX)
MRI1_Save(1,1:NWV_PHMX)=MRI1_LOCAL(1:NWV_PHMX)
MRR2_Save(1,1:NWV_PHMX)=MRR2_LOCAL(1:NWV_PHMX)
MRI2_Save(1,1:NWV_PHMX)=MRI2_LOCAL(1:NWV_PHMX)
...
!  WRITE(CFILETMP,'(A16,I1,A4)')'LandSat_AC_wlr_Case',OCEAN_CASE_SELECT,'_WND'
!ELSE
!  WRITE(CFILETMP,'(A16,I1,A4)')'LandSat_AC_ttr_Case',OCEAN_CASE_SELECT,'_WND'
!ENDIF
