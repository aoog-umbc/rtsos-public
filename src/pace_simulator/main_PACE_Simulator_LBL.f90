...
LOGICAL :: SEG1ONLY,ILS_FLAG,DIFFUSE_TRANSMITTANCE
INTEGER :: NDET
...
INTEGER :: NREC,NCOLINPUT,NQUADAINPUT,NQUADOINPUT, &
           NUMMIEUSE,NUMMIERT,MAXLORDINPUT,MAXMORDINPUT,NWV_START,NWV_END
...
IF(file_e)CYCLE
  ...
  IF(OCEAN_FCDOM_FLAG .OR. OCEAN_FCHLA_FLAG .OR. OCEAN_RAMAN_FLAG)THEN
    DO IWVEX=1,NFLRSCGRID
       ...
       IF(GAS_ABS_FLAG)THEN
          CALL TAU_TG_INIT_INELASTIC(NTLYERA,ALT_LYRA,WAVELENGTH_NANOMETER,TAU_TG)
       ELSE
          TAU_TG=0.0D0
       ENDIF
...
DO IWV=NWV_START, NWV_END
  LAMBDAEM=WV(IWV)
  WAVELENGTH_NANOMETER=WV(IWV)
  WAVELENGTH_MICRON=WV(IWV)/1000.0D0
  ...
  IF(GAS_ABS_FLAG)THEN
    CALL TAU_TG_INIT(NTLYERA,ALT_LYRA,WAVELENGTH_NANOMETER,TAU_TG)
  ELSE
    TAU_TG=0.0D0
  ENDIF
  CALL TAURCAL(WAVELENGTH_MICRON,NTLYERA,ALT_LYRA,TAUR)
...
start_time = time_array_0 (5) * 3600 + time_array_0 (6) * 60 &
    ...
    CALL RTSOSINIT(IWNDSPD,ICHLA,Indx_Inelastic_Scattering,IWV,IOPT,WAVELENGTH_MICRON,&
       NREC,NTLYERA,NTLYERO,PNDLY,TAUR,TAU_TG,RECDATASTREAM,NTLYER,NUMMIEUSE,    &
       NUMMIERT,MAXLORDINPUT,NUMMIEANGINPUT,PHMXDATASTREAM,LBDO_LD,FLAM,   &
       TEMPERTURE,SALINITY)
...
!write(1,*)NREC,NDET,NCOLINPUT,NQUADAINPUT,NQUADOINPUT, &
!DELTATAUAINPUT,DELTATAUOINPUT,NUMMIERT,MAXLORDINPUT,MAXMORDINPUT,    &
!NUMMIEANGINPUT,MU_IN,NTHETAOUT,&
!NPHIOUT,WLR_FLAG_INPUT,WCFLG_INPUT,DELTAM_INPUT
...
!ENDDO
!DO IMIE=1,NTLYERA+1
! WRITE(1,*)'PHMXDATASTREAM',IMIE
! DO ITHETA=1,NUMMIEANGINPUT(IMIE),20
...
!testing
!write(*,*)'LBL WAVELENGTH_MICRON,RTMP1,DSTOKES(1,1,1,1)=',WAVELENGTH_MICRON,RTMP1,DSTOKES(1,1,1,1)
!IF(RTMP1<0.0)THEN
!  WRITE(*,*)WAVELENGTH_MICRON,RTMP,DIRAD(1,1)
!  STOP 'CHECK FSUN LOOKUP'
!ENDIF
IF(ABSORPTION_TAU_REDUCE_FLAG)THEN
...
REAL*8 :: WAVELENGTH_BAND_LO,WAVELENGTH_BAND_HI
...
!REAL*8,DIMENSION(NWV_BAND,NTLYERA) :: TAU_TG_AVG
     ...
     ZU=ZU*YU
     BBPTCfrac_AVG(IWV_AVG)=TRAPZOID(N_AVG_SIZE,XU,ZU)
     BBPTCfrac_AVG(IWV_AVG)=BBPTCfrac_AVG(IWV_AVG)/ILS_INT_CONST
...
!WRITE(*,*)'TOTAL RAYLEIGH TAU=',SUM(TAUR)
...
SUBROUTINE TAU_TG_INIT_INELASTIC(NTLYERA,ALT_LYRA,WAVELENGTH_NANOMETER,TAU_TG)
...
SUBROUTINE TAU_TG_INIT(NTLYERA,ALT_LYRA,WAVELENGTH_NANOMETER,TAU_TG)
USE GLOBAL_DATA, ONLY :NWV,WV,NPGRID,Z_FIELD,GAS_ABS_COEFF
...
IF(ILS_FLAG)THEN
  CFILE1=TRIM(CFILE1)//'wILS_LBL.h5'
ELSE
  CFILE1=TRIM(CFILE1)//'noILS_LBL.h5'
ENDIF
