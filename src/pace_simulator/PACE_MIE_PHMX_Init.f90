...
DOUBLE PRECISION :: REFF_MINERAL,AREA_MINERAL,WAVELENGTH_MICRON_REF

DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE:: CEXT_Plankton_BAND,CSCAT_Plankton_BAND,BBFRAC_Plankton_BAND, &
				 CEXT_Mineral_BAND,CSCAT_Mineral_BAND,BBFRAC_Mineral_BAND

DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE :: PHMXMIE_Plankton_PACE,PHMXMIE_Mineral_PACE

DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE:: CEXT_ARSL_BAND,CSCAT_ARSL_BAND
DOUBLE PRECISION,DIMENSION(:,:,:,:),ALLOCATABLE::PHMXMIE_ARSL
DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE::COEFFMIE_ARSL
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE::FTRUNC_ARSL
DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE:: BETAL_ARSL,ALPHAL_ARSL,ZETAL_ARSL, &
								 DELTAL_ARSL,GAMMAL_ARSL,EPSILONL_ARSL


CONTAINS

SUBROUTINE PHMXMIE_PACE_ALLO(NUMMIEUSE,OCEAN_CASE_SELECT)
implicit none
INTEGER,INTENT(IN):: NUMMIEUSE,OCEAN_CASE_SELECT

IF(OCEAN_CASE_SELECT==2)THEN
! NUM_SCAT_ANG(NUMMIEUSE+1) Plankton
! NUM_SCAT_ANG(NUMMIEUSE+2) Mineral
	ALLOCATE(NUM_SCAT_ANG(NUMMIEUSE+2),NUMBETAL(NUMMIEUSE+2))
ELSE
...
DOUBLE PRECISION, INTENT(INOUT) :: ARSLND_NonSpherical,REFF_NonSpherical,VEFF_NonSpherical
DOUBLE PRECISION, INTENT(INOUT),DIMENSION(:) :: MRR_NonSpherical,MRI_NonSpherical
! HDF 5 DEFINITION
...
INTEGER :: IMIE,INDXVAR,ILTERM,itheta,NANGMIE_LOCAL,NWV_START,NWV_END
...
IF(WV_SEG_FLAG /= 4)THEN ! READ IN OCI AEROSOL SINGLE SCATTERING INFO
...
IF(ABS(HDF5RARR(NWV_BAND_SEG1P2)-WV_BAND(NWV_BAND_SEG1P2))>1.0E-4)THEN
   WRITE(*,*)'OCI WAVELENGTH FROM AEROSOL FILE MISMATCH WITH THOSE OF THE WAVELENGTH SETTING'
   WRITE(*,*)'NWV_BAND_SEG1P2=',NWV_BAND_SEG1P2
   ...
   WRITE(*,*) 'STOPPING AT WaveLength_OCI read in PHMXMIE_PACE_READIN '
   STOP
ENDIF
...
ENDIF ! END READ IN OCI AEROSOL SINGLE SCATTERING INFO

IF(WV_SEG_FLAG==0 .or. WV_SEG_FLAG==1 .or. WV_SEG_FLAG==3)THEN ! READ IN SPEXone AEROSOL SINGLE SCATTERING INFO
...
IF(ABS(HDF5RARR(NWV_BAND_SEG3)-WV_BAND(NWV_BAND_SEG1P2+NWV_BAND_SEG3))>1.0E-4)THEN
   WRITE(*,*)'SPEX WAVELENGTH FROM AEROSOL FILE MISMATCH WITH THOSE OF THE WAVELENGTH SETTING'
   WRITE(*,*)'NWV_BAND_SEG3=',NWV_BAND_SEG3
   ...
   WRITE(*,*)'STOPPING AT WaveLength_SPEX READIN IN PHMXMIE_PACE_READIN'
   STOP
ENDIF
...
ENDIF !END READ IN SPEXone AEROSOL SINGLE SCATTERING INFO


IF(WV_SEG_FLAG==0 .or. WV_SEG_FLAG==4)THEN ! READ IN HARP2 AEROSOL SINGLE SCATTERING INFO
...
RTMP=ABS(HDF5RARR(NWV_BAND_SEG4)-WV_BAND(NWV_BAND_SEG1P2+NWV_BAND_SEG3+NWV_BAND_SEG4)) / &
         WV_BAND(NWV_BAND_SEG1P2+NWV_BAND_SEG3+NWV_BAND_SEG4)
IF(RTMP>0.005D0)THEN
   WRITE(*,*)'HARP2 WAVELENGTH FROM AEROSOL FILE MISMATCH WITH THOSE OF THE WAVELENGTH SETTING'
   WRITE(*,*)'WAVELENGTH READ IN FROM AEROSOL FILE IS',HDF5RARR(NWV_BAND_SEG4)
   WRITE(*,*)'NWV_BAND_SEG1P2+NWV_BAND_SEG3+NWV_BAND_SEG4=',NWV_BAND_SEG1P2+NWV_BAND_SEG3+NWV_BAND_SEG4
   WRITE(*,*)'WV_BAND(NWV_BAND_SEG1P2+NWV_BAND_SEG3+NWV_SEG4)=',WV_BAND(NWV_BAND_SEG1P2+NWV_BAND_SEG3+NWV_BAND_SEG4)
   WRITE(*,*)'STOPPING AT WaveLength_HARP READIN IN PHMXMIE_PACE_READIN'
   STOP
ENDIF
...
ENDIF ! END READ IN HARP2 AEROSOL SINGLE SCATTERING INFO
...
IF(PHMXMIE_ARSL(IMIE,1,1,0)>1.0E-4) STOP 'SCATTERING ANGLE (1) IS NOT ZERO'
IF(ABS(PHMXMIE_ARSL(IMIE,1,NANGMIE_LOCAL,0)-180.0D0)>1.0E-4) STOP 'LAST SCATTERING ANGLE IS NOT 180'

IF(WV_SEG_FLAG /= 4)THEN ! READ IN OCI PHASE MATRIX
...
IF(WV_SEG_FLAG==0 .or. WV_SEG_FLAG==1 .or. WV_SEG_FLAG==3)THEN ! READ IN SPEXone PHASE MATRIX
...
IF(WV_SEG_FLAG==0 .or. WV_SEG_FLAG==4)THEN ! READ IN HARP2 PHASE MATRIX
...
INTEGER,INTENT(IN) :: IAEROSOL,NWV_PHMX,NUMMIEUSE,WV_SEG_FLAG
DOUBLE PRECISION,INTENT(IN) :: WV_PACE_REF,RH_Simu
DOUBLE PRECISION,INTENT(INOUT) :: AeroFMF
DOUBLE PRECISION,INTENT(INOUT)::REFF1,VEFF1,REFF2,VEFF2,ARSLND1,ARSLND2
DOUBLE PRECISION,DIMENSION(NWV_PHMX),INTENT(INOUT)::MRR1,MRI1,MRR2,MRI2

DOUBLE PRECISION,DIMENSION(NWV_PHMX)::WV_BAND_MICRON
DOUBLE PRECISION,DIMENSION(NUMMIEANGMAX,0:6)::PHMXMIE_LOCAL1,PHMXMIE_LOCAL2
DOUBLE PRECISION,DIMENSION(0:MAXLFULL,1:6)::COEFFMIE_LOCAL1,COEFFMIE_LOCAL2

DOUBLE PRECISION, INTENT(IN) :: PHYTOPLANKTON_INDEX_REFRACTION,PHYTOPLANKTON_SPECTRAL_SLOPE,&
                      SEDIMENT_INDEX_REFRACTION,SEDIMENT_SPECTRAL_SLOPE

INTEGER :: NDISTR,NANGMIE_LOCAL,NUMBETAL_LOCAL,ICOM,IRH
DOUBLE PRECISION :: AA1,BB1,AA2,BB2,GAMMAI,REFFO,VEFFO,AREA,CEXT1,CEXT2,CSCAT1,CSCAT2

DOUBLE PRECISION::REFF1_LOCAL,VEFF1_LOCAL,REFF2_LOCAL,VEFF2_LOCAL,ARSLND1_LOCAL,ARSLND2_LOCAL

DOUBLE PRECISION :: RTMP,RTMP1,BBFRAC_LOCAL
INTEGER,DIMENSION(1):: INTARR
INTEGER :: IWV_BAND,INTTMP,IMIE,NWV_START,NWV_END

DOUBLE PRECISION :: Func_UVIP3P
DOUBLE PRECISION,DIMENSION(NRH) :: XARRY,YARRY
...
!       REFF2=exp(log(RZIAC(IRH))-3.0*SIGC_ZIA*SIGC_ZIA)
		RTMP=Func_UVIP3P(NRH,RH,RZIAF,RH_simu,3)
		REFF1=exp(log(RTMP)-3.0*SIGF_ZIA*SIGF_ZIA)
		! Rg in number space read Eq.3a in Ahmad 2000 for details
		RTMP=Func_UVIP3P(NRH,RH,RZIAC,RH_simu,3)
...
!       MRI2(1:NWV_PHMX)=MI_ZIAC(1:NWV_PHMX,IRH)
		...
		DO IWV_BAND=1,NWV_PHMX
			...
			MRR1(IWV_BAND)=Func_UVIP3P(NRH,XARRY,YARRY,RH_simu,2)
			YARRY=MI_ZIAF(IWV_BAND,:)
			MRI1(IWV_BAND)=Func_UVIP3P(NRH,XARRY,YARRY,RH_simu,2)
			YARRY=MR_ZIAC(IWV_BAND,:)
			MRR2(IWV_BAND)=Func_UVIP3P(NRH,XARRY,YARRY,RH_simu,2)
			YARRY=MI_ZIAC(IWV_BAND,:)
			MRI2(IWV_BAND)=Func_UVIP3P(NRH,XARRY,YARRY,RH_simu,2)
		ENDDO
    ENDIF
...
DO IWV_BAND=NWV_START,NWV_END
  write(*,*)'PACE Mie Initialization, FREEFORMFLAG, IWV_BAND',FREEFORMFLAG, IWV_BAND
  ...
  IF(FREEFORMFLAG<=1) NUMBETAL(1:NUMMIEUSE)=NUMBETAL_LOCAL
  ...
  IF(FREEFORMFLAG<=1)THEN
	  ...
	  DO IMIE=1,NUMMIEUSE
        ...
        COEFFMIE_ARSL(IMIE,IWV_BAND,0:NUMBETAL(IMIE)-1)= &
				   (ARSLND1*CSCAT1*COEFFMIE_LOCAL1(1:NUMBETAL(IMIE),1) + &
					ARSLND2*CSCAT2*COEFFMIE_LOCAL2(1:NUMBETAL(IMIE),1))/ &
					CSCAT_ARSL_BAND(IMIE,IWV_BAND)
  ...
  ELSEIF(FREEFORMFLAG==2)THEN
	  NUM_SCAT_ANG(NUMMIEUSE+1)=NANGMIE_LOCAL
      NUMBETAL(NUMMIEUSE+1)=NUMBETAL_LOCAL
  ...
  ELSEIF(FREEFORMFLAG==3)THEN
      NUM_SCAT_ANG(NUMMIEUSE+2)=NANGMIE_LOCAL
      NUMBETAL(NUMMIEUSE+2)=NUMBETAL_LOCAL
	  REFF_MINERAL=REFFO
	  AREA_MINERAL=AREA
...
DOUBLE PRECISION,DIMENSION(NUMMIEANGMAX,0:6),INTENT(IN)::PHMXMIE_LOCAL1
DOUBLE PRECISION,INTENT(OUT) :: BBFRAC_LOCAL

INTEGER,DIMENSION(1) ::LOCINDXTMP
INTEGER :: NSIZE
DOUBLE PRECISION, DIMENSION(:),ALLOCATABLE :: XARRAY,YARRAY
DOUBLE PRECISION :: RTMP,RTMP1,TRAPZOID
...
IF (ABS(RTMP) <= TINY(1.0D0)) STOP 'PHASE FUNCTION ERROR 1 IN BBFRAC_CAL'
...
IF(BBFRAC_LOCAL<=0.0D0 .or. BBFRAC_LOCAL>=1.0D0) STOP 'PHASE FUNCTION ERROR 2 IN BBFRAC_CAL'
...
DOUBLE PRECISION,DIMENSION(0:MAXLFULL,1:6)::COEFFMIE_LOCAL

INTEGER,PARAMETER:: NMIE=10000, NPL=2*NMIE
INTEGER NDISTR,LMAXI,IL
DOUBLE PRECISION REFFI,VEFFI,LAM,MRR,MRI,AA1,BB1,AA2,BB2,GAMMAI,REFFO,VEFFO,AREA,CEXT,CSCAT
DOUBLE PRECISION AL1(NPL),AL2(NPL),AL3(NPL),AL4(NPL),BET1(NPL),BET2(NPL)
