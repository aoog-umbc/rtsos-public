...
INTEGER,DIMENSION(4) :: NUM_SCAT_ANG
...
ALLOCATE(PHMXMIE_ARSL_PACE(NUMMIEUSE,NWV_BAND,NUMMIEANGMAX,0:6), &
...
DEALLOCATE(PHMXMIE_ARSL_PACE,PHMXMIE_Plankton_PACE,PHMXMIE_Mineral_PACE)
...
INTEGER :: IMIE,INDXVAR,NANGMIE_LOCAL
...
DO INDXVAR=1,NWV_BAND
   PHMXMIE_ARSL_PACE(IMIE,INDXVAR,1:NANGMIE_LOCAL,0)=HDF5RARR(1:NANGMIE_LOCAL)
ENDDO
DEALLOCATE(HDF5RARR)

IF(PHMXMIE_ARSL_PACE(IMIE,1,1,0)>1.0E-6) STOP 'SCATTERING ANGLE (1) IS NOT ZERO'
IF(ABS(PHMXMIE_ARSL_PACE(IMIE,1,NANGMIE_LOCAL,0)-180.0D0)>1.0E-6) STOP 'LAST SCATTERING ANGLE IS NOT 180'
...
DO INDXVAR=1,NWV_BAND_SEG1P2
   PHMXMIE_ARSL_PACE(IMIE,INDXVAR,1:NANGMIE_LOCAL,1:6)=HDF5RARR3DIM(INDXVAR,1:NANGMIE_LOCAL,1:6)
ENDDO
...
DO INDXVAR=NWV_BAND_SEG1P2+1,NWV_BAND_SEG1P2+NWV_BAND_SEG3
   PHMXMIE_ARSL_PACE(IMIE,INDXVAR,1:NANGMIE_LOCAL,1:6) = &
...
INTEGER :: NDISTR,NANGMIE_LOCAL,ICOM,IRH
...
IF(FREEFORMFLAG<=1)THEN
  ...
  DO IMIE=1,NUMMIEUSE
    PHMXMIE_ARSL_PACE(IMIE,IWV_BAND,:,:)=(ARSLND1*CSCAT1*PHMXMIE_LOCAL1(:,:) + &
