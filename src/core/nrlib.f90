...
END MODULE nr
    ...
    MODULE RTSEC_WRAPPER
       INTERFACE rtsec
           MODULE PROCEDURE rtsec_sp,rtsec_dp
       END INTERFACE rtsec
    CONTAINS
	FUNCTION rtsec_sp(func,x1,x2,xacc)
	...
	do j=1,MAXIT
		dx=(xl-rtsec_sp)*f/(f-fl)
		xl=rtsec_sp
		fl=f
		rtsec_sp=rtsec_sp+dx
		f=func(rtsec_sp)
		if (abs(dx) < xacc .or. f == 0.0) RETURN
	end do
	call nrerror('rtsec_sp: exceed maximum iterations')
	END FUNCTION rtsec_sp

	REAL*8 FUNCTION rtsec_dp(func,x1,x2,xacc)
	USE nrtype; USE nrutil, ONLY : nrerror,swap
	IMPLICIT NONE
...
!		REAL(DP) :: func
	...
	do j=1,MAXIT
		dx=(xl-rtsec_dp)*f/(f-fl)
		xl=rtsec_dp
		fl=f
		rtsec_dp=rtsec_dp+dx
		f=func(rtsec_dp)
		if (abs(dx) < xacc .or. f == 0.0d0) RETURN
	end do
	call nrerror('rtsec_dp: exceed maximum iterations')
	END FUNCTION rtsec_dp
    END MODULE RTSEC_WRAPPER
...
SUBROUTINE HUNT_SIMPLE(XX,NXX,X,IULO)
IMPLICIT NONE
INTEGER :: NXX
REAL*8 :: XX(NXX),X
INTEGER :: IULO
