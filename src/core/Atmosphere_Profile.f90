...
REAL*8, PARAMETER :: MolarWeightDryAir=28.9647, GRAVITY_ACC=9.80665

REAL*8, EXTERNAL :: POLINT_SIMPLE

INTEGER :: jlo
REAL *8 :: SCALEHEIGHT,Z_TEMP,NUMDEN_TEMP,RTMP1,RTMP2
...
IF(debug)THEN
    NPGRID=NPGRID_Atmosphere_Profile
    ...
    P_GRID=PRESSURE_AP
    Z_FIELD=Z_FIELD_AP
    TEM_FIELD=T_FIELD_AP
    NUMDEN_TOTAL_FIELD=NUMDEN_TOTAL_FIELD_AP
	VMR_OZONE_FIELD=VMR_OZONE_FIELD_AP
    VMR_OXYGEN_FIELD=VMR_OXYGEN_FIELD_AP
    VMR_H2O_FIELD=VMR_H2O_FIELD_AP
	VMR_CO2_FIELD=VMR_CO2_FIELD_AP
	VMR_NO2_FIELD=VMR_NO2_FIELD_AP
    VMR_CH4_FIELD=VMR_CH4_FIELD_AP
    return
ENDIF
...
SCALEHEIGHT=-((Z_FIELD_AP(NPGRID-2) - Z_FIELD_AP(NPGRID-1)))    &
		    / LOG(PRESSURE_AP(NPGRID-2)/ PRESSURE_AP(NPGRID-1))
...
RTMP2=1000.0D0*RTMP1*AvogadroN/GRAVITY_ACC/MolarWeightDryAir  ! 1000 is g to kg.
RTMP2=RTMP2/ABS(Z_FIELD(NPGRID-1)-Z_FIELD(NPGRID))/1000.0d0 !Z_FIELD is in km

NUMDEN_TOTAL_FIELD(NPGRID) = 2.0D0*RTMP2-NUMDEN_TOTAL_FIELD_AP(NPGRID-1)
...
ELSE
  TEM_FIELD(NPGRID) = POLINT_SIMPLE(2,PRESSURE_AP(NPGRID-2),T_FIELD_AP(NPGRID-2), &
								    PRESSURE_SURFACE,2)
ENDIF

VMR_OZONE_FIELD(1:NPGRID-1) = VMR_OZONE_FIELD_AP(1:NPGRID-1)
IF(NPGRID>NPGRID_Atmosphere_Profile)THEN
   VMR_OZONE_FIELD(NPGRID) = VMR_OZONE_FIELD_AP(NPGRID-1)
ELSE
   VMR_OZONE_FIELD(NPGRID) = POLINT_SIMPLE(2,PRESSURE_AP(NPGRID-2), &
										   VMR_OZONE_FIELD_AP(NPGRID-2),  &
										   PRESSURE_SURFACE,2)
ENDIF
...
ELSE
	VMR_OXYGEN_FIELD(NPGRID) = POLINT_SIMPLE(2,PRESSURE_AP(NPGRID-2),         &
											   VMR_OXYGEN_FIELD_AP(NPGRID-2),  &
											   PRESSURE_SURFACE,2)
ENDIF

VMR_H2O_FIELD(1:NPGRID-1) = VMR_H2O_FIELD_AP(1:NPGRID-1)
IF(NPGRID>NPGRID_Atmosphere_Profile)THEN
	VMR_H2O_FIELD(NPGRID) = VMR_H2O_FIELD_AP(NPGRID-1)
ELSE
	VMR_H2O_FIELD(NPGRID) = POLINT_SIMPLE(2,PRESSURE_AP(NPGRID-2),         &
							            VMR_H2O_FIELD_AP(NPGRID-2),  &
										PRESSURE_SURFACE,2)
ENDIF
...
ELSE
	VMR_CO2_FIELD(NPGRID) = POLINT_SIMPLE(2,PRESSURE_AP(NPGRID-2),         &
									    VMR_CO2_FIELD_AP(NPGRID-2),  &
									    PRESSURE_SURFACE,2)
ENDIF
...
ELSE
	VMR_NO2_FIELD(NPGRID) = POLINT_SIMPLE(2,PRESSURE_AP(NPGRID-2),   &
			                            VMR_NO2_FIELD_AP(NPGRID-2),  &
										PRESSURE_SURFACE,2)
ENDIF
...
ELSE
	VMR_CH4_FIELD(NPGRID) = POLINT_SIMPLE(2,PRESSURE_AP(NPGRID-2),   &
						                VMR_CH4_FIELD_AP(NPGRID-2),  &
									    PRESSURE_SURFACE,2)
ENDIF
...
RTMP1=ABS(RTMP1)/Dobson

!rescale ozone profile with input column ozone
NUMDEN_FIELD_TEMP=NUMDEN_FIELD_TEMP/RTMP1*OZONE_COLUMN
...
RTMP2=ABS(RTMP2)/Density_liquid_water*100.0d0        ! Column Water depth in cm

! Rescale water vapor profile using input values
NUMDEN_FIELD_TEMP=NUMDEN_TOTAL_FIELD*VMR_H2O_FIELD
NUMDEN_FIELD_TEMP=NUMDEN_FIELD_TEMP/RTMP2*H2O_COLUMN
