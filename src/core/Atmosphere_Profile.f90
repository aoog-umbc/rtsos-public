...
DOUBLE PRECISION,DIMENSION(NPGRID_Atmosphere_Profile) :: PRESSURE_AP,Z_FIELD_AP,T_FIELD_AP, &
                  NUMDEN_TOTAL_FIELD_AP,VMR_OZONE_FIELD_AP,VMR_OXYGEN_FIELD_AP,&
                  VMR_H2O_FIELD_AP,VMR_CO2_FIELD_AP,VMR_NO2_FIELD_AP,       &
                  VMR_CH4_FIELD_AP

INTEGER :: NPGRID
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: P_GRID,Z_FIELD,TEM_FIELD,  &
			NUMDEN_TOTAL_FIELD,VMR_OZONE_FIELD,VMR_OXYGEN_FIELD,     &
			VMR_H2O_FIELD,VMR_CO2_FIELD,VMR_NO2_FIELD,VMR_CH4_FIELD

DOUBLE PRECISION, PARAMETER :: AvogadroN=6.022E23              ! molecules mol^{-1}
DOUBLE PRECISION, PARAMETER :: MolarWeight_H2O=18.016              ! kg/kmol

DOUBLE PRECISION :: OZONE_STD_COLUMN ! Whole column OZONE IN THE INPUT atmosphere IN DOBSON UNIT
DOUBLE PRECISION :: H2O_STD_COLUMN   ! Whole column WATER VAPOR THE INPUT atmosphere IN CENTIMETERS.
DOUBLE PRECISION :: NO2_STD_COLUMN !  Whole column NO2 IN THE INPUT atmosphere IN DOBSON UNIT
...
DOUBLE PRECISION,DIMENSION(9) :: RARRY
...
DOUBLE PRECISION,INTENT(INOUT) :: PRESSURE_SURFACE ! SURFACE PRESSURE IN mb.


DOUBLE PRECISION, PARAMETER :: MolarWeightDryAir=28.9647,MolarWeightH2O=18.01528, &
                     GRAVITY_ACC=9.80665
...
ELSE
   jlo=bisearch(NPGRID_Atmosphere_Profile,PRESSURE_AP,PRESSURE_SURFACE)
...
IF(NPGRID>NPGRID_Atmosphere_Profile)THEN
!	TEM_FIELD(NPGRID) = T_FIELD_AP(NPGRID_Atmosphere_Profile-1) + &
!	(T_FIELD_AP(NPGRID_Atmosphere_Profile)-T_FIELD_AP(NPGRID_Atmosphere_Profile-1)) &
!		*(Z_TEMP-Z_FIELD_AP(NPGRID_Atmosphere_Profile-1)) / &
!		 (Z_FIELD_AP(NPGRID_Atmosphere_Profile)-Z_FIELD_AP(NPGRID_Atmosphere_Profile-1))
	TEM_FIELD(NPGRID) = Func_UVIP3P(2,Z_FIELD_AP(NPGRID_Atmosphere_Profile-1),&
						 T_FIELD_AP(NPGRID_Atmosphere_Profile-1),Z_TEMP,1)

ELSE
!    TEM_FIELD(NPGRID) =T_FIELD_AP(NPGRID-1) + &
!			(T_FIELD_AP(NPGRID)-T_FIELD_AP(NPGRID-1)) &
!			   *(Z_TEMP-Z_FIELD_AP(NPGRID-1)) / &
!				(Z_FIELD_AP(NPGRID)-Z_FIELD_AP(NPGRID-1))
	TEM_FIELD(NPGRID) = Func_UVIP3P(2,Z_FIELD_AP(NPGRID-1),T_FIELD_AP(NPGRID-1), &
										Z_TEMP,1)
...
IF(NPGRID>NPGRID_Atmosphere_Profile)THEN
   VMR_OZONE_FIELD(NPGRID) = Func_UVIP3P(2,PRESSURE_AP(NPGRID_Atmosphere_Profile-1), &
...
ELSE
   VMR_OZONE_FIELD(NPGRID) = Func_UVIP3P(2,PRESSURE_AP(NPGRID-1), &
...
ELSE
	VMR_OXYGEN_FIELD(NPGRID) = Func_UVIP3P(2,PRESSURE_AP(NPGRID-1),         &
...
IF(NPGRID>NPGRID_Atmosphere_Profile)THEN
    VMR_H2O_FIELD(NPGRID) = Func_UVIP3P(2,PRESSURE_AP(NPGRID_Atmosphere_Profile-1),&
									VMR_H2O_FIELD_AP(NPGRID_Atmosphere_Profile-1),   &
									PRESSURE_SURFACE,1)
ELSE
	VMR_H2O_FIELD(NPGRID) = Func_UVIP3P(2,PRESSURE_AP(NPGRID-1),         &
...
ELSE
	VMR_CO2_FIELD(NPGRID) = Func_UVIP3P(2,PRESSURE_AP(NPGRID-1),         &
...
ELSE
	VMR_NO2_FIELD(NPGRID) = Func_UVIP3P(2,PRESSURE_AP(NPGRID-1),   &
...
ELSE
	VMR_CH4_FIELD(NPGRID) = Func_UVIP3P(2,PRESSURE_AP(NPGRID-1),   &
...
ELSE
	NUMDEN_TOTAL_FIELD(NPGRID) = Func_UVIP3P(2,PRESSURE_AP(NPGRID-1),   &
...
SUBROUTINE ALLO_ATMOS_SUB
  ALLOCATE(P_GRID(NPGRID),Z_FIELD(NPGRID),TEM_FIELD(NPGRID),       &
		   NUMDEN_TOTAL_FIELD(NPGRID),VMR_H2O_FIELD(NPGRID),       &
		   VMR_CO2_FIELD(NPGRID))
END SUBROUTINE ALLO_ATMOS_SUB

SUBROUTINE DEALLO_ATMOS_SUB
  DEALLOCATE(P_GRID,Z_FIELD,TEM_FIELD, NUMDEN_TOTAL_FIELD,VMR_H2O_FIELD,&
             VMR_CO2_FIELD)
END SUBROUTINE DEALLO_ATMOS_SUB


SUBROUTINE GAS_COLUMN_RESCALE(OZONE_COLUMN,H2O_COLUMN)
DOUBLE PRECISION,INTENT(IN) :: OZONE_COLUMN ! OZONE IN THE WHOLE COLUMN IN DOBSON UNIT
DOUBLE PRECISION,INTENT(IN) :: H2O_COLUMN   ! WATER VAPOR IN THE WHOLE COLUMN IN CENTIMETERS.

DOUBLE PRECISION, PARAMETER :: Dobson=2.6867E20   !molecules per meter square, equivalent to 10^{-5} meter in one STP
!DOUBLE PRECISION, PARAMETER :: Num_Mol_atm_cm=Dobson*1000     ! 1 cm = 10^{-2} meter

DOUBLE PRECISION, PARAMETER :: Density_liquid_water = 1000.0d0 ! kg/m3

DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: NUMDEN_FIELD_TEMP,Z_FIELD_TEMP

DOUBLE PRECISION,EXTERNAL :: TRAPZOID
...
DOUBLE PRECISION,INTENT(IN) :: NO2_COLUMN ! NO2 IN THE WHOLE COLUMN IN DOBSON UNIT
