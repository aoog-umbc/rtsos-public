...
ENDDO
 ...
 write(*,*)'PMAX integration elapsed time =', finish_time-start_time
...
CALL pBRDM_LAND(COST1,0.0D0,COST2,PHI(IPHI),TEM1)
IF(SCL)THEN
RTMP=TEM1(1,1)
TEM1=0.0D0
TEM1(1,1)=RTMP
...
USE RTUTILITY, ONLY : TAU,NALYR,HEIGHT_ATMOSPHERE,ALYR,SECANT_THETAL, &
               SECANT_THETAL_PRSV,RAYLEIGH_SCALE_HEIGHT,RADIUS_EARTH
IMPLICIT NONE
REAL*8,INTENT(IN) :: MU_IN
INTEGER :: ILAYER,JLAYER,NXX
REAL*8 :: POLINT_SIMPLE,RTMP,SINT0,SUMTMP,XX,YY,ZZ
...
! CALCULATE SECANT_THETAL OF THE SOLAR BEAM FOR A LINE OF SIGHT ALONG A VIEWING DIRECTION
! SENSOR IS ASSUMED TO BE LOCATED AT THE SURFACE.
SUBROUTINE SLANT_PATH_SURF_FSS(INDX_LAYER,MU_IN,MUV,PHIV,&
					SECANT_THETAL_LOCAL)
USE RTUTILITY, ONLY : PI,TAU,NALYR,HEIGHT_ATMOSPHERE,ALYR,SECANT_THETAL, &
			   SECANT_THETAL_PRSV,RAYLEIGH_SCALE_HEIGHT,RADIUS_EARTH
IMPLICIT NONE
INTEGER,INTENT(IN) :: INDX_LAYER
REAL*8,INTENT(IN) :: MU_IN,MUV,PHIV
REAL*8, INTENT(OUT)::SECANT_THETAL_LOCAL
...
IF(ABS(RTMP-RADIUS_EARTH-HEIGHT_ATMOSPHERE(INDX_LAYER-1))>1.0E-6)THEN
  WRITE(*,*)'SINTV,RADIUS_EARTH,HEIGHT_ATMOSPHERE(INDX_LAYER-1),LDIS,RTMP='
  WRITE(*,*)SINTV,RADIUS_EARTH,HEIGHT_ATMOSPHERE(INDX_LAYER-1),LDIS,RTMP
  STOP
ENDIF
...
IF(ABS(COSD0)>1.0D0) STOP 'CHECK COSD0'

SINT0=SQRT(1.0D0-COSD0*COSD0)

RTMP=(RADIUS_EARTH+HEIGHT_ATMOSPHERE(INDX_LAYER))*SINT0
YY=0.D0
DO JLAYER=1,INDX_LAYER
 XX= SQRT((RADIUS_EARTH+HEIGHT_ATMOSPHERE(JLAYER-1))**2-RTMP**2) -  &
	 SQRT((RADIUS_EARTH+HEIGHT_ATMOSPHERE(JLAYER  ))**2-RTMP**2)
 YY=YY+XX/(HEIGHT_ATMOSPHERE(JLAYER-1)-HEIGHT_ATMOSPHERE(JLAYER)) * &
		  (ALYR(JLAYER)%TAUT-ALYR(JLAYER)%TAUB)
 IF(YY<0.0d0) STOP 'CHECK SLANT_PATH_PSS SUBROUTINE'
ENDDO
SECANT_THETAL_LOCAL=YY/ALYR(INDX_LAYER)%TAUT

ENDSUBROUTINE SLANT_PATH_SURF_FSS

! CALCULATE SECANT_THETAL OF THE SOLAR BEAM FOR A LINE OF SIGHT ALONG A VIEWING DIRECTION
! SENSOR IS ASSUMED TO BE LOCATED AT THE TOA.
SUBROUTINE SLANT_PATH_TOA_FSS(INDX_LAYER,MU_IN,MUV,PHIV,&
					SECANT_THETAL_LOCAL)
USE RTUTILITY, ONLY : PI,TAU,NALYR,HEIGHT_ATMOSPHERE,ALYR,SECANT_THETAL, &
			   SECANT_THETAL_PRSV,RAYLEIGH_SCALE_HEIGHT,RADIUS_EARTH
IMPLICIT NONE
...
IF(SINTV<1.0E-4 .OR. SINTV>SINT2 .OR. INDX_LAYER==0)THEN
   SECANT_THETAL_LOCAL=SECANT_THETAL_PRSV(INDX_LAYER)
   RETURN
ENDIF
...
!IF(theta1<0.0d0)stop 'check SLANT_PATH_TOA_FSS'

LDIS=(RADIUS_EARTH+HEIGHT_ATMOSPHERE(0))*SIN(THETA1)/SINT1
...
RTMP=SQRT(XI*XI+YI*YI+ZI*ZI)
IF(ABS(RTMP-RADIUS_EARTH-HEIGHT_ATMOSPHERE(INDX_LAYER))>1.0E-6)THEN
  WRITE(*,*)'SINTV,RADIUS_EARTH,HEIGHT_ATMOSPHERE(INDX_LAYER),LDIS,RTMP='
  WRITE(*,*)SINTV,RADIUS_EARTH,HEIGHT_ATMOSPHERE(INDX_LAYER),LDIS,RTMP
  STOP
ENDIF
