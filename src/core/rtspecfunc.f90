...
IF(abs(HEIGHT_TEMP-HEIGHT_ATMOSPHERE(0))<1.0e-6)THEN
  SECANT_THETAL_LOCAL=SECANT_THETAL_PRSV(0)
  RETURN
ENDIF
...
IF(INDX_LAYER==1)THEN
   SECANT_THETAL_LOCAL=XX/(HEIGHT_ATMOSPHERE(INDX_LAYER-1)-HEIGHT_TEMP)
ELSE
   TAUFRAC=(HEIGHT_ATMOSPHERE(INDX_LAYER-1)-HEIGHT_TEMP) / &
...
!	 IF(YY<0.0d0) STOP 'CHECK SLANT_PATH_PSS SUBROUTINE'
	ENDDO
    SECANT_THETAL_LOCAL=YY/(ALYR(INDX_LAYER)%TAUB+TAUFRAC)
ENDIF
...
INTEGER :: ILAYER,JLAYER
REAL*8 :: TAUFRAC,RTMP,SINT0,XX,YY,ZZ
...
IF(SINTV>SINT2)THEN
   SECANT_THETAL_LOCAL=SECANT_THETAL_PRSV(INDX_LAYER-1) !?? CHECK LATER
   RETURN
ENDIF
...
ELSE
    LDIS=HEIGHT_ATMOSPHERE(0)-HEIGHT_TEMP
ENDIF
...
RTMP=SQRT(XI*XI+YI*YI+ZI*ZI)
!IF(ABS(RTMP-RADIUS_EARTH-HEIGHT_TEMP)>1.0E-6)THEN
!  WRITE(*,*)'SINTV,RADIUS_EARTH,HEIGHT_TEMP,LDIS,RTMP='
!  WRITE(*,*)SINTV,RADIUS_EARTH,HEIGHT_TEMP,LDIS,RTMP
!  STOP
!ENDIF
