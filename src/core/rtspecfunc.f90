...
DOUBLE PRECISION,DIMENSION(4,4) :: MTRX2
LOGICAL :: PM_EXP_FLG
...
ELSE ! DIRECT INTEGRATION
  ...
  PHIRG=PI
  CALL gauss_legendre(-PHIRG,PHIRG,PHI_TMP,WPHI_TMP,NUMPHI)
...
USE RossLiBRDF
IMPLICIT NONE
DOUBLE PRECISION,INTENT(IN)::WAVELENGTH_MICRON,MU0
INTEGER(I4B) ::L,M,K,IQUAD,IQUAD2,MAXMS,NUMPHI,IPHI
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: PHI,WPHI
DOUBLE PRECISION,DIMENSION(4,4)::SLI1,SLI2,FFINT,TEM1,TEM2,FRES_REFL,FRES_TRAN
DOUBLE PRECISION SINT1,SINT2,COST1,COST2,CAPA,LITA,LITB, &
...
DOUBLE PRECISION FUNCTION expbessi(n,x)
INTEGER n,IACC
DOUBLE PRECISION bessi,x,BIGNO,BIGNI
PARAMETER (IACC=10000,BIGNO=1.0d38,BIGNI=1.0d-38)
!U    USES bessi0
INTEGER j,m
DOUBLE PRECISION bi,bim,bip,tox,bessi0,BESEI1,BESEI0
!DOUBLE PRECISION mutmp,fln,pi,sqrttwopi,eightbr
!DOUBLE PRECISION expbessi0,expbessi1
...
!        expbessi=bessi*expbessi0(x)/bi
      ...
      implicit none
      DOUBLE PRECISION, INTENT(IN) :: SA,SB,SC,FA,FB,FC
	  DOUBLE PRECISION :: SPHI,FPHI
      DOUBLE PRECISION, DIMENSION(4,4),INTENT(OUT) :: SLI1, SLI2
      DOUBLE PRECISION :: COSA,COSB,COSC,SINA,SINB,SINC, &
...
END SUBROUTINE DMLSPEC
      ...
      DOUBLE PRECISION,ALLOCATABLE,DIMENSION(:)::MU_TMP,PHI_TMP
      DOUBLE PRECISION,DIMENSION(4,4)::BRDMO
      DOUBLE PRECISION DMU_CM,DMU_TMP,DPHI_TMP,RTMP
      NMU_CM=30
...
DOUBLE PRECISION,INTENT(OUT)::REFL_COEFF
DOUBLE PRECISION :: MU_SEARCH,RTMP
INTEGER :: IU_LO,M_PL,K_LO,ND,NI
DOUBLE PRECISION,DIMENSION(1) :: XII,YII
...
IF(ATFLAG)THEN
  CALL UVIP3P(M_PL,NMU_CM,MU_CM,REFL_CM_AT,1,XII,YII)
ELSE
  CALL UVIP3P(M_PL,NMU_CM,MU_CM,REFL_CM_OT,1,XII,YII)
ENDIF
REFL_COEFF=YII(1)
...
DOUBLE PRECISION,INTENT(IN) :: MU0
LOGICAL,INTENT(IN) :: ATFLAG
DOUBLE PRECISION,INTENT(OUT)::TRAN_COEFF
DOUBLE PRECISION :: MU_SEARCH,RTMP
...
IF(ATFLAG)THEN
  CALL UVIP3P(M_PL,NMU_CM,MU_CM,TRAN_CM_AT,1,XII,YII)
ELSE
  CALL UVIP3P(M_PL,NMU_CM,MU_CM,TRAN_CM_OT,1,XII,YII)
ENDIF
TRAN_COEFF=YII(1)
...
!IMPLICIT NONE
!DOUBLE PRECISION,INTENT(IN) :: N1,N2,THETA1,THETA2
!DOUBLE PRECISION,DIMENSION(4,4),INTENT(OUT) :: FRES_REFL

!DOUBLE PRECISION :: NREL,NREL2,GAMMA_RE,GAMMA_IM,ALPHA,BETA,  &
...
!IMPLICIT NONE
!DOUBLE PRECISION,INTENT(IN) :: N1,N2,THETA1,THETA2
!DOUBLE PRECISION,DIMENSION(4,4),INTENT(OUT) :: FRES_TRAN

!DOUBLE PRECISION :: NREL, ALPHA,BETA,GAMMA,SINT2,SINT1,SINT12,COST12,RTMP1
...
DOUBLE PRECISION :: Func_UVIP3P,RTMP,SINT0,XX,YY,ZZ
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XARRY,YARRY
...
DO JLAYER=1,ALYR(NALYR)%ITAUE
   SECANT_THETAL(JLAYER)=Func_UVIP3P(NXX,XARRY,YARRY,TAU(JLAYER),2)
ENDDO
...
DOUBLE PRECISION,INTENT(IN) :: HEIGHT_TEMP,MU_IN,MUV,PHIV
DOUBLE PRECISION, INTENT(OUT)::SLANT_TAU_LOCAL

INTEGER :: ILAYER,JLAYER
DOUBLE PRECISION :: RTMP,SINT0,XX,YY,ZZ

DOUBLE PRECISION :: SINT1,SINT2,SINTV,THETA1,LDIS,COSD0
DOUBLE PRECISION :: XI,YI,ZI,AI,BI,CI,TAUFRAC
...
INTEGER :: ILAYER,JLAYER
DOUBLE PRECISION :: RTMP,SINT0,XX,YY,ZZ !,TAUFRAC

DOUBLE PRECISION :: SINT1,SINTV,THETA1,LDIS,COSD0,DIST0 !,SINT2
DOUBLE PRECISION :: XI,YI,ZI,AI,BI,CI,DISSIGN
...
!IMPLICIT NONE
!DOUBLE PRECISION,INTENT(IN) :: MUV
!INTEGER,INTENT(IN) :: INDX_LAYER
!DOUBLE PRECISION,INTENT(OUT) :: SECANT_THETAL_LAYER
!
!DOUBLE PRECISION :: SINT1,SINT2,SINTV,THETA1,LDIS1,LDIS2
!
!SINTV=SQRT(1.0D0-MUV*MUV)
