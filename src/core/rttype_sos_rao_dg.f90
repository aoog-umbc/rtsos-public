...
MODULE PHASE_MATRIX_TRUNCATION

USE RTTYPE
INTEGER(I4B) :: NUMMIE
LOGICAL :: TRUC_FLAG=.true.    ! UTILIZE THE DLETA FIT or Deltam TECHNIQUE
LOGICAL :: PHMX_COEFF_EXT_UNASSIGN_FLAG=.TRUE.
...
INTEGER,PARAMETER :: MAXLFULL=2000
...
REAL(DP), DIMENSION(0:2) :: &
		  BETAL_Ray_Depol0p029=(/1.0_DP,0.0_DP,0.4785575_DP/),&
          ALPHAL_Ray_Depol0p029=(/0.0_DP,0.0_DP,2.8713629_DP/),&
          ZETAL_Ray_Depol0p029=(/0.0_DP,0.0_DP,0.0_DP/),&
          DELTAL_Ray_Depol0p029=(/0.0_DP,1.392801_DP,0.0_DP/),&
		  GAMMAL_Ray_Depol0p029=(/0.0_DP,0.0_DP,-1.1722288_DP/),&
		  EPSILONL_Ray_Depol0p029=(/0.0_DP,0.0_DP,0.0_DP/)
...
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: PHF_RESIDUE

INTEGER,DIMENSION(:),ALLOCATABLE :: NUMORDFULLBETAL
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: FULLBETAL
...
SUBROUTINE PHMX_COEFF_ALLO(MAXLORD,NUMMIEINPUT,NUMMIEANGINPUT)
IMPLICIT NONE
INTEGER,INTENT(IN) :: MAXLORD,NUMMIEINPUT
INTEGER,INTENT(IN),DIMENSION(NUMMIEINPUT) ::NUMMIEANGINPUT

INTEGER :: MAXMIEANG_LOCAL
! NUMBER OF SETS OF MIE SCATTERING MATRIX
!IF(MAXLORD<NQUADO)MAXLORD=NQUADO
!WRITE(9,"('MAXLORD,MAXMORD=',I5,2x,I5)")MAXLORD,MAXMORD
!WRITE(9,'("THERE ARE",I4,1X,"SETS OF MIE TYPES.")')NUMMIE
NUMMIE=NUMMIEINPUT
MAXMIEANG_LOCAL=MAXVAL(NUMMIEANGINPUT)
ALLOCATE(NUMORD(NUMMIE),FTRUNC(NUMMIE))
ALLOCATE(BETAL(NUMMIE,0:MAXLORD),ALPHAL(NUMMIE,0:MAXLORD), &
		 ZETAL(NUMMIE,0:MAXLORD),DELTAL(NUMMIE,0:MAXLORD), &
		 GAMMAL(NUMMIE,0:MAXLORD),EPSILONL(NUMMIE,0:MAXLORD))
...
ALLOCATE(NUMMIEANG(NUMMIE),THETA_SNG(NUMMIE,MAXMIEANG_LOCAL), &
		 MU_SNG(NUMMIE,MAXMIEANG_LOCAL),PM_AO_SNG(NUMMIE,MAXMIEANG_LOCAL),&
		 PHF_RESIDUE(NUMMIE,MAXMIEANG_LOCAL),NUMORDFULLBETAL(NUMMIE),&
		 FULLBETAL(NUMMIE,0:MAXLFULL) )


END SUBROUTINE PHMX_COEFF_ALLO

SUBROUTINE PHMX_COEFF_DEALLO

DEALLOCATE(NUMORD,FTRUNC,BETAL,ALPHAL,ZETAL,DELTAL,GAMMAL,EPSILONL)

DEALLOCATE(NUMMIEANG,THETA_SNG,MU_SNG,PM_AO_SNG,PHF_RESIDUE,NUMORDFULLBETAL ,&
		 FULLBETAL)

END SUBROUTINE PHMX_COEFF_DEALLO

END MODULE PHASE_MATRIX_TRUNCATION
...
LOGICAL :: VERBOSE_FLAG=.TRUE.
LOGICAL :: OCEAN,OCEAN_NO_TRAN,LAMB,pBRDF,FLATO,WCFLG,RSRADDFLG,DETA,DETO,WLR_FLAG,   &
           SAVE_RAMAN_FLAG,SAVE_FLUORESCENCE_FLAG,                  &
           RAMAN_SOURCE_FLAG,FLUORESCENCE_SOURCE_FLAG
LOGICAL :: SCL = .FALSE.    ! CALCULATED SCALAR RADIANCE
LOGICAL :: NIGHTTIME_FLAG =.FALSE.  ! IF TRUE, NO SOLAR SOURCE IS NEEDED
LOGICAL :: PSEUDO_SPHERICAL_SHELL = .FALSE.
LOGICAL :: SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION = .FALSE.
LOGICAL :: RAD_MAP=.TRUE.,SNG_RAD_CORR=.TRUE.,SEC_RAD_CORR=.TRUE.
!surface options
LOGICAL :: SHDWFLG=.true. ! IF TRUE, USE SHADOWING FUNCTION
						  ! SHOULD BE ALWAYS TRUE.
LOGICAL :: ENORM_FLAT=.false.       ! ENGERGY RENORMALIZATION according to flat surface
LOGICAL :: ENGFACNORM=.FALSE.       ! if true, use Energy renormalization for rough surface
						 ! reflection and transmission matrices according to
						 ! Nakajima's scheme.
LOGICAL :: GEOSR=.true.       ! USE THE GEOMETRIC SERIES APPROXIMATION

LOGICAL :: LINEXP=.true.       ! USE THE LINEAR - EXPONENTIAL APPROXIMATION

LOGICAL :: OC_IF_DS=.false.    ! if true do double scattering for ocean interface scattering
					! need to be tested in the future
LOGICAL :: OSFREXP=.false.     ! USE SERIES COMBINATION for THE OCEAN BRDF
					! IF TRUE, THE INPUT FILE
					! "fresnel_refaw.coeff" AND "fresnel_tranaw.coeff"
					! WILL BE USED.  THESE TWO FILES ARE FOR NWRE=1.338
					! IF OTHER WATER REFRACTIVE INDEX IS NEEDED,
					! DELTA FIT NEED TO BE re-RUN TO GENERATE
					! NEW COEFFICIENT FILES FOR FRESNEL SURFACE.
LOGICAL :: MUFLG=.false.        ! IF TRUE OUTPUT MU OF THE ZENITH ANGLE
					! IF FALSE OUT PUT THETA OF THE ZENITH ANGLE
					

LOGICAL :: FWDCN_CORR_FLAG=.false.
...
INTEGER :: I_SURFACE_ROUGHNESS_PARA=1
	! I_SURFACE_ROUGHNESS_PARA=1 COX&MUNK 1954
	! I_SURFACE_ROUGHNESS_PARA=2 GORDON & WANG 1992

INTEGER :: IPSS_VIEWANGLE_GROUND=0
...
REAL(DP) :: SURFACE_TEMPERATURE,SURFACE_EMISSIVITY,CMB_Rad,SURFACE_RAD
INTEGER(I4B) :: MAXLORD,MAXMORD,MAXKORD,MAXSORD
...
REAL(DP), DIMENSION(4) :: ESUN=(/PI,0.0D0,0.0D0,0.0D0/)
...
REAL(DP),DIMENSION(:),ALLOCATABLE :: BT_Planck
! BT_Planck: Planck function calculated based on temperature and wavelength.
...
TYPE(ARRAY4),DIMENSION(:,:,:),ALLOCATABLE :: LRADA,LRADO,LRADA_SNG,LRADA_SNG_SPSH
...
DOUBLE PRECISION,DIMENSION(NMUFWDCN):: MUFWDCN_LCL
DOUBLE PRECISION,DIMENSION(NPHIFWDCN):: PHIFWDCN_LCL
DOUBLE PRECISION,DIMENSION(NMUFWDCN,NPHIFWDCN):: MUFWDCN,PHIFWDCN
DOUBLE PRECISION :: MINMUFWDCN
...
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: SHDWXJA,SHDWXJO
...
DOUBLE PRECISION, DIMENSION(4), PARAMETER :: Aj=(/0.41D0, 0.39D0, 0.10D0, 0.10D0/);
DOUBLE PRECISION, DIMENSION(4), PARAMETER :: Kj=(/3250D0, 3425D0, 3530D0, 3625D0/);
DOUBLE PRECISION, DIMENSION(4), PARAMETER :: deltaKj=(/210D0, 175D0, 140D0, 140d0/);
DOUBLE PRECISION, DIMENSION(NKGRID),PARAMETER::KppGrid=(/2960.0d0,3080.0d0,3200.0d0,&
           3320.0d0,3440.0d0,3560.0d0,3680.0d0,3800.0d0/)
DOUBLE PRECISION, DIMENSION(NKGRID),PARAMETER::KppWeight=(/60.0d0,120.0d0,120.0d0,&
              120.0d0,120.0d0,120.0d0,120.0d0,60.0d0/)
DOUBLE PRECISION,DIMENSION(NKGRID):: LAMBDAEX,fRfuncjEX

CONTAINS
SUBROUTINE FRFUNCJCAL(LAMBDAEM)
IMPLICIT NONE
DOUBLE PRECISION,INTENT(IN) :: LAMBDAEM
DOUBLE PRECISION :: KEM,KEX,fRfuncj,fNormC,FNORMFAC
INTEGER :: IKPP,IJ
KEM=1.0D7/LAMBDAEM
fRfuncjEX=0.0D0
DO IKPP=1,NKGRID
...
LOGICAL :: NPQ_FLAG
DOUBLE PRECISION :: QUANTUM_YIELD_CHLA_MIN,QUANTUM_YIELD_CHLA_MAX, &
...
DOUBLE PRECISION,DIMENSION(:), ALLOCATABLE :: LAMBDAEXFLRSC,fCDOMfuncjEX,fCHLAfuncjEX
...
DOUBLE PRECISION,DIMENSION(N_FCDOM_WAVELENGTH,N_FCDOM_TYPE) :: A0_CDOM
DOUBLE PRECISION,DIMENSION(N_FCDOM_WAVELENGTH) :: WAVELENGTH_FCDOM_TABLE=(/310.0D0,330.0D0,&
                  350.0D0,370.0D0,390.0D0,410.0D0,430.0D0,450.0D0,470.0D0,490.0D0/)
DOUBLE PRECISION,DIMENSION(N_FCDOM_TYPE)::WEIGHT_CDOM_TYPE
DOUBLE PRECISION,DIMENSION(N_FCDOM_TYPE)::A1_CDOM=(/0.470d0,0.389d0,0.466d0,0.471d0,0.304d0,&
                                          0.379d0,0.346d0,0.447d0,0.356d0/)
DOUBLE PRECISION,DIMENSION(N_FCDOM_TYPE)::A2_CDOM=(/0.407d0,0.386d0,0.386d0,0.386d0,0.591d0,&
                                          0.362d0,0.411d0,0.417d0,0.406d0/)
DOUBLE PRECISION,DIMENSION(N_FCDOM_TYPE)::B1_CDOM=(/8.077d-4,10.073d-4, 8.340d-4,8.204d-4, &
                               12.169d-4,10.043d-4,10.891d-4,8.594d-4,10.694d-4/)
DOUBLE PRECISION,DIMENSION(N_FCDOM_TYPE)::B2_CDOM=(/-4.57d-4,-4.20d-4,-4.13d-4,-4.20d-4,   &
...
INTEGER :: ILAMBDAPP
DOUBLE PRECISION :: FNORMFAC_CDOM,FNORMFAC_CHLA
...
DOUBLE PRECISION :: SIGMA_CHLA,LAMBDA0CHLA,FAC1,GCHLAFAC,HCFAC,FCFAC1,FCFAC2
INTEGER :: ILAMBDAPP,FCDOM_FH_indx
DOUBLE PRECISION :: FNORMFAC_CDOM,FNORMFAC_CHLA

DOUBLE PRECISION,DIMENSION(N_FCDOM_WAVELENGTH) :: A0_CDOM_TEMP
DOUBLE PRECISION :: A0_CDOM_LOCAL
...
IF(OCEAN_FCDOM_FLAG) THEN
  ...
  DO ILAMBDAPP=1,NFLRSCGRID
    ...
    A0_CDOM_LOCAL=Func_UVIP3P(N_FCDOM_WAVELENGTH,WAVELENGTH_FCDOM_TABLE,&
                                 A0_CDOM_TEMP,LAMBDAEXFLRSC(ILAMBDAPP),2)
...
DOUBLE PRECISION,DIMENSION(NWATER_DEPTH),INTENT(IN)::WATER_DEPTH,IPAR_SCALAR
DOUBLE PRECISION,INTENT(OUT)::IPAR_AVG
...
DOUBLE PRECISION,INTENT(IN) :: IPAR_AVG
DOUBLE PRECISION, EXTERNAL :: Func_UVIP3P
DOUBLE PRECISION,DIMENSION(1:12) ::Ig_SORT,ET_SORT ! DATA FROM MORRISON AND GOODWIN, FIG. 2a, 2c, and 3c.
...
DOUBLE PRECISION,DIMENSION(1:12) :: Ig_log
DOUBLE PRECISION :: IPAR_AVG_log
...
QUANTUM_YIELD_CHLA_ET=Func_UVIP3P(12,Ig_log,ET_SORT,IPAR_AVG_log,2)
