...
REAL*8,SAVE :: WV_REF=-999.0d0 ! 0.55d0 or 0.870  ! additional wavelength at which aerosol optical depth is assigned.
...
REAL*8,DIMENSION(NWV_SF) ::    MI_DUST_SF=(/0.0006D0, 0.0006D0, 0.0006D0, 0.0006D0,&
...
REAL*8,DIMENSION(NWV_SF) ::    MR_SOOT_SF=(/1.95D0, 1.95D0, 1.95D0,1.95D0,1.95D0, &
...
REAL*8,DIMENSION(NWV_SF) ::    MI_SOOT_SF=(/0.79D0, 0.79D0, 0.79D0,0.79D0,0.79D0, &
...
SUBROUTINE  HYGROSCOPIC_GROWTH_EMIX(R0DL,R0WS,R0BC,R0S,R0SS,RHI, RRH_DL,RRH_WS,RRH_BC,RRH_S,RRH_SS)
  ...
  IF(RHI >= 0.9) THEN
     ...
     GAMMA_S = POLINT_SIMPLE(5, RHV(3:), GAMMA_SA(3:),RHI,3)
     GAMMA_BC = POLINT_SIMPLE(5, RHV(3:), GAMMA_BCA(3:),RHI,3)
  ELSEIF(RHI<0.9) THEN
     GAMMA_WS = POLINT_SIMPLE(3, RHV(:3), GAMMA_WSA(:3),RHI,2)
     GAMMA_S = POLINT_SIMPLE(3, RHV(:3), GAMMA_SA(:3),RHI,2)
     GAMMA_BC = POLINT_SIMPLE(3, RHV(:3), GAMMA_BCA(:3),RHI,2)
  ENDIF
