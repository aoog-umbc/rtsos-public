...
DOUBLE PRECISION,DIMENSION(NRH)::RH=(/0.30,0.50,0.70,0.75,0.80,0.85,0.90,0.95/)

DOUBLE PRECISION,DIMENSION(NWV_SF) :: WV_SF=(/0.30D0, 0.3371D0, 0.40D0, 0.4880D0, 0.5145D0, &
...
!SF data
!BC refractive index changed according to Bond and Bergstrom, 2006
!Dustlike refractive index is replaced by refractive index of fine mode non absorbing insoluble(FNAI) (similar to  Li et al., 2019) from Koepke et al.,1997
!Hygroscopic growth factors of Soot and BrC adopted from Chin et al., 2002
 
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MR_WATER_SOLUBLE_SF=(/1.53D0, 1.53D0, 1.53D0, 1.53D0, &
       1.53D0, 1.53D0, 1.53D0, 1.53D0, 1.52D0, 1.52D0,1.51D0, 1.51D0, 1.46D0, 1.42D0,&
       1.42D0, 1.42D0/)
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MI_WATER_SOLUBLE_SF=(/0.008D0, 0.005D0, 0.005D0,  &
       0.005D0, 0.005D0, 0.006D0, 0.006D0, 0.007D0,0.012D0, 0.017D0, 0.02D0,     &
       0.023D0, 0.017D0, 0.008D0, 0.01D0, 0.012D0/)
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MR_DUST_SF=(/1.53D0, 1.53D0, 1.53D0, 1.53D0,      &
       1.53D0, 1.53D0, 1.53D0, 1.53D0, 1.52D0, 1.52D0, 1.46D0, 1.40D0,1.33D0,    &
       1.26D0, 1.22D0, 1.18D0/)
!DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MI_DUST_SF=(/0.008D0, 0.008D0, 0.008D0, 0.008D0,  &
!       0.008D0, 0.008D0, 0.008D0, 0.008D0, 0.008D0, 0.008D0,0.008D0, 0.008D0,    &
!       0.008D0, 0.008D0, 0.009D0, 0.009D0/)
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MI_DUST_SF=(/0.0006D0, 0.0006D0, 0.0006D0, 0.0006D0,&
       0.0006D0, 0.0006D0, 0.0006D0, 0.0006D0, 0.0006D0, 0.0006D0,0.0006D0, 0.0006D0,&
       0.0006D0, 0.0006D0, 0.0006D0, 0.0006D0/)
!DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MR_SOOT_SF=(/1.74D0, 1.75D0, 1.75D0, 1.75D0, 1.75D0, &
!       1.75D0, 1.75D0, 1.75D0, 1.75D0, 1.75D0, 1.76D0, 1.77D0,1.79D0, 1.80D0, 1.81D0, 1.82D0/)
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MR_SOOT_SF=(/1.95D0, 1.95D0, 1.95D0,1.95D0,1.95D0, &
       1.95D0, 1.95D0, 1.95D0, 1.95D0, 1.95D0, 1.95D0, 1.95D0,1.95D0,1.95D0,1.95D0, 1.95D0/)
!DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MI_SOOT_SF=(/0.47D0, 0.47D0, 0.46D0, 0.45D0, 0.45D0, &
!       0.44D0, 0.43D0, 0.43D0, 0.43D0, 0.44D0, 0.45D0, 0.46D0,0.48D0, 0.49D0, 0.50D0, 0.51D0/)
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MI_SOOT_SF=(/0.79D0, 0.79D0, 0.79D0,0.79D0,0.79D0, &
       0.79D0, 0.79D0, 0.79D0, 0.79D0, 0.79D0, 0.79D0, 0.79D0,0.79D0,0.79D0,0.79D0, 0.79D0/)
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MR_SALT_SF=(/1.51D0,1.51D0,1.50D0,1.50D0,1.50D0, &
       1.50D0,1.49D0,1.49D0,1.48D0,1.47D0,1.47D0,1.46D0,1.45D0,1.45D0,1.44D0,1.43D0/)
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MI_SALT_SF=(/2.0D-6,4.0D-7,3.0D-8,2.0D-8,1.0D-8,  &
       1.0D-8,2.0D-8,1.0D-7,3.0D-6,2.0D-4,4.0D-4,6.0D-4,8.0D-4,1.0D-3,2.0D-3,4.0D-3/)
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MR_WATER_SF=(/1.349D0,1.345D0,1.339D0,1.335D0,   &
       1.334D0,1.333D0,1.332D0,1.331D0,1.329D0,1.326D0,1.323D0,1.318D0,1.312D0,1.306D0,1.292D0,1.261D0/)
DOUBLE PRECISION,DIMENSION(NWV_SF) ::    MI_WATER_SF=(/1.6D-8,8.45D-9,1.86D-9,9.69D-10,   &
...
DOUBLE PRECISION,DIMENSION(NRH) :: RTROPOSPHERE=(/0.027288,0.02748,0.02846,0.0306,0.03274,0.03579,0.03884,0.04238/)
DOUBLE PRECISION,DIMENSION(NRH) :: RRURALC=(/0.43462,0.4377, 0.4571,0.5024,0.5477,0.59695,0.6462,0.7078/)
DOUBLE PRECISION,DIMENSION(NRH) :: ROCEANIC=(/0.1666,0.1711,0.2041,0.26105,0.3180,0.34915,0.3803,0.4606/)
DOUBLE PRECISION,DIMENSION(NRH) :: RURBANF=(/0.025378,0.02563,0.02911,0.032125,0.03514,0.038505,0.04187,0.04904/)
DOUBLE PRECISION,DIMENSION(NRH) :: RURBANC=(/0.40678,0.4113,0.4777,0.5291,0.5805,0.6433,0.7061,0.8634/)
DOUBLE PRECISION,DIMENSION(NRH) :: RZIAF=(/0.150, 0.152, 0.158, 0.167,0.187,0.204, 0.221, 0.246/)

! The RZIAC values here are different from Table 4 of Ahmad, et al., Applied Optics, 2010
...
! This is commented out on 10/29/2025, as the smaller RZIAC data lead to artifacts near backscatter.
! Further information is needed from GSFC to confirm which option to use.
!DOUBLE PRECISION,DIMENSION(NRH) :: RZIAC=(/2.1969,2.2293,2.6343,3.1329,3.5694,3.8187,4.1742,4.9941/)

!Table 4 of Ahmad, et al., Applied Optics, 2010
DOUBLE PRECISION,DIMENSION(NRH) :: RZIAC=(/2.441,2.477,2.927,3.481,3.966,4.243,4.638,5.549/)


DOUBLE PRECISION,DIMENSION(NRH) :: RATIO_WETDRY_ZIAF=(/1.006,1.0190,1.0630,1.118,1.2550,1.371, 1.4860, 1.6480/)
DOUBLE PRECISION,DIMENSION(NRH) :: RATIO_WETDRY_ZIAC=(/1.009,1.0240,1.2100,1.439,1.6390, 1.753, 1.9170, 2.2930/)
DOUBLE PRECISION,DIMENSION(NFINEMODE) :: RATIO_FINE_MODE_ZIA=(/0.0, 0.01, 0.02, 0.05, 0.1, 0.2,&
                                       0.3, 0.5, 0.8, 0.95/) ! from personal communication with Zia
!standard deviation
DOUBLE PRECISION,PARAMETER :: SIGF_SF=0.35*LOG(10.0),SIGC_SF=0.4*LOG(10.0),&
                    SIGF_ZIA=0.437,SIGC_ZIA=0.672

DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: MR_TRPOSPHERE,MI_TRPOSPHERE,MR_RURALC,MI_RURALC,&
...
DOUBLE PRECISION,DIMENSION(:), ALLOCATABLE::MR_ZIACR,MI_ZIACR, MR_FLEXF, MR_FLEXC, MI_FLEXF,MI_FLEXC
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::MR_SOOTF_TMP1,MI_SOOTF_TMP1,MR_DUSTF_TMP1,MI_DUSTF_TMP1,&
...
SUBROUTINE WV_REF_ASSIGN(WAVELENGTH_MICRON)
DOUBLE PRECISION, INTENT(IN) ::WAVELENGTH_MICRON
WV_REF=WAVELENGTH_MICRON
ENDSUBROUTINE WV_REF_ASSIGN

SUBROUTINE ARSL_INDX_INIT(NWV, WV)
IMPLICIT NONE
INTEGER, INTENT(IN) :: NWV
DOUBLE PRECISION,DIMENSION(NWV),INTENT(IN)::WV

DOUBLE PRECISION,DIMENSION(NRH) ::RATIO_WETDRY_TROPO,RATIO_WETDRY_RURALC,RATIO_WETDRY_OCEANIC,&
                        RATIO_WETDRY_URBANF,RATIO_WETDRY_URBANC
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::MR_WATER_SOLUBLE,MI_WATER_SOLUBLE,MR_DUST,MI_DUST,MR_SALT,MI_SALT,&
                      MR_SOOT,MI_SOOT,MR_WATER,MI_WATER,MR_BRC,MI_BRC
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::MR_TROPO_RH0,MI_TROPO_RH0,MR_URBAN_RH0,MI_URBAN_RH0, &
	 MR_FLEX_RH0_F, MI_FLEX_RH0_F, MR_FLEX_RH0_C, MI_FLEX_RH0_C
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE::MR_SOOTF_TMP,MI_SOOTF_TMP,MR_DUSTF_TMP,MI_DUSTF_TMP, &
	 MR_WATER_SOLUBLE_TMP,MI_WATER_SOLUBLE_TMP,MR_BRC_TMP, MI_BRC_TMP
DOUBLE PRECISION :: WV_tmp
...
IF(WV_REF<0.0d0) stop 'WV_REF is not assigned'
...
DO IWV=1,NWV+1
   ...
   ELSE
      WV_tmp=WV(IWV);
   ENDIF
   MR_WATER_SOLUBLE(IWV)=Func_UVIP3P(NWV_SF,WV_SF,MR_WATER_SOLUBLE_SF,WV_tmp,2)
   MI_WATER_SOLUBLE(IWV)=Func_UVIP3P(NWV_SF,WV_SF,MI_WATER_SOLUBLE_SF,WV_tmp,2)

   MR_DUST(IWV)=Func_UVIP3P(NWV_SF,WV_SF,MR_DUST_SF,WV_tmp,2)
   MI_DUST(IWV)=Func_UVIP3P(NWV_SF,WV_SF,MI_DUST_SF,WV_tmp,2)

   MR_SALT(IWV)=Func_UVIP3P(NWV_SF,WV_SF,MR_SALT_SF,WV_tmp,2)
   MI_SALT(IWV)=EXP(Func_UVIP3P(NWV_SF,WV_SF,LOG(MI_SALT_SF),WV_tmp,2))

   MR_SOOT(IWV)=Func_UVIP3P(NWV_SF,WV_SF,MR_SOOT_SF,WV_tmp,2)
   MI_SOOT(IWV)=Func_UVIP3P(NWV_SF,WV_SF,MI_SOOT_SF,WV_tmp,2)

   MR_WATER(IWV)=Func_UVIP3P(NWV_SF,WV_SF,MR_WATER_SF,WV_tmp,2)
   MI_WATER(IWV)=Func_UVIP3P(NWV_SF,WV_SF,MI_WATER_SF,WV_tmp,2)

   MR_BRC(IWV)=Func_UVIP3P(NWV_JM,WV_JM,MR_BRC_JM,WV_tmp,2)
   IF (WV_tmp < 0.375D0) THEN
	  MI_BRC(IWV)=K_KK_375*(0.375D0/WV(IWV))**5.7D0
   ELSEIF (0.375D0 < WV_tmp .AND. WV_tmp < 0.532d0) THEN
	  V_0=C/WV_0*1D6
	  V=C/WV(IWV)*1D6
	  MI_BRC(IWV)=ALPHA*GAMMA_KK*V/((V_0**2-V**2)**2+(GAMMA_KK*V)**2)
   ELSEIF (WV_tmp >0.532d0) THEN
	  MI_BRC(IWV)=K_KK_532
   ENDIF
...
SUBROUTINE ARSL_REF_INDX_INIT_IMIX(NWV,WV,rhi,r0f,r0c, rf1,rf2,rf3,nmode)
  ...
  DOUBLE PRECISION, DIMENSION(NWV),INTENT(IN)::WV
  DOUBLE PRECISION,  INTENT(IN) :: rf1,rf2,rf3,RHI,R0F,R0C
  DOUBLE PRECISION :: RRH_F,RRH_C
  DOUBLE PRECISION :: WDRF,WDRC
  DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::MR_WATER_SOLUBLE,MI_WATER_SOLUBLE,MR_DUST,MI_DUST,MR_SALT,MI_SALT,&
       MR_SOOT,MI_SOOT,MR_WATER,MI_WATER,MI_BRC,MR_BRC
  DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE:: MR_FLEX_RH0_F, MI_FLEX_RH0_F, MR_FLEX_RH0_C, MI_FLEX_RH0_C
  DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::MR_SOOTF_TMP,MI_SOOTF_TMP,MR_DUSTF_TMP,MI_DUSTF_TMP, &
       MR_WATER_SOLUBLE_TMP,MI_WATER_SOLUBLE_TMP,MR_BRC_TMP, MI_BRC_TMP
  DOUBLE PRECISION :: WV_tmp
  DOUBLE PRECISION :: Func_UVIP3P
...
SUBROUTINE ARSL_REF_INDX_INIT_EMIX(NWV,WV,RHI,R0DL,R0WS,R0BC,R0S,R0SS)
  ...
  INTEGER, INTENT(IN) :: NWV
  DOUBLE PRECISION, DIMENSION(NWV),INTENT(IN)::WV
  DOUBLE PRECISION, INTENT(IN) :: R0DL,R0WS,R0BC,R0S,R0SS,RHI
  DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::MR_WATER_SOLUBLE,MI_WATER_SOLUBLE,MR_DUST,MI_DUST,MR_SALT,MI_SALT,&
       MR_SOOT,MI_SOOT,MR_WATER,MI_WATER,MI_BRC,MR_BRC
  DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE:: MR_FLEX_RH0_F, MI_FLEX_RH0_F, MR_FLEX_RH0_C, MI_FLEX_RH0_C
  DOUBLE PRECISION :: WV_tmp
  DOUBLE PRECISION :: Func_UVIP3P
  INTEGER :: IWV
  DOUBLE PRECISION :: RRH_DL,RRH_WS,RRH_BC,RRH_S,RRH_SS, RRH_F,RRH_C
  DOUBLE PRECISION :: WDRDL,WDRWS,WDRBC,WDRS,WDRSS
...
SUBROUTINE ARSL_REF_INDX_INIT_IMIX2(NWV,WV,RHI,R0DL,R0WS,R0BC,R0S,R0SS, RF1,RF2,RF3)
  ...
  INTEGER, INTENT(IN) :: NWV
  DOUBLE PRECISION, DIMENSION(NWV),INTENT(IN)::WV
  DOUBLE PRECISION, INTENT(IN) :: R0DL,R0WS,R0BC,R0S,R0SS,RHI, RF1, RF2, RF3
  DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::MR_WATER_SOLUBLE,MI_WATER_SOLUBLE,MR_DUST,MI_DUST,MR_SALT,MI_SALT,&
       MR_SOOT,MI_SOOT,MR_WATER,MI_WATER,MI_BRC,MR_BRC
  DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE:: MR_FLEX_RH0_F, MI_FLEX_RH0_F, MR_FLEX_RH0_C, MI_FLEX_RH0_C
  DOUBLE PRECISION :: WV_tmp
...
SUBROUTINE HYGROSCOPIC_GROWTH_IMIX(R0F,R0C,RHI, RRH_F,RRH_C)
  ...
  DOUBLE PRECISION,DIMENSION(NRH_ZIA) :: WETDRYRF_ZIA, WETDRYRC_ZIA, RH_ZIA
  DOUBLE PRECISION, INTENT(IN) :: R0F,R0C,RHI
  DOUBLE PRECISION :: GAMMA_F, GAMMA_C
  DOUBLE PRECISION, INTENT(OUT) :: RRH_F,RRH_C
  DOUBLE PRECISION,DIMENSION(NRH_ZIA) :: GAMMA_FA, GAMMA_CA
  INTEGER :: IRH
  DOUBLE PRECISION :: Func_UVIP3P
  ...
  IF (RHI >= 0.7) THEN
     GAMMA_F = Func_UVIP3P(6, RH_ZIA(3:), GAMMA_FA(3:),RHI,3)
     GAMMA_C = Func_UVIP3P(6, RH_ZIA(3:), GAMMA_CA(3:),RHI,3)
  ELSEIF (RHI < 0.7) THEN
     GAMMA_F = Func_UVIP3P(3, RH_ZIA(:3), GAMMA_FA(:3),RHI,2)
     GAMMA_C = Func_UVIP3P(3, RH_ZIA(:3), GAMMA_CA(:3),RHI,2)
  END IF
...
SUBROUTINE  HYGROSCOPIC_GROWTH_EMIX(R0DL,R0WS,R0BC,R0S,R0SS,RHI, RRH_DL,RRH_WS,RRH_BC,RRH_S,RRH_SS)
  ...
  !DOUBLE PRECISION :: GAMMA_SS, GAMMA_WS
  DOUBLE PRECISION :: GAMMA_SS, GAMMA_WS, GAMMA_S, GAMMA_BC
  DOUBLE PRECISION, INTENT(IN) :: RHI,R0DL,R0WS,R0BC,R0S,R0SS
  !DOUBLE PRECISION :: GAMMAWS, GAMMASS
  DOUBLE PRECISION :: GAMMAWS, GAMMASS,GAMMAS, GAMMABC
  DOUBLE PRECISION, INTENT(OUT) :: RRH_DL,RRH_WS,RRH_BC,RRH_S,RRH_SS
  !DOUBLE PRECISION,DIMENSION(NRHV) :: GAMMA_WSA, GAMMA_SSA
  DOUBLE PRECISION,DIMENSION(NRHV) :: GAMMA_WSA, GAMMA_SSA,GAMMA_SA, GAMMA_BCA
  INTEGER :: IRH
  DOUBLE PRECISION :: Func_UVIP3P
  !DOUBLE PRECISION, DIMENSION(NRHV)::RRHWS,RRHSS, RHV, WDRWSA,WDRSSA
  DOUBLE PRECISION, DIMENSION(NRHV)::RRHWS,RRHSS, RHV, WDRWSA,WDRSSA,WDRSA,WDRBCA
  DOUBLE PRECISION, PARAMETER :: VWS = 0.806D0, VSS = 0.708D0
  ...
  WDRSA=(/1D0,1D0,1.2D0,1.4D0,1.5D0,1.7D0,1.9D0/)
  WDRBCA=(/1.2D0,1.4D0,1.5D0,1.6D0,1.8D0,2D0,2.2D0/)

  DO IRH = 1,NRHV
     GAMMA_WSA(IRH) = 3.0*LOG(WDRWSA(IRH))/LOG(1/(1-RHV(IRH)))
     GAMMA_SSA(IRH) = 3.0*LOG(WDRSSA(IRH))/LOG(1/(1-RHV(IRH)))
     GAMMA_SA(IRH) = 3.0*LOG(WDRSA(IRH))/LOG(1/(1-RHV(IRH)))
     GAMMA_BCA(IRH) = 3.0*LOG(WDRBCA(IRH))/LOG(1/(1-RHV(IRH)))
       END DO

  IF(RHI >= 0.9) THEN
     GAMMA_WS = Func_UVIP3P(5, RHV(3:), GAMMA_WSA(3:),RHI,2)
     GAMMA_S = Func_UVIP3P(5, RHV(3:), GAMMA_SA(3:),RHI,2)
     GAMMA_BC = Func_UVIP3P(5, RHV(3:), GAMMA_BCA(3:),RHI,2)
  ELSEIF(RHI<0.9) THEN
     GAMMA_WS = Func_UVIP3P(3, RHV(:3), GAMMA_WSA(:3),RHI,1)
     GAMMA_S = Func_UVIP3P(3, RHV(:3), GAMMA_SA(:3),RHI,1)
     GAMMA_BC = Func_UVIP3P(3, RHV(:3), GAMMA_BCA(:3),RHI,1)
  ENDIF

  
  GAMMA_SS = Func_UVIP3P(NRHV, RHV(:), GAMMA_SSA(:),RHI,2)
  RRH_WS = ((1/(1-RHI))**GAMMA_WS)**(1.0d0/3.0d0)*R0WS
  RRH_SS = ((1/(1-RHI))**GAMMA_SS)**(1.0d0/3.0d0)*R0SS
  RRH_S = ((1/(1-RHI))**GAMMA_S)**(1.0d0/3.0d0)*R0S
  RRH_BC = ((1/(1-RHI))**GAMMA_BC)**(1.0d0/3.0d0)*R0BC
  RRH_DL = R0DL
  !RRH_BC = R0BC
  !RRH_S = R0S
