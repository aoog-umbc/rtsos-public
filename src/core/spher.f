...
C          [Eq. (5.245) of Ref. 1]                                        
C               AA=a        ! == effective radius
C               BB=b        ! == effective variance
...
C        180*(I-1)/(NPNA-1) (DEGREES), WHERE I NUMBERS THE ANGLES
     ...
     &                 NANGMIE_LOCAL,PHMXMIE_LOCAL)
      ...
      PARAMETER (NMIE=50000, NPL=2*NMIE, NPNA=721)
...
!      PRINT 1003
 1003 FORMAT(' ',5X,'<',11X,'F11',6X,'     F33',6X,'     F12',
     &  6X,'     F34')
      ...
      NANGMIE_LOCAL=N-1
      IF(NANGMIE_LOCAL>NUMMIEANGMAX)
     &         STOP 'NANGMIE_LOCAL>NUMMIEANGMAX, STOP'
      PHMXMIE_LOCAL=0.0D0
!      DN=1D0/DFLOAT(N-1)
!      DA=DACOS(-1D0)*DN
!      DB=180D0*DN
!      TB=-DB
!      TAA=-DA
! modified by Pengwang Zhai
      DN=1D0/DFLOAT(NPNA-1)
      DA=DACOS(-1D0)*(1.0d0-5.0d0/180.0d0)*DN
      DAA=DACOS(-1D0)*5.0d0/180.0d0*DN
      DB=(180D0-5.0d0)*DN
      DBB=5.0d0*DN
      TB=-DBB
      TAA=-DAA
 ...
 350        IF(L.LT.2) GO TO 400
            ...
            P3=PP3
            PP3=P
            P=(PL1*U*PP4-DSQRT(DFLOAT(L*L-4))*P4)/DSQRT(L1*L1-4.0D0)
            P4=PP4
            PP4=P
 400    CONTINUE
        F33=(F2-F3)*0.5D0
...
!         WRITE (10,1004) TB,F11,F33,F12,F34
       PHMXMIE_LOCAL(I1,0)=TB
       PHMXMIE_LOCAL(I1,1)=F11
       PHMXMIE_LOCAL(I1,2)=F11
       PHMXMIE_LOCAL(I1,3)=F33
       PHMXMIE_LOCAL(I1,4)=F33
       PHMXMIE_LOCAL(I1,5)=F12
       PHMXMIE_LOCAL(I1,6)=F34
  500  CONTINUE
 1004  FORMAT(' ',F8.4,1X,6(1X,ES16.8))
       RETURN
       END
...
!     &       ' gamma=',F7.3)
      ...
      ENDDO                                                                     
      GOTO 400
C     ADDED BY ZHAI, PENGWANG
  390 CONTINUE
 1010 FORMAT('JUNGE DISTRIBUTION SPOW=-',F4.2)
      SPOW=BB
      DO 255 I=1,NNK
         X=YY(I)                                                                
         WY(I)=WY(I)/(X**SPOW)
  255 CONTINUE                                                                  
C ADDITION OVER BY ZHAI, PENGWANG
