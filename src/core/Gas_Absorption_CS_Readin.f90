...
REAL*8,PARAMETER :: NAVOGADRO=6.02214D23, GAS_CONSTANT=8.31
...
REAL*8,DIMENSION(NWV_H2O_ABS_LUT)::WV_H2O_ABS_LUT
REAL*8,DIMENSION(NPGRID_H2O_LUT):: PGRID_H2O_LUT,TEMREF_H2O_LUT,H2OVMR_REF_LUT
REAL*8,DIMENSION(NTEM_PERT_LUT)::TEM_PERT_LUT
REAL*8,DIMENSION(NH2OVMR_PERT)::H2OVMR_PERT
REAL*4,DIMENSION(NPGRID_H2O_LUT,NWV_H2O_ABS_LUT,NH2OVMR_PERT,NTEM_PERT_LUT):: &
                H2O_ABS_COEFF_LUT

REAL*8,DIMENSION(NPGRID_H2O_LUT,NTEM_PERT_LUT)::TEMGRID_H2O_LUT
REAL*8,DIMENSION(NPGRID_H2O_LUT,NH2OVMR_PERT):: H2OVMR_GRID_LUT
REAL*8,DIMENSION(2),PARAMETER :: WAVELENGTH_LUT_RANGE_H2O=(/396.0D0,2300.0D0/)
...
INTEGER :: hdferr
TYPE(C_PTR) :: f_ptr
REAL*8,DIMENSION(:),TARGET,ALLOCATABLE :: HDF5RARRD1
REAL*4,DIMENSION(:,:,:,:),TARGET,ALLOCATABLE :: HDF5RARRD4
...
H2O_ABS_FILEDIR=trim(atmos_dir)//'/H2O/'
H2O_ABS_FILENAME='PACE_H2O_Abs_396to2300_Single_LUT.h5'
...
REAL*8,INTENT(IN),DIMENSION(NPRESSURE)::PRESSURE,TEMPERATURE, &
                                        NUMDEN_TOTAL_FIELD,H2OVMR_FIELD
REAL*8,INTENT(INOUT),DIMENSION(NWAVELENGTH)::WAVELENGTH ! IN NANOMETER

REAL*8,INTENT(INOUT),DIMENSION(NWAVELENGTH,NPRESSURE)::GAS_ABS_COEFF

INTEGER :: IPRESSURE,IWV,MPL

INTEGER :: KLOPRESSURE,KLOWAVELENGTH,KLOVMR1,KLOVMR2,KLOTEMP1,KLOTEMP2,IULO

REAL*8 :: NUMDEN !,CONV_FAC

REAL*8, ALLOCATABLE,DIMENSION(:)::XKT,XKP,XKVMR

REAL*8 ::ABSCOEFF1,ABSCOEFF2,ABSCOEFF3,ABSCOEFF4,ABSCOEFF5,ABSCOEFF6,RTMP

REAL*8, DIMENSION(2)::XK,VK
...
!  write(*,*)'PRESSURE,H2O NUMDEN=',PRESSURE(IPRESSURE),NUMDEN
  IULO=INT(FLOAT(NPGRID_H2O_LUT)*ABS((PRESSURE(IPRESSURE)-PGRID_H2O_LUT(1)) &
                    /(PGRID_H2O_LUT(NPGRID_H2O_LUT)-PGRID_H2O_LUT(1))))
  call hunt(PGRID_H2O_LUT,NPGRID_H2O_LUT,PRESSURE(IPRESSURE),IULO)
  KLOPRESSURE=min(max(IULO-(MPL-1)/2,1),NPGRID_H2O_LUT+1-MPL)

  XKVMR=H2OVMR_GRID_LUT(KLOPRESSURE,:)
  IULO=INT(FLOAT(NH2OVMR_PERT)*ABS((H2OVMR_FIELD(IPRESSURE)-XKVMR(1)) &
         /(XKVMR(NH2OVMR_PERT)-XKVMR(1))))
  call hunt(XKVMR,NH2OVMR_PERT,H2OVMR_FIELD(IPRESSURE),IULO)
  ...
  IULO=INT(FLOAT(NTEM_PERT_LUT)*ABS((TEMPERATURE(IPRESSURE)-XKT(1)) &
         /(XKT(NTEM_PERT_LUT)-XKT(1))))
  call hunt(XKT,NTEM_PERT_LUT,TEMPERATURE(IPRESSURE),IULO)
  ...
  DO IWV=1,NWAVELENGTH
        IF(WAVELENGTH(IWV)<WAVELENGTH_LUT_RANGE_H2O(1) .OR.  &
           WAVELENGTH(IWV)>WAVELENGTH_LUT_RANGE_H2O(2)) THEN
            CYCLE
        ENDIF
        IULO=INT(FLOAT(NWV_H2O_ABS_LUT)*ABS((WAVELENGTH(IWV)-WV_H2O_ABS_LUT(1)) &
        /(WV_H2O_ABS_LUT(NWV_H2O_ABS_LUT)-WV_H2O_ABS_LUT(1))))
        call hunt(WV_H2O_ABS_LUT,NWV_H2O_ABS_LUT,WAVELENGTH(IWV),IULO)
        ...
        CALL POLINT(XK,VK,MPL,WAVELENGTH(IWV),ABSCOEFF1,RTMP)
        ...
        CALL POLINT(XK,VK,MPL,WAVELENGTH(IWV),ABSCOEFF2,RTMP)

        XK(1)=TEMGRID_H2O_LUT(KLOPRESSURE,KLOTEMP1)
        XK(2)=TEMGRID_H2O_LUT(KLOPRESSURE,KLOTEMP1+1)
        VK(1)=ABSCOEFF1
        VK(2)=ABSCOEFF2
        CALL POLINT(XK,VK,MPL,TEMPERATURE(IPRESSURE),ABSCOEFF3,RTMP)
        ...
        VK(1)=ABSCOEFF1
        VK(2)=ABSCOEFF2
        CALL POLINT(XK,VK,MPL,TEMPERATURE(IPRESSURE),ABSCOEFF4,RTMP)

        XK(1)=H2OVMR_GRID_LUT(KLOPRESSURE,KLOVMR1)
        XK(2)=H2OVMR_GRID_LUT(KLOPRESSURE,KLOVMR1+1)
        VK(1)=ABSCOEFF3
        VK(2)=ABSCOEFF4
        CALL POLINT(XK,VK,MPL,H2OVMR_FIELD(IPRESSURE),ABSCOEFF5,RTMP)
        ...
        VK(1)=ABSCOEFF3
        VK(2)=ABSCOEFF4
        CALL POLINT(XK,VK,MPL,H2OVMR_FIELD(IPRESSURE),ABSCOEFF6,RTMP)

        XK(1)=PGRID_H2O_LUT(KLOPRESSURE)
        XK(2)=PGRID_H2O_LUT(KLOPRESSURE+1)
        VK(1)=ABSCOEFF5
        VK(2)=ABSCOEFF6
        CALL POLINT(XK,VK,MPL,PRESSURE(IPRESSURE),ABSCOEFF1,RTMP)
...
REAL*8,DIMENSION(NWV_CO2_ABS_LUT)::WV_CO2_ABS_LUT
REAL*8,DIMENSION(NPGRID_CO2_LUT):: PGRID_CO2_LUT,TEMREF_CO2_LUT,CO2VMR_REF_LUT
REAL*8,DIMENSION(NTEM_PERT_LUT)::TEM_PERT_LUT
REAL*4,DIMENSION(NPGRID_CO2_LUT,NWV_CO2_ABS_LUT,1,NTEM_PERT_LUT):: &
                CO2_ABS_COEFF_LUT

REAL*8,DIMENSION(NPGRID_CO2_LUT,NTEM_PERT_LUT)::TEMGRID_CO2_LUT
REAL*8,DIMENSION(2),PARAMETER :: WAVELENGTH_LUT_RANGE_CO2=(/781.0D0,2300.0D0/)
CONTAINS
...
CO2_ABS_FILEDIR=trim(atmos_dir)//'/CO2/'
CO2_ABS_FILENAME='PACE_CO2_Abs_781to2300_Single_LUT.h5'
...
IMPLICIT NONE
INTEGER,INTENT(IN) :: NPRESSURE,NWAVELENGTH
REAL*8,INTENT(IN),DIMENSION(NPRESSURE)::PRESSURE,TEMPERATURE,NUMDEN_TOTAL_FIELD,CO2VMR_FIELD
REAL*8,INTENT(INOUT),DIMENSION(NWAVELENGTH)::WAVELENGTH ! IN NANOMETER
...
REAL*8, ALLOCATABLE,DIMENSION(:)::XKT,XKP
...
!  write(*,*)'PRESSURE,CO2 NUMDEN=',PRESSURE(IPRESSURE),NUMDEN
  IULO=INT(FLOAT(NPGRID_CO2_LUT)*ABS((PRESSURE(IPRESSURE)-PGRID_CO2_LUT(1)) &
                    /(PGRID_CO2_LUT(NPGRID_CO2_LUT)-PGRID_CO2_LUT(1))))
  call hunt(PGRID_CO2_LUT,NPGRID_CO2_LUT,PRESSURE(IPRESSURE),IULO)
  ...
  DO IWV=1,NWAVELENGTH
        ...
        IULO=INT(FLOAT(NWV_CO2_ABS_LUT)*ABS((WAVELENGTH(IWV)-WV_CO2_ABS_LUT(1)) &
                /(WV_CO2_ABS_LUT(NWV_CO2_ABS_LUT)-WV_CO2_ABS_LUT(1))))
        call hunt(WV_CO2_ABS_LUT,NWV_CO2_ABS_LUT,WAVELENGTH(IWV),IULO)
...
REAL*8,DIMENSION(NWV_CH4_ABS_LUT)::WV_CH4_ABS_LUT
REAL*8,DIMENSION(NPGRID_CH4_LUT):: PGRID_CH4_LUT,TEMREF_CH4_LUT,CH4VMR_REF_LUT
REAL*8,DIMENSION(NTEM_PERT_LUT)::TEM_PERT_LUT
REAL*4,DIMENSION(NPGRID_CH4_LUT,NWV_CH4_ABS_LUT,1,NTEM_PERT_LUT):: &
                CH4_ABS_COEFF_LUT

REAL*8,DIMENSION(NPGRID_CH4_LUT,NTEM_PERT_LUT)::TEMGRID_CH4_LUT
REAL*8,DIMENSION(2),PARAMETER :: WAVELENGTH_LUT_RANGE_CH4=(/1084.D0,2300.0D0/)
CONTAINS
...
CH4_ABS_FILEDIR=trim(atmos_dir)//'/CH4/'
CH4_ABS_FILENAME='PACE_CH4_Abs_1084to2300_Single_LUT.h5'
...
IMPLICIT NONE
INTEGER,INTENT(IN) :: NPRESSURE,NWAVELENGTH
REAL*8,INTENT(IN),DIMENSION(NPRESSURE)::PRESSURE,TEMPERATURE,NUMDEN_TOTAL_FIELD,CH4VMR_FIELD
REAL*8,INTENT(INOUT),DIMENSION(NWAVELENGTH)::WAVELENGTH ! IN NANOMETER
...
!  write(*,*)'PRESSURE,CH4 NUMDEN=',PRESSURE(IPRESSURE),NUMDEN
  IULO=INT(FLOAT(NPGRID_CH4_LUT)*ABS((PRESSURE(IPRESSURE)-PGRID_CH4_LUT(1)) &
                    /(PGRID_CH4_LUT(NPGRID_CH4_LUT)-PGRID_CH4_LUT(1))))
  call hunt(PGRID_CH4_LUT,NPGRID_CH4_LUT,PRESSURE(IPRESSURE),IULO)
  ...
  DO IWV=1,NWAVELENGTH
        ...
        IULO=INT(FLOAT(NWV_CH4_ABS_LUT)*ABS((WAVELENGTH(IWV)-WV_CH4_ABS_LUT(1)) &
                /(WV_CH4_ABS_LUT(NWV_CH4_ABS_LUT)-WV_CH4_ABS_LUT(1))))
        call hunt(WV_CH4_ABS_LUT,NWV_CH4_ABS_LUT,WAVELENGTH(IWV),IULO)
...
REAL*8,DIMENSION(NWV_OXYGEN2_ABS_LUT)::WV_OXYGEN2_ABS_LUT
REAL*8,DIMENSION(NPGRID_OXYGEN2_LUT):: PGRID_OXYGEN2_LUT,TEMREF_OXYGEN2_LUT,OXYGEN2VMR_REF_LUT
REAL*8,DIMENSION(NTEM_PERT_LUT)::TEM_PERT_LUT
REAL*4,DIMENSION(NPGRID_OXYGEN2_LUT,NWV_OXYGEN2_ABS_LUT,1,NTEM_PERT_LUT):: &
                OXYGEN2_ABS_COEFF_LUT

REAL*8,DIMENSION(NPGRID_OXYGEN2_LUT,NTEM_PERT_LUT)::TEMGRID_OXYGEN2_LUT

REAL*8,DIMENSION(2,7):: WAVELENGTH_SEG_ENDPOINTS
...
OXYGEN2_ABS_FILEDIR=trim(atmos_dir)//'/O2/HITRAN2012/'
...
DO ISEG=1,7
    ...
    OXYGEN2_ABS_FILENAME='PACE_O2_Abs_'//TRIM(CFILETMPS)//'to'//TRIM(CFILETMPE)//'_Single_LUT.h5'
...
IMPLICIT NONE
INTEGER,INTENT(IN) :: NPRESSURE,NWAVELENGTH
REAL*8,INTENT(IN),DIMENSION(NPRESSURE)::PRESSURE,TEMPERATURE,NUMDEN_TOTAL_FIELD,OXYGEN2VMR_FIELD
REAL*8,INTENT(INOUT),DIMENSION(NWAVELENGTH)::WAVELENGTH ! IN NANOMETER
...
!  write(*,*)'PRESSURE,OXYGEN2 NUMDEN=',PRESSURE(IPRESSURE),NUMDEN
  IULO=INT(FLOAT(NPGRID_OXYGEN2_LUT)*ABS((PRESSURE(IPRESSURE)-PGRID_OXYGEN2_LUT(1)) &
                    /(PGRID_OXYGEN2_LUT(NPGRID_OXYGEN2_LUT)-PGRID_OXYGEN2_LUT(1))))
  call hunt(PGRID_OXYGEN2_LUT,NPGRID_OXYGEN2_LUT,PRESSURE(IPRESSURE),IULO)
  ...
  DO IWV=1,NWAVELENGTH
        ...
        IULO=INT(FLOAT(NWV_OXYGEN2_ABS_LUT)*ABS((WAVELENGTH(IWV)-WV_OXYGEN2_ABS_LUT(1)) &
                /(WV_OXYGEN2_ABS_LUT(NWV_OXYGEN2_ABS_LUT)-WV_OXYGEN2_ABS_LUT(1))))
        call hunt(WV_OXYGEN2_ABS_LUT,NWV_OXYGEN2_ABS_LUT,WAVELENGTH(IWV),IULO)
...
INTEGER,PARAMETER :: NPGRID_OZONE_LUT=1,NWV_OZONE_ABS_LUT=15503

REAL*8,DIMENSION(NWV_OZONE_ABS_LUT)::WV_OZONE_ABS_LUT
REAL*8,DIMENSION(NPGRID_OZONE_LUT):: PGRID_OZONE_LUT,TEMREF_OZONE_LUT,OZONEVMR_REF_LUT

REAL*8, DIMENSION(NWV_OZONE_ABS_LUT) :: C0_ARRAY_OZONE,C1_ARRAY_OZONE,C2_ARRAY_OZONE
REAL*8,DIMENSION(2),PARAMETER :: WAVELENGTH_LUT_RANGE_OZONE=(/116.650D0,850.0D0/)
...
IMPLICIT NONE
INTEGER,INTENT(IN) :: NPRESSURE,NWAVELENGTH
REAL*8,INTENT(IN),DIMENSION(NPRESSURE)::PRESSURE,TEMPERATURE,NUMDEN_TOTAL_FIELD,OZONEVMR_FIELD
REAL*8,INTENT(INOUT),DIMENSION(NWAVELENGTH)::WAVELENGTH ! IN NANOMETER
...
REAL*8 ::ABS_CRS_M2,ABSCOEFF1,ABSCOEFF2,ABSCOEFF3,RTMP,TEMPERATURE_DIFF
...
!  write(*,*)'PRESSURE,OZONE NUMDEN=',PRESSURE(IPRESSURE),NUMDEN
  ...
  DO IWV=1,NWAVELENGTH
        ...
        IULO=INT(FLOAT(NWV_OZONE_ABS_LUT)*ABS((WAVELENGTH(IWV)-WV_OZONE_ABS_LUT(1)) &
                /(WV_OZONE_ABS_LUT(NWV_OZONE_ABS_LUT)-WV_OZONE_ABS_LUT(1))))
        call hunt(WV_OZONE_ABS_LUT,NWV_OZONE_ABS_LUT,WAVELENGTH(IWV),IULO)
        ...
        CALL POLINT(XK,VK,MPL,WAVELENGTH(IWV),ABSCOEFF3,RTMP)

!        ABS_CRS_M2=(ABSCOEFF1+ABSCOEFF2*TEMPERATURE_DIFF + ABSCOEFF3*(TEMPERATURE_DIFF**2)) &
!                    *1.0E-20*1.0E-4
        ABS_CRS_M2=ABSCOEFF1*1.0E-20*1.0E-4
...
REAL*8,DIMENSION(NWV_NO2_ABS_LUT)::WV_NO2_ABS_LUT
REAL*8,DIMENSION(NPGRID_NO2_LUT):: PGRID_NO2_LUT,TEMREF_NO2_LUT,NO2VMR_REF_LUT

REAL*8, DIMENSION(NWV_NO2_ABS_LUT) :: C0_ARRAY_NO2,C1_ARRAY_NO2,C2_ARRAY_NO2
REAL*8,DIMENSION(2),PARAMETER :: WAVELENGTH_LUT_RANGE_NO2=(/241.10D0,759.9D0/)
...
IMPLICIT NONE
INTEGER,INTENT(IN) :: NPRESSURE,NWAVELENGTH
REAL*8,INTENT(IN),DIMENSION(NPRESSURE)::PRESSURE,TEMPERATURE,NUMDEN_TOTAL_FIELD,NO2VMR_FIELD
REAL*8,INTENT(INOUT),DIMENSION(NWAVELENGTH)::WAVELENGTH ! IN NANOMETER
...
!              NUMDEN_TOTAL_FIELD(IPRESSURE),NO2VMR_FIELD(IPRESSURE)
  ...
  DO IWV=1,NWAVELENGTH
        ...
        IULO=INT(FLOAT(NWV_NO2_ABS_LUT)*ABS((WAVELENGTH(IWV)-WV_NO2_ABS_LUT(1)) &
                /(WV_NO2_ABS_LUT(NWV_NO2_ABS_LUT)-WV_NO2_ABS_LUT(1))))
        call hunt(WV_NO2_ABS_LUT,NWV_NO2_ABS_LUT,WAVELENGTH(IWV),IULO)
...
REAL*8,DIMENSION(NUM_BAND_SEG) :: WV_O4_ABS_LUT=(/343.4D0,360.5D0,380.2D0,446.7D0,&
                                      477.3D0,532.2D0,577.2D0,630.0D0,1065.2D0/)
REAL*8,DIMENSION(NUM_BAND_SEG) :: CRS_O4_LUT=(/1.20D0,4.10D0,2.4D0,0.57D0,6.3D0,1.0D0,11.0D0,7.2D0,12.0D0/)
REAL*8,DIMENSION(NUM_BAND_SEG) :: FWHM_O4_LUT=(/4.2D0,4.8D0,4.4D0,5.6D0,6.2D0,10.2D0, 11.6D0,13.8D0,27.3D0/)
...
IMPLICIT NONE
INTEGER,INTENT(IN) :: NPRESSURE,NWAVELENGTH
REAL*8,INTENT(IN),DIMENSION(NPRESSURE)::PRESSURE,TEMPERATURE,NUMDEN_TOTAL_FIELD,O4VMR_FIELD
REAL*8,INTENT(INOUT),DIMENSION(NWAVELENGTH)::WAVELENGTH ! IN NANOMETER

REAL*8,INTENT(INOUT),DIMENSION(NWAVELENGTH,NPRESSURE)::GAS_ABS_COEFF

REAL*8 :: NUMDEN !,CONV_FAC
REAL*8 :: WAVELENGTH_DIFF,LN2,FACT1,FACT2

INTEGER :: I_BAND_SEG
INTEGER :: IPRESSURE,IWV,MPL,KLOWAVELENGTH,IULO
REAL*8 ::ABS_CRS_M2,ABSCOEFF1,ABSCOEFF2,ABSCOEFF3,RTMP
