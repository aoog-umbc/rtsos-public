...
MPLIN=4  ! DEFAULT ORDER OF POLYNOMIAL INTERPOLATION FOR RADIANCES
...
IF(OCEAN .AND. WCFLG .AND. M==0)THEN
    IRRAD_A=0.0D0
    ITAU=ALYR(NALYR)%ITAUE
    DO JQUAD=1,NQDAD2
...
ENDIF
  ...
  ENDDO
  IF(IMU>NMUOUTSHFLA)STOP 'CHECK IMU SEARCH 1'
 ...
 IF(SNG_RAD_CORR) THEN !SINGLE SCATTERING CORRECTION WITHOUT MAPPING
  DO ITHETA=1,NTHETAOUT
    DO IMU=1,NMUOUTSHFLA
     IF(ABS(MUOUT(ITHETA)-MUOUTSHFLA(IMU))<1.0E-4)EXIT
    ENDDO
    IF(IMU>NMUOUTSHFLA)STOP 'CHECK IMU SEARCH 2'
...
IF(SNG_RAD_CORR .AND. RAD_MAP)THEN
  DO ITHETA=1,NTHETAOUT
    ...
    ENDDO
    IF(IMU>NMUOUTSHFLO)STOP 'CHECK IMU SEARCH 3'
...
IF(SNG_RAD_CORR)THEN
  ...
  IF(RAMAN_SOURCE_FLAG .OR. FLUORESCENCE_SOURCE_FLAG)THEN
    DO ICOM1=1,4
      LSPO(1:NQUADO,OLYR(1)%ITAUS:NTTAU,0:2)%VRAD(ICOM1) =        &
         LSPO(1:NQUADO,OLYR(1)%ITAUS:NTTAU,0:2)%VRAD(ICOM1) -   &
         LSPMNO_INELASTIC_SNG(1:NQUADO,OLYR(1)%ITAUS:NTTAU,0:2)%VRAD(ICOM1)
    ENDDO
  ENDIF
...
DO IPHI=1,NPHIOUT
  ...
  IF(SNG_RAD_CORR) THEN !SINGLE SCATTERING CORRECTION WITHOUT MAPPING
    ...
    DO IMU=1,NMUOUTSHFLO
     IF(ABS(MUOUT(ITHETA)-MUOUTSHFLO(IMU))<1.0E-4)EXIT
    ENDDO
    IF(IMU>NMUOUTSHFLO)STOP 'CHECK IMU SEARCH 4'
       RADDETU(ITHETA,:)=RADDETU(ITHETA,:) + &
...
IF(DETO)DEALLOCATE(INDXDETO,TAUDETO,INDXDETO_SNGSCAT)
...
IF(OCEAN .AND. (.NOT. FLATO) .AND. DETO)THEN
  ...
  DO M=0,MAXMORD
    ...
    CALL RADINTPL2(MPLIN,NQDOD2,XJO,RADOTMP,NMUOUTSHFLO,MUOUTSHFLO,RADOUTTMP)
...
IF(FLAG1 .AND. DETO .AND. (.NOT. OCEAN_NO_TRAN))THEN
  DO ITHETA=1,NMUOUTSHFLO
    ...
    IF(ABS(COS(THETA1)+MUOUTSHFLA(IMU))>1.0E-4) STOP 'CHECK IMU SEARCH HERE 5'
...
! OCEAN DIFFUSE LIGHT TRANSMITTANCE FOR FLAT OCEAN
  ...
  DO ITHETA=NMUOUTSHFLA/2+1,NMUOUTSHFLA
    ...
    IF(IMU>NMUOUTSHFLO) STOP 'CHECK IMU SEARCH HERE 6'
