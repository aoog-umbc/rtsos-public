...
IF(SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION) PSEUDO_SPHERICAL_SHELL=.TRUE.
...
DO IDTAU=1,NDTAUA  
  ...
  IF(.NOT. WLR_FLAG) THEN
    DO ICOM1=1,4
     LRADA(1:NMUOUTSHFLA,IDTAU,1:NPHIOUT)%VRAD(ICOM1)= &
      LRADA(1:NMUOUTSHFLA,IDTAU,1:NPHIOUT)%VRAD(ICOM1) + &
      LRADA_SNG(1:NMUOUTSHFLA,INDXDETA_SNGSCAT(IDTAU),1:NPHIOUT)%VRAD(ICOM1)
    ENDDO
  ENDIF
ENDDO ! IDTAU LOOP
...
INTEGER :: IMIE,IQUAD,I,IPHI,ITHETA,IDTAU,ILAYER
...
ARRY2(4)=-ARRY2(4)
  ...
  IF(SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION)THEN
      CALL SLANT_PATH_SURF_FSS(I,MU_IN,MUOUTSHFLA(ITHETA),PHIOUT(IPHI),&
                          SECANT_THETAL_LOCAL)
  ELSE
      SECANT_THETAL_LOCAL=SECANT_THETAL_PRSV(I)
  ENDIF
  ...
  IF(ABS(MUOUTSHFLA(ITHETA)-MU_IN)<TINY2)THEN
    ...
    LRADA_SNG(ITHETA,I,IPHI)%VRAD(:)=    &
      RTMP2*LRADA_SNG(ITHETA,I-1,IPHI)%VRAD(:)   +          &
           EXP(-ALYR(I)%TAUT*SECANT_THETAL_LOCAL) *          &
  ...
  ELSE
    ...
    LRADA_SNG(ITHETA,I,IPHI)%VRAD(:)=    &
	  RTMP2*LRADA_SNG(ITHETA,I-1,IPHI)%VRAD(:) + &
        RTMP * EXP(-ALYR(I)%TAUT*SECANT_THETAL_LOCAL)*(1.0D0 -       &
		  EXP(-RTMP1*(1.0D0/MU_IN-1.0D0/MUOUTSHFLA(ITHETA))) ) * ARRY2(:)

!    LRADA_SNG(ITHETA,I,IPHI)%VRAD(:)=    &
!      RTMP2*LRADA_SNG(ITHETA,I-1,IPHI)%VRAD(:) + &
!                    RTMP * (EXP(-ALYR(I)%TAUT*SECANT_THETAL_PRSV(I)) -    &
!      EXP(RTMP1/MUOUTSHFLA(ITHETA))*EXP(-ALYR(I)%TAUB*SECANT_THETAL_PRSV(I-1))) * ARRY2(:)
  ENDIF
...
ARRY2(4)=-ARRY2(4)
  ...
  IF(SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION)THEN
		CALL SLANT_PATH_TOA_FSS(I,MU_IN,MUOUTSHFLA(ITHETA),PHIOUT(IPHI),&
							SECANT_THETAL_LOCAL)
  ELSE
		SECANT_THETAL_LOCAL=SECANT_THETAL_PRSV(I)
  ENDIF
  ...
  LRADA_SNG(ITHETA,I,IPHI)%VRAD(:)=                              &
    RTMP2*LRADA_SNG(ITHETA,I+1,IPHI)%VRAD(:)   + &
     RTMP* EXP(-ABS(ALYR(I+1)%TAUB*SECANT_THETAL_LOCAL)) *            &
