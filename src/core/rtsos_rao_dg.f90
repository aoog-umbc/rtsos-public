...
LOGICAL :: TRUC_FLAG,GEOSR,SGLNT,MUFLG
...
LINEXP=.true.       ! USE THE LINEAR - EXPONENTIAL APPROXIMATION
GEOSR=.true.       ! USE THE GEOMETRIC SERIES APPROXIMATION
SCL = .true.       ! CALCULATED SCALAR RADIANCE

OC_IF_DS=.false.    ! if true do double scattering for ocean interface scattering
                    ! need to be tested in the future
OSFREXP=.false.     ! USE SERIES COMBINATION for THE OCEAN BRDF
...
ENORM_FLAT=.false.       ! ENGERGY RENORMALIZATION according to flat surface
ENGFACNORM=.FALSE.       ! if true, use Energy renormalization for rough surface
...
DO M=1,NREC
  ...
  IF(I>0 .AND. .NOT.OCEAN) THEN
    ...
    IF(PSEUDO_SPHERICAL_SHELL) HEIGHT_ATMOSPHERE(IALYR-1)=RTMP5
...
IF(RAMAN_SOURCE_FLAG .OR. FLUORESCENCE_SOURCE_FLAG)  &
...
IF(FLATO .AND. WCFLG)THEN
     WCFLG=.FALSE.
     WRITE(*,*)'WARNING, WHITE CAP FLAG TURNED OFF'
     WRITE(*,*)'BECAUSE FLAT OCEAN IS USED'
ENDIF
...
!  WCLBDO=0.22D0               ! VALUE OBTAINED IN FRASER, JGR, 1997.
	IF(WNDSPD<6.33D0)THEN
		FWC=0.0D0
	ELSE IF (WNDSPD<12.0D0)THEN
		FWC=8.75E-5*(WNDSPD-6.33D0)**3
	ELSE IF(WNDSPD>12.0D0)THEN
...
CALL ATMOS_OCEAN_CONFIG(TRUC_FLAG)
...
IF(pBRDF) CALL CALPBRDF
IF(pBRDF) CALL CALSPBRDF(MU_IN)
CALL GETSDML(MU_IN)
...
IF((OCEAN .AND. (.NOT.FLATO)) .or. pBRDF )THEN
...
write(*,*)'SNGL_FWDCN_CRRCT_ATMOS elapsed time =', finish_time-start_time
ENDIF
...
IF(pBRDF)DEALLOCATE(SFRES_REFLA)
IF(DETA)DEALLOCATE(LRADA)
IF(SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION) DEALLOCATE(LRADA_SNG,LRADA_SNG_SPSH)
...
DEALLOCATE(BETAL,ALPHAL, ZETAL,DELTAL, GAMMAL,EPSILONL)
...
IF(PSEUDO_SPHERICAL_SHELL)DEALLOCATE(HEIGHT_ATMOSPHERE)
...
IF((OCEAN .AND. (.NOT. FLATO)) .OR. pBRDF)DEALLOCATE(FRES_REFLA)
...
DO IDTAU=1,NDTAUA  
  ...
  IF(.NOT. WLR_FLAG) THEN
    ...
    DO IPHI=1,NPHIOUT
	  ...
	  IF(SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION .AND. &
         LRADA_SNG_SPSH(ITHETA,INDXDETA_SNGSCAT(IDTAU),IPHI)%VRAD(1)>1.0E-6)THEN
...
! testing spherical shell single scattering
!IF((idtau==1 .OR. idtau==NDTAUA) .and. iphi==1)THEN
!write(*,'(5(2x,E12.6))')acos(MUOUTSHFLA(ITHETA))/pi*180.0d0,&
!		  LRADA_SNG(ITHETA,INDXDETA_SNGSCAT(IDTAU),IPHI)%VRAD(1),&
!          LRADA_SNG_SPSH(ITHETA,INDXDETA_SNGSCAT(IDTAU),IPHI)%VRAD(1),&
!          LRADA(ITHETA,IDTAU,IPHI)%VRAD(1),ARRY3(1)
!ENDIF

		 LRADA(ITHETA,IDTAU,IPHI)%VRAD=ARRY3

!testing ratio algorithm
...
SUBROUTINE ATMOS_OCEAN_CONFIG(TRUC_FLAG)
USE RTTYPE
USE RTUTILITY
implicit none
LOGICAL :: TRUC_FLAG
...
INTEGER :: IMIE,IQUAD,I,IPHI,ITHETA,IDTAU,ILAYER,ILAYERSUB
...
REAL*8 :: SECANT_THETAL_VIEWB,SECANT_THETAL_VIEWT,SECANT_THETAL_LAYER,SECANT_THETAL_LOCAL
REAL*8 :: TAUTEMP,TAU_INTERVAL,HEIGHT_TEMP,HEIGHT_INTERVAL,SOURCETEMP,WEIGHT
!INTEGER :: NUMPHI,IPHI_TMP
!REAL*8,DIMENSION(:),ALLOCATABLE :: PHI_TMP,WPHI_TMP
!REAL*8 :: XIFUNC
THETA_IN=ACOS(MU_IN)
IF(SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION) NHEIGHTINT=3
DO I=1,4
...
! convention sign changes.
  ...
  IF(SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION)THEN
	CALL SLANT_PATH_SURF_LAYER(MUOUTSHFLA(ITHETA),I,SECANT_THETAL_LAYER)
    RTMP=ALYR(I)%TAUT-ALYR(I)%TAUB
    RTMP1=DABS(RTMP*SECANT_THETAL_LAYER)
    RTMP2= EXP(-RTMP1)

	HEIGHT_INTERVAL=(HEIGHT_ATMOSPHERE(I)-HEIGHT_ATMOSPHERE(I-1))/(NHEIGHTINT-1.0D0)
	TAU_INTERVAL=(ALYR(I)%TAUT-ALYR(I)%TAUB)/(NHEIGHTINT-1.0D0)
	...
	DO ILAYERSUB=1,NHEIGHTINT
		...
		ELSE
			WEIGHT=1.0D0
		ENDIF
		TAUTEMP=ALYR(I)%TAUB+TAU_INTERVAL*(ILAYERSUB-1)
		HEIGHT_TEMP=HEIGHT_ATMOSPHERE(I)+HEIGHT_INTERVAL*(ILAYERSUB-1)

		CALL SLANT_PATH_SURF_FSS(HEIGHT_TEMP,I,MU_IN,MUOUTSHFLA(ITHETA),PHIOUT(IPHI),&
								 SECANT_THETAL_LOCAL)

		SOURCETEMP=SOURCETEMP+ WEIGHT*EXP(-ABS(TAUTEMP*SECANT_THETAL_LOCAL))* &
							   EXP(-ABS(ALYR(I)%TAUT-TAUTEMP)*SECANT_THETAL_LAYER)
	ENDDO
	LRADA_SNG_SPSH(ITHETA,I,IPHI)%VRAD(:)=                              &
	  RTMP2*LRADA_SNG_SPSH(ITHETA,I-1,IPHI)%VRAD(:)+SOURCETEMP*RTMP1*ARRY2(:)/(NHEIGHTINT-1.0D0)
...
! convention sign changes.
  ...
  IF(SPHERICAL_SHELL_SINGLESCATTERING_CORRECTION)THEN
	  CALL SLANT_PATH_TOA_LAYER(MUOUTSHFLA(ITHETA),I+1,SECANT_THETAL_LAYER)

	  RTMP= ALYR(I+1)%TAUT-ALYR(I+1)%TAUB
	  RTMP1=DABS(RTMP*SECANT_THETAL_LAYER)
	  RTMP2=EXP(-RTMP1)
      ...
      HEIGHT_TEMP=HEIGHT_ATMOSPHERE(I+1)
      SOURCETEMP=0.0D0
      DO ILAYERSUB=1,NHEIGHTINT
          IF(ILAYERSUB==1 .OR. ILAYERSUB==NHEIGHTINT) THEN
              WEIGHT=0.5D0
          ELSE
              WEIGHT=1.0D0
          ENDIF
          TAUTEMP=ALYR(I+1)%TAUT-TAU_INTERVAL*(ILAYERSUB-1)
		  ...
		  CALL SLANT_PATH_TOA_FSS(HEIGHT_TEMP,I+1,MU_IN,MUOUTSHFLA(ITHETA),PHIOUT(IPHI),&
								  SECANT_THETAL_LOCAL)
		  SOURCETEMP=SOURCETEMP+ WEIGHT*EXP(-ABS(TAUTEMP*SECANT_THETAL_LOCAL))* &
                                 EXP(-ABS(ALYR(I+1)%TAUT-TAUTEMP)*SECANT_THETAL_LAYER)
      ENDDO
      LRADA_SNG_SPSH(ITHETA,I,IPHI)%VRAD(:)=                              &
     	RTMP2*LRADA_SNG_SPSH(ITHETA,I+1,IPHI)%VRAD(:)+SOURCETEMP*RTMP1*ARRY2(:)/(NHEIGHTINT-1.0D0)
!if(iphi==1 .and. ITHETA==NMUOUTSHFLA)write(*,*)HEIGHT_ATMOSPHERE(I),SECANT_THETAL_LOCAL,1.0/MU_IN
...
USE RTUTILITY, ONLY : NUMMIEANG,MU_SNG,PM_AO_SNG,NUMMIE,SCL
IMPLICIT NONE
...
DO ITAU=OLYR(I)%ITAUS,OLYR(I)%ITAUE
   ...
   ELSE IF(QSRC(1)>QSRC(2) .AND. QSRC(2)>QSRC(3))THEN
     ...
     RTMP2=(QSRC(2)/QSRC(3))**2-QSRC(1)/QSRC(3)
   ELSE
     WRITE(*,*)'SOURCEF_OCEAN_INELASTIC, Check 1, QSRC=',QSRC
     SQRTSIGN=0.0D0
     RTMP2=0.0D0
   ENDIF
...
ENDDO
   ...
   ELSE IF(QSRC(1)>QSRC(2) .AND. QSRC(2)>QSRC(3))THEN
     ...
     RTMP2=(QSRC(2)/QSRC(3))**2-QSRC(1)/QSRC(3)
   ELSE
     WRITE(*,*)'SOURCEF_OCEAN_INELASTIC, Check 2, QSRC=',QSRC
     SQRTSIGN=0.0D0
     RTMP2=0.0D0
   ENDIF
   TAUDIFF=TAU_INELASTIC_AGD(KLO+2)-TAU_INELASTIC_AGD(KLO)
...
ELSE  ! USE EXPANSION SCHEME IN DISORT REPORT
  ...
  DO ILTERM=MAXLORD+1,NK !  NUMFULLBETAL(IMIE)
    FBAVG=0.0D0
    DO IL_AVG=1,ILAYER
      IMIE=ALYR(IL_AVG)%IPT
      IF(IL_AVG==NALYR)THEN
...
!  ENDIF
  NUMFULLBETAL(IMIE)=ILTERM

  DEALLOCATE(PLNML,TWOX,F1TMP,F2TMP,DTMP,WTANG,PLNMLtmp)
  FULLBETAL(IMIE,0:NUMFULLBETAL(IMIE))=FULLBETAL(IMIE,0:NUMFULLBETAL(IMIE))/&
                                     FULLBETAL(IMIE,0)
ENDDO

!testing
!DO ILTERM=0,NUMFULLBETAL(1)
!  write(*,*)ILTERM,FULLBETAL(1,ILTERM)*(2.0*ILTERM+1)
!enddo
IF(MAXLORD>MAXVAL(NUMFULLBETAL))THEN
  MAXLORD=MAXVAL(NUMFULLBETAL)
  MAXMORD=MAXLORD
ENDIF
...
REAL*8,DIMENSION(NDIM) :: WAVELENGTH_TABLE=(/0.412D0, 0.443D0, 0.490D0, &
           0.510D0, 0.555D0, 0.670D0,0.765D0,0.865D0/)
REAL*8,DIMENSION(NDIM) :: AWC=(/1.0D0,1.0D0,1.0D0,1.0D0,1.0D0, &
           0.889D0,0.760D0, 0.645D0/)
REAL*8 :: AWC_LOCAL
...
WHITECAP_REFLECTANCE=0.22D0*AWC_LOCAL
RETURN
