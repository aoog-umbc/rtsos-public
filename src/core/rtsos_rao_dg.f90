...
DOUBLE PRECISION, INTENT(IN) ::  DELTATAUAINPUT,DELTATAUOINPUT
DOUBLE PRECISION,INTENT(IN),DIMENSION(NREC,7) :: RECDATASTREAM
INTEGER,INTENT(IN),DIMENSION(NUMMIEINPUT) ::NUMMIEANGINPUT
DOUBLE PRECISION,INTENT(IN),DIMENSION(NUMMIEINPUT,NUMMIEANGMAX,0:6) :: PHMXDATASTREAM

INTEGER(I4B),INTENT(IN) :: NTHETAOUT,NPHIOUT
DOUBLE PRECISION, INTENT(IN) :: MU_IN
DOUBLE PRECISION,DIMENSION(NTHETAOUT), INTENT(IN) :: MUOUT
DOUBLE PRECISION,DIMENSION(NPHIOUT), INTENT(IN)  :: PHIOUT
DOUBLE PRECISION,DIMENSION(NDET,2), INTENT(OUT)  :: DIRAD  ! DIRAD(:,1) DOWNWELLING IRADIANCE
                                   ! DIRAD(:,2) UPWELLING IRADIANCE
DOUBLE PRECISION,DIMENSION(NDET,NTHETAOUT,NPHIOUT,4), INTENT(OUT) :: DSTOKES
LOGICAL,INTENT(IN) :: WLR_FLAG_INPUT,WCFLG_INPUT
INTEGER,INTENT(IN) :: DELTAM_INPUT

DOUBLE PRECISION :: THETA_IN

DOUBLE PRECISION, EXTERNAL :: WHITECAP_REFLECTANCE
...
DOUBLE PRECISION :: TAU_COARSE_LIMIT1,TAU_COARSE_LIMIT2,DELTATAU_COARSE1,DELTATAU_COARSE2
...
INTEGER(I4B) :: IMIE,LORDER,I,L,M,N,ICOM,  &
      ...
      IDTAUO,IALYR,IOLYR,ICOM1,Interp_ORDER_IN,NMUOUTSHFLA,NMUOUTSHFLO
...
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: RADATMP,RADATMP1,RADOTMP,RADOTMP1,RADOUTTMP
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: RADDETU,RADDETK

DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XJAD2,XJOD2

TYPE(ARRAY4),DIMENSION(:,:),ALLOCATABLE :: LRADATMP,LSPATMP,LRADATMP1,LSPATMP1
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: MUTMP

INTEGER :: NUMPHI,IPHI_TMP,IMU,MAXMLOCAL
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: PHI_TMP,WPHI_TMP
...
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: MUOUTSHFLA,MUOUTSHFLO
...
Interp_ORDER_IN=3  ! DEFAULT ORDER OF POLYNOMIAL INTERPOLATION FOR RADIANCES
...
DO M=1,NREC
  ...
  IF(I==-100) THEN
    ...
    IF(abs(RTMP5)>1.0d-6)THEN
		IF(.NOT. WCFLG_INPUT) &
		   WRITE(*,*)'SETTING WCFLG=.TRUE. FOR SURFACE DEBRIE REFLECTANCE CALCULATION'
		WCFLG=.TRUE.
		WCLBDO=RTMP4*PI    ! the unit of RTMP4 is the same as Remote Sensing Reflectance RSR
		FWC=RTMP5
		IF(WNDSPD<0.5D0)THEN
		   WRITE(*,*)'RESET WNDSPD=0.5D0 TO AVOID FLATO AND WHITE CAP CONFLICT'
		   WNDSPD=0.5D0
		ENDIF
    ENDIF
...
IF(OCEAN .AND. WCFLG .AND. ABS(FWC)<1.0D-6)THEN
...
call gauss_legendre(X1,X2,XJTMP,GAUWTTMP,NQUADAINPUT/2)
...
IF(OCEAN .AND. (.NOT. OCEAN_NO_TRAN))THEN
 ...
 IF(DOUBLE_GAUSSIAN_QUAD)THEN
	...
	DO IQUAD=1,NQDAD2
		...
		XJO(IQUAD)=-RTMP
		WTO(IQUAD)=WTA(IQUAD)*                     &
			 DABS(XJA(IQUAD)/XJO(IQUAD))/NWRE/NWRE
	ENDDO
	...
	call gauss_legendre(X1,X2,XJTMP,GAUWTTMP,NQUADDIF/2)

	DO IQUAD=1,NQUADDIF/2
		XJO(IQUAD+NQDAD2)=XJTMP(IQUAD)
		WTO(IQUAD+NQDAD2)=GAUWTTMP(IQUAD)
	ENDDO
    DO IQUAD=NQUADO/2+1,NQUADO-1
		XJO(IQUAD)=-XJO(NQUADO-IQUAD+1)
		WTO(IQUAD)=WTO(NQUADO-IQUAD+1)
	ENDDO
 ...
 ELSE
	...
	call gauss_legendre(X1,X2,XJTMP,GAUWTTMP,NQUADOINPUT/2)

	DO IQUAD=1,NQUADOINPUT/2
		! GAUSSIAN QUADRATURE AND WEIGHT IN OCEAN
		XJO(IQUAD+1)=-XJTMP(NQUADOINPUT/2-IQUAD+1)
		XJO(NQUADOINPUT+2-IQUAD)=-XJO(IQUAD+1)
		WTO(IQUAD+1)=GAUWTTMP(NQUADOINPUT/2-IQUAD+1)
		WTO(NQUADOINPUT+2-IQUAD)=WTO(IQUAD+1)
	ENDDO
 ENDIF
...
! SKY LIGHT TRANSMITTANCE
  ...
  ELSE
    ...
    DO JQUAD=1,NQDOD2
      ...
      ENDDO
      CALL RADINTPL2(Interp_ORDER_IN,NQDAD2,XJA,RADATMP1,NQDAD2,XJAD2,RADATMP)
...
! OCEAN LIGHT TRANSMITTANCE
  ...
  ELSE
    ...
    DO JQUAD=NQDAD2+1,NQUADA
      ...
      ENDDO
      CALL RADINTPL2(Interp_ORDER_IN,NQDOD2,XJO(NQDOD2+1),RADOTMP1,NQDOD2, &
	                 XJOD2,RADOTMP)
      DO IQUAD=NQDOD2+1,NQUADO
...
! ATMOSPHERIC SINGLE SCATTERING CORRECTION
  ...
  CALL SNGL_SCAT_CRRCT_ATMOS(MU_IN,NMUOUTSHFLA,&
         NPHIOUT,PHIOUT,MUOUTSHFLA,Interp_ORDER_IN)
...
! OCEAN SINGLE SCATTERING CORRECTION
  ...
  IF(RAD_MAP) THEN
      ...
      CALL RADOUT_MAPPING(MU_IN,IRRAD,IRRAD_A,IRRAD_O,NMUOUTSHFLA,&
            MUOUTSHFLA, NMUOUTSHFLO,MUOUTSHFLO,NPHIOUT,PHIOUT,Interp_ORDER_IN)
  ...
  DO ITHETA=1,NMUOUTSHFLA
    ...
    DO IPHI=1,NPHIOUT
      ...
      IF(MUSCAT>MINMUFWDCN)THEN
         ...
         ENDDO
          CALL RADINTPL2(Interp_ORDER_IN,NQDAD2,XJA,RADATMP,1,MUTMP,RADOUTTMP)
          ARRY1(:)=RADOUTTMP(1,:)
...
!  call gauss_legendre(-PI,PI,PHI_TMP,WPHI_TMP,NUMPHI)
...
DO IPHI=1,NPHIOUT
 ...
 ENDDO
 CALL RADINTPL2(Interp_ORDER_IN,NQUADA,XJA,RADDETK,NTHETAOUT,MUOUT,RADDETU)
 IF(SNG_RAD_CORR) THEN !SINGLE SCATTERING CORRECTION WITHOUT MAPPING
  DO ITHETA=1,NTHETAOUT
!    DO IMU=1,NMUOUTSHFLA
!     IF(ABS(MUOUT(ITHETA)-MUOUTSHFLA(IMU))<1.0E-4)EXIT
...
DOUBLE PRECISION :: THETA1,SINT1,SINT2,COST1,COST2,CAPA,LITA,LITB, &
	   MUDIFFSQ,MUDIFFQU,RTMP,RTMP1,TWOCOS,EXPBA,SHDWFCT
TYPE(MATRIX44),DIMENSION(:),ALLOCATABLE::RTMX
DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE :: HKWAR,HKWAT,HKWOT
INTEGER :: NUMPHI,IPHI
DOUBLE PRECISION :: THRESH1,THRESH2,THRESH3,PHIRG,E1O12
DOUBLE PRECISION :: SINMPHI,COSMPHI
DOUBLE PRECISION,DIMENSION(4,4)::FFINT,TEM1,TEM2
DOUBLE PRECISION,DIMENSION(:), ALLOCATABLE::PHI,WPHI

DOUBLE PRECISION,DIMENSION(:), ALLOCATABLE::ENGFCTAI,ENGFCTOI

  ! HKW() IS FUNCTIONS OF: VARIABLE MU',VARIABLE MU,ORDER K 

DOUBLE PRECISION expbessi,SHADOW,REFL_COEFF,TRAN_COEFF
LOGICAL FLAG1,FLAG2
...
!    PHIRG=ACOS(1.0D0-THRESH3/LITB)
  ...
  call gauss_legendre(-PHIRG,PHIRG,PHI,WPHI,NUMPHI)
  WPHI=WPHI/TWOPI
...
DOUBLE PRECISION MU0,SINT1,SINT2,COST1,COST2,CAPA,LITA,LITB, &
...
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: SHKWAR,SHKWAT
DOUBLE PRECISION,DIMENSION(4,4)::FFINT,TEM1,TEM2
DOUBLE PRECISION,DIMENSION(:), ALLOCATABLE::PHI,WPHI

DOUBLE PRECISION :: SSHDW,SHDWFCT

DOUBLE PRECISION expbessi,SHADOW
...
DOUBLE PRECISION :: TOL,X1,X2,MU0T,SIN0,SIN0T,MUTMP,FBTMAT,FBTMP
DOUBLE PRECISION :: THETA1,THETA2
DOUBLE PRECISION,DIMENSION(4,4) :: MTRX1
...
!call gauss_legendre(X1,X2,XJSO,WTSO,NQUADSO)
...
DOUBLE PRECISION :: TOL,X1,X2,COST1,COST2,SINT1,SINT2,MUTMP,FBTMAT,FBTMP
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: MUAT,WTMUAT
INTEGER :: IQUAD,JQUAD
...
DO IQUAD=1,NQDOD2
  ...
  IF(SINT1>1.0D0)THEN
    X2=0.0D0
    call gauss_legendre(X1,X2,MUAT,WTMUAT,NQDAD2)
...
DOUBLE PRECISION :: X1,X2,COST1,COST2,SINT1,SINT2
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: MUOT,WTMUOT
INTEGER :: IQUAD,JQUAD
...
DO IQUAD=NQDAD2+1,NQUADA
  ...
  X1=COST2/NWRE
  X2=1.0D0
  call gauss_legendre(X1,X2,MUOT,WTMUOT,NQDOD2)
  DO JQUAD=1,NQDOD2
...
DOUBLE PRECISION FUNCTION FBTMAT(MUP,MU)
USE RTTYPE
USE RTUTILITY
IMPLICIT NONE
DOUBLE PRECISION MUP,MU

DOUBLE PRECISION :: LITAT,LITBT,DNOM,SINTP,SINT
...
SUBROUTINE RADINTPL2(Interp_ORDER_IN,NK,XJK,RADKTMP,NU,XJU,RADUTMP) 
USE RTTYPE
USE RTUTILITY
IMPLICIT NONE

INTEGER,INTENT(IN) :: NK,NU,Interp_ORDER_IN
DOUBLE PRECISION, DIMENSION(NK,4),INTENT(IN)::RADKTMP
DOUBLE PRECISION, DIMENSION(NU,4),INTENT(OUT)::RADUTMP
DOUBLE PRECISION, DIMENSION(NK),INTENT(IN)::XJK
DOUBLE PRECISION, DIMENSION(NU),INTENT(IN)::XJU
DOUBLE PRECISION, DIMENSION(:),ALLOCATABLE::VSK,VSU

INTEGER :: ICOM,IR,Interp_ORDER
DOUBLE PRECISION :: RTMP

Interp_ORDER=Interp_ORDER_IN
RADUTMP=0.0d0
...
DO ICOM=1,4
   ...
   CALL UVIP3P(Interp_ORDER_IN,NK,XJK,VSK,NU,XJU,VSU)
...
DOUBLE PRECISION, INTENT(IN)::MU0,TAUDETS,PHI,MU
DOUBLE PRECISION,DIMENSION(4),INTENT(OUT)::ARRY1
DOUBLE PRECISION, DIMENSION(4,4)::FRES_TRAN,SLI1,SLI2,TEM1,TEM2
DOUBLE PRECISION :: COST1,SINT1,COST2,SINT2,CAPA,COSINOM,COSTNOM,MUDIFFSQ, &
          LITA,LITB,EXPBA,COST,NTGFCT,RTMP,RTMP1,RTMP2
DOUBLE PRECISION :: THRESH2,THETA1,THETA2,SA,SB,SC,SPHI,FA,FB,FC,FPHI,SHDWFCT,&
          SHADOW
COST1=MU0
SINT1=DSQRT(1.0D0-COST1*COST1)
...
DOUBLE PRECISION, INTENT(IN)::MU0,TAUDETS,PHI,MU
DOUBLE PRECISION,DIMENSION(4),INTENT(OUT)::ARRY1
DOUBLE PRECISION, DIMENSION(4,4)::FRES_REFL,SLI1,SLI2,TEM1,TEM2
DOUBLE PRECISION :: COST1,SINT1,COST2,SINT2,CAPA,CAPAP,MUDIFFSQ, &
          LITA,LITB,EXPBA, COST,MUNSQINV,NTGFCT,RTMP,RTMP2
DOUBLE PRECISION :: THRESH2,THETA1,THETA2,SA,SB,SC,SPHI,FA,FB,FC,FPHI,E1O12,&
          SHDWFCT,SHADOW
...
DOUBLE PRECISION FUNCTION SHADOW(U,SIGMASQ)
USE RTUTILITY, ONLY : PI

IMPLICIT NONE
DOUBLE PRECISION :: DERFC
DOUBLE PRECISION U,SIGMASQ
DOUBLE PRECISION USQ,SUSQ,NU,NUSR
...
DOUBLE PRECISION, INTENT(IN)::MU0,TAUDETS,PHI,MU
DOUBLE PRECISION,DIMENSION(4),INTENT(OUT)::ARRY1
DOUBLE PRECISION, DIMENSION(4,4)::TEM1
DOUBLE PRECISION :: COST1,SINT1,COST2,SINT2,RTMP
...
DOUBLE PRECISION,DIMENSION(4),INTENT(OUT)::ARRY1
DOUBLE PRECISION, DIMENSION(4,4)::TEM1
DOUBLE PRECISION :: COST1,SINT1,COST2,SINT2,WAVELENGTH_MICRON,RTMP
...
DOUBLE PRECISION :: SIGN,RTMP, RTMP1,RTMP2,RTMP3,INTCOEFF1,INTCOEFF2
REAL(DP),DIMENSION(4)   :: SRC0,SRC1,ARRY1
DOUBLE PRECISION,PARAMETER :: SLOPE_CUTOFF=100.0D0
...
INTEGER(I4B),INTENT(IN) :: NMUOUTSHFLA,NMUOUTSHFLO,NPHIOUT,Interp_ORDER_IN
DOUBLE PRECISION,DIMENSION(NMUOUTSHFLA),INTENT(IN) :: MUOUTSHFLA
DOUBLE PRECISION,DIMENSION(NMUOUTSHFLO),INTENT(IN) :: MUOUTSHFLO
DOUBLE PRECISION,DIMENSION(NPHIOUT),INTENT(IN) :: PHIOUT
...
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XJAD2,XJOD2
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: RADATMP,RADATMP1,RADOUTTMP,RADOTMP,RADOTMP1

TYPE(ARRAY4),DIMENSION(:,:),ALLOCATABLE :: LRADATMP,LSPATMP,LRADATMP1,&
                        LSPATMP1,LSPOTMP, LRADOTMP
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: MUTMP

DOUBLE PRECISION :: COST1,COST2,MUSCAT,SINT1,SINT2,THETA1,THETA2
LOGICAL :: FLAG1,FLAG2
DOUBLE PRECISION,PARAMETER :: SLOPE_CUTOFF=100.0D0
...
! UPDATE LSPA_OUT
    CALL RADINTPL2(Interp_ORDER_IN,NQDAD2,XJA(NQDAD2+1),RADATMP,&
...
IF((OCEAN .AND. (.NOT. FLATO) .AND. DETA .AND. (.NOT.WLR_FLAG)) &
  ...
  DO M=0,MAXMORD
    ...
    CALL RADINTPL2(Interp_ORDER_IN,NQDAD2,XJA(NQDAD2+1),RADATMP,NMUOUTSHFLA,MUOUTSHFLA,RADOUTTMP)
...
DOUBLE PRECISION :: RTMP, RTMP1,RTMP2,RTMP3,THETA1,THETA_IN
DOUBLE PRECISION,DIMENSION(4)   :: Thermal_Arry,ARRY1,ARRY2,ARRY3,ARRY4
DOUBLE PRECISION,DIMENSION(4,4) :: FRES_MTRX,MTRX1,MTRX2
...
INTEGER(I4B),INTENT(IN) :: NMUOUTSHFLA,NPHIOUT,Interp_ORDER_IN
DOUBLE PRECISION,DIMENSION(NMUOUTSHFLA),INTENT(IN) :: MUOUTSHFLA
DOUBLE PRECISION,DIMENSION(NPHIOUT),INTENT(IN) :: PHIOUT
...
DOUBLE PRECISION :: RTMP, RTMP1,RTMP2,THETA1,COST1,COST2,PHI1,PHI2,&
...
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: RADATMP,RADATMP1

INTEGER :: NHEIGHTINT !SUB INTERVALS TO PERFORM HEIGHT INTEGRAL FOR
                      ! SINGLE SCATTERING SPHERICAL SHELL SOLUTION
DOUBLE PRECISION :: SECANT_THETAL_VIEWB,SECANT_THETAL_VIEWT,SECANT_THETAL_LAYER,&
          SECANT_THETAL_LOCAL,SLANT_TAU_LOCAL
DOUBLE PRECISION :: TAUTEMP,TAUTEMP1,TAU_INTERVAL,HEIGHT_TEMP,HEIGHT_INTERVAL,SINTV,&
          SOURCETEMP,WEIGHT,DIST0,LDIS
DOUBLE PRECISION :: WAVELENGTH_MICRON
logical :: emiss_lut_outside=.false.
!INTEGER :: NUMPHI,IPHI_TMP
!DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: PHI_TMP,WPHI_TMP
!DOUBLE PRECISION :: XIFUNC
Thermal_Arry=0.0D0
...
! ROUGH OCEAN SURFACE CONTRIBUTION TO SINGLE SCATTERING CORRECTION
  ...
  DO IDTAU=1,NDTAUA
    ...
    DO IPHI=1,NPHIOUT
      ...
      ENDDO
      CALL RADINTPL2(Interp_ORDER_IN,NQUADA,XJA,RADATMP,NMUOUTSHFLA, &
	               MUOUTSHFLA,RADATMP1)
    DO ITHETA=1,NMUOUTSHFLA
      LRADA_SNG(ITHETA,INDXDETA_SNGSCAT(IDTAU),IPHI)%VRAD(:)=         &
         LRADA_SNG(ITHETA,INDXDETA_SNGSCAT(IDTAU),IPHI)%VRAD(:) + &
...
DOUBLE PRECISION :: RTMP, RTMP1,RTMP2,THETA1,THETA_IN
...
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: RADOTMP,RADOTMP1
...
DOUBLE PRECISION,INTENT(IN) :: MU0,PHI0,MUVIEW,PHIVIEW
INTEGER,INTENT(IN)  :: IMIE
DOUBLE PRECISION,DIMENSION(4,4),INTENT(OUT)::PHASE_MATRIX

DOUBLE PRECISION :: MUSCAT,PHMXELEM

DOUBLE PRECISION, DIMENSION(4,4)::SLI1,SLI2,TEM1,TEM2
DOUBLE PRECISION :: COST1,SINT1,COST2,SINT2,RTMP
DOUBLE PRECISION :: SA,SB,SC,SPHI,FA,FB,FC,FPHI
DOUBLE PRECISION, DIMENSION(:),ALLOCATABLE::VSK,XJMIE
INTEGER :: IMIEANG,ICOM1,ICOM2,Interp_ORDER,IULO,KLO
DOUBLE PRECISION,DIMENSION(1) :: XI,YI
ALLOCATE(VSK(NUMMIEANG(IMIE)),XJMIE(NUMMIEANG(IMIE)))

DO IMIEANG=1,NUMMIEANG(IMIE)
  XJMIE(IMIEANG)=MU_SNG(IMIE,NUMMIEANG(IMIE)-IMIEANG+1)
ENDDO
...
DO ICOM2=1,4
   ...
   DO IMIEANG=1,NUMMIEANG(IMIE)
      VSK(IMIEANG)=PM_AO_SNG(IMIE,NUMMIEANG(IMIE)-IMIEANG+1)%PHMX(ICOM1,ICOM2)
   ENDDO

   CALL UVIP3P(Interp_ORDER,NUMMIEANG(IMIE),XJMIE,VSK,1,XI,YI)
   TEM1(ICOM1,ICOM2)=YI(1)
...
DOUBLE PRECISION,DIMENSION(4,4):: MTRX1
DOUBLE PRECISION,DIMENSION(4):: ARRY1,ARRY2
...
INTEGER :: IULO,Interp_ORDER,KLO
DOUBLE PRECISION :: RTMP,RTMP1,RTMP2
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: EXTARRYTMP
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: SOURCEOTMP,SOURCEOM0TMP

DOUBLE PRECISION,DIMENSION(3)::QSRC,ELCOEFF
DOUBLE PRECISION :: SQRTSIGN,TAUDIFF,TAUDIFF1,SRCTMP
...
INTEGER :: IULO,Interp_ORDER,KLO
DOUBLE PRECISION :: RTMP,RTMP1,RTMP2,RTMP3
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: EXTARRYTMP
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: SOURCEOTMP,SOURCEOM0TMP
...
DO ITAU=OLYR(I)%ITAUS,OLYR(I)%ITAUE
  ...
  EXTARRYTMP(ITAU)=Func_UVIP3P(NWATER_DEPTH,TAU_INELASTIC_AGD,EXT_COEFF_INELASTIC_EM,RTMP1,Interp_ORDER)
...
DO ITAU=OLYR(I)%ITAUS,OLYR(I)%ITAUE
   RTMP1=TAU_PRSV(ITAU)-OITAU_PRSV
   IULO=bisearch(NWATER_DEPTH,TAU_INELASTIC_AGD,RTMP1)
   KLO=min(max(IULO-(Interp_ORDER-1)/2,1),NWATER_DEPTH-Interp_ORDER)
...
DOUBLE PRECISION :: SINT1,SINT2,TAN1,TAN2,COST,THETA1,THETA2,SHDWFCT,&
          SPHI,FPHI,SA,SB,SC,FA,FB,FC,NTGFCT,RTMP,Gfac,hotfac
DOUBLE PRECISION,DIMENSION(4,4) :: TEM1,TEM2,FRES_MTRX,SLI1,SLI2
SINT1=DSQRT(1.0D0-COST1*COST1)
SINT2=DSQRT(1.0D0-COST2*COST2)

! hot spot function evaluation follows Eq. 5-6 of Rahman et al., 1993
! https://doi.org/10.1029/93JD02072
TAN1=abs(SINT1/COST1)
TAN2=abs(SINT2/COST2)
Gfac=sqrt(TAN1*TAN1+TAN2*TAN2+2.0d0*TAN1*TAN2*COS(PHI2-PHI1))
hotfac=1.0D0+(1.0D0-pBRDFa)/(1+Gfac)
...
! The Netherlands, SRON-SPEXoneL1-2020-02, issue: 0.90, 2020
	...
	FRES_MTRX=pBRDFe*EXP(-tan(THETA1))*EXP(-vfac)*TEM1/(4.0d0*(abs(COST1)+abs(COST2)))
    BRDML=FRES_MTRX*abs(COST1)
...
BRDML(1,1)=BRDML(1,1)+pBRDFa*ABS(COST1)*exp(pBRDFb*COST)* &
       (ABS(COST1*COST2)*(ABS(COST1)+ABS(COST2)))**(pBRDFk-1)*hotfac
...
DOUBLE PRECISION,INTENT(IN) :: COST1,PHI1,COST2,PHI2,NWRE,NWIM
DOUBLE PRECISION,DIMENSION(4,4),INTENT(OUT) :: BRDMO

DOUBLE PRECISION :: SINT1,SINT2,CAPA,MUDIFFSQ,LITA,LITB,SHDWFCT,EXPBA,&
...
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XJINT,WJINT,PHI,WPHI

DOUBLE PRECISION,DIMENSION(4,4) :: TEM1,TEM2
  NUMPHI=40
  NUMXJ=40
  ALLOCATE(XJINT(NUMXJ),WJINT(NUMXJ),PHI(NUMPHI),WPHI(NUMPHI))
  call gauss_legendre(-PI/6,PI/6,PHI,WPHI,NUMPHI)
  call gauss_legendre(-0.4D0,0.4D0,XJINT,WJINT,NUMXJ)
...
DOUBLE PRECISION,INTENT(IN) :: COST1,PHI1,COST2,PHI2,NWRE,NWIM
DOUBLE PRECISION,DIMENSION(4,4),INTENT(OUT) :: BTDMO
...
DOUBLE PRECISION,INTENT(IN) :: COST1,PHI1,COST2,PHI2,NRELRE,NRELIM
DOUBLE PRECISION,DIMENSION(4,4),INTENT(OUT) :: BTDMO
...
INTEGER :: IMU,IPHI
DOUBLE PRECISION :: RTMP,THETA_IN
DOUBLE PRECISION,DIMENSION(3,3):: MTRX_LCL2GLB
DOUBLE PRECISION,DIMENSION(3):: VEC1,VEC2

DOUBLE PRECISION,DIMENSION(NMUFWDCN):: WMU_LCL
DOUBLE PRECISION,DIMENSION(NPHIFWDCN):: WPHI_LCL

DOUBLE PRECISION :: PHI_UPDT
...
DOUBLE PRECISION FUNCTION PHI_UPDT(VECTOR3)
USE RTTYPE
implicit none
DOUBLE PRECISION,DIMENSION(3),INTENT(IN)::VECTOR3
...
DOUBLE PRECISION :: RTMP, RTMP1,RTMP2,COST1,COST2,PHI1,PHI2,MUSCAT,&
          SINT1,SINT2
DOUBLE PRECISION,DIMENSION(4)   :: ARRY1,ARRY2
DOUBLE PRECISION,DIMENSION(4,4) :: MTRX1
!LOGICAL :: DIFF_FLG
...
implicit none
DOUBLE PRECISION,INTENT(IN) :: MU_IN
DOUBLE PRECISION,INTENT(IN)::COSTRST,PHIRST
DOUBLE PRECISION,DIMENSION(4),INTENT(OUT)::VRADRST

INTEGER :: IMU,IPHI,IPHI1
DOUBLE PRECISION,DIMENSION(3,3):: MTRX_GLB2LCL
DOUBLE PRECISION,DIMENSION(3):: VEC1,VEC2
DOUBLE PRECISION :: SINTRST,PHILCL,MULCL,WT1,WT2,WT3,WT4

DOUBLE PRECISION :: PHI_UPDT,RTMP,THETA_IN
...
DOUBLE PRECISION,INTENT(IN) :: COST1,PHI1,COST2,PHI2
DOUBLE PRECISION,INTENT(OUT) :: RADCORR

INTEGER :: IMIE,IL_AVG,ILTERM
DOUBLE PRECISION :: PHASE_FUNCTION_RESIDUE,XIFUNC

DOUBLE PRECISION :: PFRSD,PSPIKE,PSPIKE_AVG,PHINT

INTEGER :: IANG
DOUBLE PRECISION :: SINT,SINT1,SINT2,SINT1P,MUSCAT,MUMIN,MUMAX,MU1,&
          COEFCI,COEFDI,COEFCP,COEFDP,WTANG,COST1P,RTMP,RTMP1,&
          WTTOTAL,LBDOAVG,FTRUNCAVG,FBAVG
LOGICAL :: CUTTING

INTEGER :: NK,IMIN,IMAX,KINT,ILYINX
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: XJK,PLNML,YJK
DOUBLE PRECISION,DIMENSION(1) :: XI,YI
...
ALLOCATE(XJK(NUMMIEANGMAX),YJK(NUMMIEANGMAX))
XI=MUSCAT
...
YJK(1:NK)=PHF_RESIDUE(IMIE,1:NK)
CALL UVIP3P(2,NK,XJK,YJK,1,XI,YI)
PFRSD=YI(1)
PSPIKE=0.0D0
...
DO IANG=1,NUMMIEANG(IMIE)
  ...
  ELSE
    ...
    IF(MUMAX>MUMIN)THEN
	  IMIN=bisearch(NK,XJK,MUMIN)
	  IMAX=bisearch(NK,XJK,MUMAX)
...
ELSE  ! USE EXPANSION SCHEME IN DISORT REPORT
  ...
  NK=MAXVAL(NUMORDFULLBETAL)
  ALLOCATE(PLNML(0:NK))
  CALL poly_leg(MUSCAT,PLNML(0),NK+1)  
...
DOUBLE PRECISION FUNCTION PHASE_FUNCTION_RESIDUE(IMIE,MUSCAT)
USE RTTYPE
USE RTUTILITY
USE PHASE_MATRIX_TRUNCATION
IMPLICIT NONE
INTEGER,INTENT(IN) :: IMIE
DOUBLE PRECISION,INTENT(IN) :: MUSCAT

DOUBLE PRECISION,ALLOCATABLE,DIMENSION(:)::DL00,DL20,DL2P2,DL2N2
DOUBLE PRECISION,ALLOCATABLE,DIMENSION(:)::XJK,VSK
DOUBLE PRECISION,DIMENSION(1) :: XI,YI

INTEGER :: NK,Interp_ORDER,L
DOUBLE PRECISION :: PHFFIT,PHFORI,RTMP
...
Interp_ORDER=2
XI=MUSCAT
CALL UVIP3P(Interp_ORDER,NK,XJK,VSK,1,XI,YI)
PHFORI=YI(1)
DEALLOCATE(VSK,XJK)
...
DOUBLE PRECISION FUNCTION XIFUNC(TAUCORR,COST2,COST1P)
DOUBLE PRECISION,INTENT(IN) :: TAUCORR,COST2,COST1P
DOUBLE PRECISION :: RTMP
...
DOUBLE PRECISION :: PHASE_FUNCTION_RESIDUE
...
DOUBLE PRECISION,dimension(:),allocatable::muang,p11
...
DOUBLE PRECISION,dimension(numanglocal),intent(in)::muang,p11
DOUBLE PRECISION,dimension(0:maxlfull),intent(out)::fullbetal_local

INTEGER :: ITHETA,ILTERM

DOUBLE PRECISION :: RTMP
DOUBLE PRECISION,DIMENSION(:,:),ALLOCATABLE :: PLNML
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: TWOX,F1TMP,F2TMP,DTMP,WTANG,PLNMLtmp
...
INTEGER,INTENT(OUT) :: NMUOUTSHFLA,NMUOUTSHFLO
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: MUOUTSHFLA,MUOUTSHFLO
DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: MUOUTTMP
INTEGER :: IMUOUT
DOUBLE PRECISION :: RTMP,THETA1,THETA2
...
DOUBLE PRECISION FUNCTION WHITECAP_REFLECTANCE(WAVELENGTH_MICRON)
...
DOUBLE PRECISION,DIMENSION(NDIM) :: WAVELENGTH_TABLE=(/0.503702783,0.595283098,    &
...
DOUBLE PRECISION,DIMENSION(NDIM) :: AWC=(/21.95668372,22.03437283,21.98116722,&
...
ELSE IF (WAVELENGTH_MICRON>WAVELENGTH_TABLE(NDIM))THEN
    AWC_LOCAL=AWC(NDIM)
ELSE
	AWC_LOCAL=Func_UVIP3P(NDIM,WAVELENGTH_TABLE,AWC,WAVELENGTH_MICRON,2)
ENDIF
