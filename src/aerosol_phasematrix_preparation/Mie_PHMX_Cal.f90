...
REAL*8,DIMENSION(:,:,:),ALLOCATABLE ::PHMXMIE_ARSL_PACE

CONTAINS

SUBROUTINE PHMXMIE_ALLO
ALLOCATE(CEXT_ARSL_BAND(NWV_BAND),CSCAT_ARSL_BAND(NWV_BAND), &
		 PHMXMIE_ARSL_PACE(NWV_BAND,NUMMIEANGMAX,0:6))
END SUBROUTINE PHMXMIE_ALLO

SUBROUTINE PHMXMIE_DEALLO
DEALLOCATE(CEXT_ARSL_BAND,CSCAT_ARSL_BAND,PHMXMIE_ARSL_PACE)
...
IF(FREEFORMFLAG==1 .AND. (IAEROSOL==-98 .or. IAEROSOL==-96))THEN
	...
	DO IWV_BAND=NWV_BAND_START,NWV_BAND_END
		...
		PHMXMIE_ARSL_PACE(IWV_BAND,:,:)= PHMXMIE_LOCAL1(:,:)
    ENDDO
	write(*,*) '**********************************************'
        ...
        WRITE(*,*)'dust mode number density',ARSLNDD
          ...
          DO IWV_BAND=NWV_BAND_START,NWV_BAND_END
            ...
            ELSEIF(DSFRAC>0 .OR. DFRAC>0)THEN
                PHMXMIE_ARSL_PACE(IWV_BAND,:,0)=PHMXMIE_LOCALD_MIE(IWV_BAND,:,0)
             ENDIF
...
ENDIF
    ...
    CSCAT_ARSL_BAND=ARSLND1*CSCATF+ARSLND2*CSCATC
     ...
     DO IWV_BAND=NWV_BAND_START,NWV_BAND_END
        ...
        PHMXMIE_ARSL_PACE(IWV_BAND,:,0)=PHMXMIE_LOCAL1A_WS(IWV_BAND,:,0)
        ELSEIF (ARSLND1_BC>0)THEN
        PHMXMIE_ARSL_PACE(IWV_BAND,:,0)=PHMXMIE_LOCAL1A_BC(IWV_BAND,:,0)
        ELSEIF (ARSLND1_S>0)THEN
        PHMXMIE_ARSL_PACE(IWV_BAND,:,0)=PHMXMIE_LOCAL1A_S(IWV_BAND,:,0)
