#author: Pengwang Zhai, Associate Professor, UMBC. Email: pwzhai@umbc.edu
import numpy as np
import random

def input_writer(filestrbase,ichla,itheta,itau,iaerosol,iadg,ibbp,iBp):
    ...
    f.write("%f" % adg440[iadg] + '        #adg440 (1/m), range: 0.0:2.5 \n')
    f.write("%f" % bbp660_BackscatterCoeff[ibbp] + '        #bbp660 (1/m), range 0:0.1\n')
    f.write("%f" % Bp660_BackscatterFraction[iBp] + '      #Bp660_BackscatterFraction, range 0"0.05 \n')
...
irh=5
wndspd=5.0
theta0=np.array([35.0,40.0,45.0,50.0,55.0,60.0])
wv_pace_ref=873.0
tau_ref=np.array([0.05,0.15])

Height_Particle=3.0
Aerosol_Model=np.array([14,17])
...
adg440=np.array([0.5, 1.0, 2.5])   #adg440 (1/m), range: 0.0:2.5
bbp660_BackscatterCoeff=np.array([0.01, 0.05])    #bbp660 (1/m), range 0:0.1
Bp660_BackscatterFraction=np.array([0.01,0.03])   #Bp660_BackscatterFraction, range 0:0.05
...
S_Bp=0.0                          #S_Bp power spectral slope of backscattering fraction (1/nm) range: -0.2:0.2
	...
	for itheta in range(len(theta0)):
		for itau in range(len(tau_ref)):
			for iaerosol in range(len(Aerosol_Model)):
				for iadg in range(len(adg440)) :
					for ibbp in range(len(bbp660_BackscatterCoeff)) :
						for iBp in range(len(Bp660_BackscatterFraction)) :
							filestrbase='OceanCase%d' % ocean_case_select \
										+ 'tau_ref_%05.2f' % tau_ref[itau]       \
										+'chla%05.2f' % chla[ichla] \
										+'theta0_%05.2f' %theta0[itheta] \
										+'aerosolmodel%d'%Aerosol_Model[iaerosol] \
										+'adg440%05.2f' %adg440[iadg] \
										+'bbp660%05.2f' %bbp660_BackscatterCoeff[ibbp] \
										+'Bp660%05.2f' %Bp660_BackscatterFraction[iBp]
							input_writer(filestrbase,ichla,itheta,itau,iaerosol,iadg,ibbp,iBp)
