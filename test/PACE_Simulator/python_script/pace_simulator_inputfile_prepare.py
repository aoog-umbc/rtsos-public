...
def input_writer(filestrbase,ichla,itheta,ipss,itau,iocean):
...
irh=5
wndspd=5.0
theta0=np.array([20.0, 60.0])
wv_pace_ref=532.0
tau_ref=np.array([0.1,0.3])
...
for ichla in range(len(chla)):
    for itheta in range(len(theta0)):
        for itau in range(len(tau_ref)):
            for iocean in range(len(ocean_case_select)):
                filestrbase='OceanCase%d' % ocean_case_select[iocean] \
                    ...
                    +'pss%d'%pss_flag[ipss]
                input_writer(filestrbase,ichla,itheta,ipss,itau,iocean)
